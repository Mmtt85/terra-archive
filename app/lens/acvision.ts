...
// 게이트는 값이 싼 순서로 세 겹이다 — ① 화면 서명(여기, 공짜) ② 행 위생(여기, 공짜)
//   ③ 맹약 아이콘 매칭(acbond.ts, 행당 ≈71ms — ①②를 통과한 행에만 돈다).
//
...
// ② 행 위생 — 진짜 밴 행의 모양을 못박는다. 조건별 실측(밴 구간 밖 오탐 행 49개 → 누적):
//      col 0 부터 빈칸 없이 연속 → 5   (주력. 오탐은 c1 c2 c5 c6 처럼 뚝뚝 끊긴다)
//      + 카드 2장 이상          → 1   (한 장짜리 허깃 행을 쓸어낸다)
//      + 행 안 red 폭 ≤ 0.10    → 0   (진짜 최대 0.058 vs 가장 가까운 오탐 0.15)
//      + red 최대 ≤ 0.55        → 0   (실측 잉여지만 비용 0 이고 '고르게 새빨간' 잡음의 유일한 방어)
//    온전한 행 검출은 87/89 그대로 — **잃는 행이 없다**. ban1.jpg 0행 · ban2.jpg 5행도 그대로.
//    ⚠ red **하한**은 넣지 마라 (진짜 최소 0.19 vs 오탐 0.14 로 붙어 있다).
...
//      (오탐 0 · 87/89 · f015 살음) 진짜 한 장 행을 구제할 여지가 남는다.
const RED_SPREAD_MAX = 0.10, RED_MAX = 0.55;

function rowClean(r: AcRow): boolean {
  if (r.cut || !r.cards.length) return false;
  if (!r.cards.every((c, i) => c.col === i && c.tier !== null)) return false;
  const reds = r.cards.map((c) => c.red);
  return Math.max(...reds) - Math.min(...reds) <= RED_SPREAD_MAX && Math.max(...reds) <= RED_MAX;
}

/** 밴으로 받아도 되는 행만 — 위 ② 규칙. 프레임이 밴 화면인지(①)는 호출자가 isAcBanScreen 으로 먼저 본다. */
