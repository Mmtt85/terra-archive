  ...
  return (
    <ModalWindow className="operator-modal lens-modal" onClose={onClose} label={`◉ ${t("PRTS 시뮬레이션")}`}
      ...
      <div className="lens-body lens-help">
        ...
        <h3>{t("무엇을 읽나")}</h3>
        <ul>
          <li>{rich(t("**밴 기물** — 시작 화면의 '사용 제한 오퍼레이터' 카드를 얼굴로 알아봅니다. 목록은 스크롤해야 다 보이므로 25초 안에 끝까지 내려 주세요. 인식은 화면이 멈춘 순간에 일어나니 조금씩 내린 뒤 한 번씩 완전히 멈춰야 하고, 화면에 온전히 보인 카드만 셉니다."))}</li>
          <li>{rich(t("**시뮬레이션 종류** — 표준·험지·극한·초월을 로딩 화면과 화면 위쪽 붉은 배지에서 읽습니다."))}</li>
          <li>{rich(t("**전략** — '선택한 전략' 확정 화면에서 내가 고른 전략을 편성기에 자동으로 넣습니다. 연합에서 다른 참가자의 전략은 참가자 카드의 그림으로 알아봅니다 (아직 실플레이 확인 전)."))}</li>
          <li>{rich(t("**맹약 중첩 수** — 편성 계산기가 유일하게 손으로 받던 값입니다. 전투 화면 위쪽 맹약 원형에서 읽어 자동으로 채웁니다."))}</li>
          <li>{rich(t("**배치 가능 인원** — 남은 배치 칸을 따라가고, 9가 보이면 '인사부 파일'을 쓴 것으로 보고 9번째 배치 칸을 열어 줍니다."))}</li>
