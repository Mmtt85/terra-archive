...
//
// ⚠ **모달은 예외 없이 전부 이 창을 쓴다** (사용자 재지시 2026-09-05: "간이 모달 같은 거
//   없애고 모든 모달을 기존에 쓰던 모달창으로 싹 다"). 제목줄·×를 직접 그린 <section>을
//   .modal-backdrop 으로 감싸는 '간이 모달'을 새로 만들지 말 것 — 그날 전수조사로 7곳
//   (스샷 레이더/PRTS/클립보드 도움말 3종 · 오퍼 스캐너 · PRTS 리플레이 · 록라 테마 선택 ·
//   업데이트 내역 상세)을 여기로 모았다. 창 안에서 또 창을 열어도 된다 (z가 알아서 쌓인다).
//
// ⚠ **형제로 나란히 둔 창의 key 에 맨숫자를 쓰지 말 것.** 창을 앞으로 끌어올리려고
//   `key={raise}` 같은 카운터를 주는데(z는 마운트 때 정해진다), 작전창·적창·재료창이
//   각자 1이 되는 순간 **서로 key가 같아진다** — React가 둘을 같은 창으로 보고 DOM을
//   재사용한다 (실측: /stages·/items·/farm·/sim 에서 "two children with the same key `1`").
//   `key={`st-${stageRaise}`}` 처럼 **무슨 창인지 접두**를 붙인다 (2026-09-17 전수조사).
//
// 대상 밖 — **모달이 아닌 것들만**:
