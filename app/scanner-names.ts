...
const NAME_BY_ID = new Map<string, string>(); // opId → KR 표시명 (학습 인덱스 식별 표시용)
...
export function nameOf(opId: string): string { return NAME_BY_ID.get(opId) ?? opId; }
...
export async function initNames(): Promise<void> {
  ...
  loadingPromise = (async () => {
    ...
    const out: Cand[] = [];
    for (const op of ops) NAME_BY_ID.set(op.id, op.name);
