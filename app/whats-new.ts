// 신규 기능 배지 — 기능 출시일로부터 **7일간** '새기능'을 표시한다
// (사용자 정책 2026-07-23, 2026-07-26에 "모든 배지는 넣은 날짜부터 7일"로 통일).
// 새 기능을 추가하면 여기에 키와 출시일(YYYY-MM-DD, KST 자정 기준)만 등록하면 된다.
// days를 지정하면 예외로 그 기간을 쓰지만, 기본은 7일로 두는 것이 정책이다.
// 판정은 클라이언트 렌더 시각 기준.
export const FEATURE_RELEASED: Record<string, string | { date: string; days: number }> = {
  ...
  invest: "2026-07-24", // 인프라 오퍼 육성 추천 (한때 3일이었으나 7일 통일 정책으로 복귀)
