...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return enemyMetadata("ko", id);
}

export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const { enemy, stages } = enemyPageData("ko", id);
  return (
    <>
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(enemyJsonLd("ko", id)) }}
      />
      <HomeKo initialTab="enemy" initialEnemy={id} pageEnemy={enemy} pageEnemyStages={stages} />
    </>
  );
}
