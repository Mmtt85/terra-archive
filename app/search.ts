...
// 처음 1초로 넣었다가 "너무 느리다"고 하향 — 다시 올리지 말 것.
// 한 글자마다 450명 로스터·수천 항목을 걸러 다시 그리던 비용이 사라진다 — 입력 지연의
// 실측 원인은 계산(1~8ms)이 아니라 매 글자 목록 재렌더·재페인트였다.
//
// ⚠ 왜 하필 350ms인가 (CLS 실측 2026-08-02): Chrome은 **마지막 입력 후 500ms까지만**
// 레이아웃 이동을 CLS에서 면제한다(hadRecentInput). 종전 값 500ms는 이 경계와 정확히
// 같아서, 디바운스 발화 + 렌더가 **항상 면제 창 밖**에 떨어졌다 — 검색할 때마다 그리드
// 재정렬이 통째로 CLS로 집계됐고(/operators 필드 데이터 operator-card 0.148×42건),
// 실측으로도 재현됐다(0.063, recentInput=false). 350ms면 렌더에 150ms 여유가 있어
// 대부분의 기기에서 면제 창 안에 든다. 500ms 이상으로 되돌리면 안 된다.
