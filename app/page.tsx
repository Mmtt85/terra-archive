...
const combatTags = Array.from(new Set(operators.flatMap((operator) => operator.combatTags))).filter((tag) => tag.trim().length > 0).sort((a, b) => a.localeCompare(b, "ko"));
...
// 한국 시각(KST) 기준으로 표기 — KR 팬사이트 기준
function fmtDate(iso: string, withTime: boolean) {
...
function shortStatus(b: Broadcast, now: number): string {
  const state = bcastState(b, now);
  if (state === "live") return "생방송 중";
  if (state === "upcoming") {
    const ms = Date.parse(b.start) - now;
    return ms < HOUR ? "곧 시작" : ms < DAY ? `${Math.round(ms / HOUR)}시간 후` : `D-${Math.ceil(ms / DAY)}`;
  }
  return new Intl.DateTimeFormat("ko-KR", { timeZone: "Asia/Seoul", month: "numeric", day: "numeric" }).format(new Date(b.start));
}
...
export default function Home() {
  ...
  const handleSubmitNickname = async (opId: string, rawName: string): Promise<string> => {
    ...
    if (name.length > 16) return "별명은 16자 이내로 부탁드려요";
    if (nickAlreadySent(opId, name)) return "이미 이 별명을 제보하셨어요, 감사합니다!";
  ...
  useEffect(() => {
    const base = "테라 아카이브 | 명일방주(Arknights) KR 팬사이트";
    document.title = selected
      ? `${selected.name} - 명일방주 오퍼레이터 | 테라 아카이브`
      : tab === "planner"
        ? "인프라 플래너 - 명일방주 기반시설 편성 | 테라 아카이브"
        : tab === "recruit"
          ? "공채 도우미 - 명일방주 공개모집 계산기 | 테라 아카이브"
          : base;
  }, [tab, selected]);
  ...
  const filtered = useMemo(() => {
    ...
    return operators.filter((operator) => {
      ...
      const matchesQuery = !keyword || [operator.name, operator.code, operator.job, operator.subProfession, operator.position, ...operator.combatTags, ...operator.factions, operator.reason, ...operator.aliases, ...communityNicknames, ...operator.concepts].join(" ").toLowerCase().includes(keyword);
      return matchesFaction && matchesConcept && matchesMethod && matchesTags && matchesJob && matchesSubProfession && matchesQuery;
    });
  }, [selectedFactions, selectedConcepts, selectedMethods, tags, selectedJobs, selectedSubProfessions, query, nicknames]);
    ...
    <main className={tab !== "archive" ? "base-main" : ""}>
      <header className="site-header" id="top">
        ...
        <nav className="main-tabs" aria-label="주요 탭">
          <button className={`tab-archive${tab === "archive" ? " selected" : ""}`} onClick={() => switchTab("archive")}><span className="tab-icon" aria-hidden>▤</span>오퍼 백과사전</button>
          <button className={`tab-planner${tab === "planner" ? " selected" : ""}`} onClick={() => switchTab("planner")}><span className="tab-icon" aria-hidden>⌂</span>인프라 플래너</button>
          <button className={`tab-recruit${tab === "recruit" ? " selected" : ""}`} onClick={() => switchTab("recruit")}><span className="tab-icon" aria-hidden>◎</span>공채 도우미</button>
        </nav>
      ...
      {tab === "archive" && <section className="explorer" aria-labelledby="explorer-title">
        <div className="filter-panel">
          ...
          <FilterGroup title="컨셉덱" items={concepts} selected={selectedConcepts} onToggle={toggleIn(setSelectedConcepts)} rows={2} countForItem={(item) => operators.filter((operator) => operator.concepts.includes(item)).length} />
          <FilterGroup title="직군" items={jobs} selected={selectedJobs} onToggle={toggleIn(setSelectedJobs)} countForItem={(item) => operators.filter((operator) => operator.job === item).length} />
          <FilterGroup title="세부 직군" items={subProfessions} selected={selectedSubProfessions} onToggle={toggleIn(setSelectedSubProfessions)} countForItem={(item) => operators.filter((operator) => operator.subProfession === item).length} />
          <FilterGroup title="전투 태그" items={combatTags} selected={tags} onToggle={toggleTag} countForItem={(item) => operators.filter((operator) => operator.combatTags.includes(item)).length} />
          <FilterGroup title="공격 방식" items={attackMethods} selected={selectedMethods} onToggle={toggleIn(setSelectedMethods)} countForItem={(item) => operators.filter((operator) => POSITION_METHODS.includes(item) ? operator.position === item : damageTypeOf(operator) === item).length} />
          <FilterGroup title="공식 소속" items={factions} selected={selectedFactions} onToggle={toggleIn(setSelectedFactions)} countForItem={(item) => operators.filter((operator) => operator.factions.includes(item)).length} />
        ...
        <div className="results">
          ...
          {sorted.length > 0 ? (
            ...
            <div className="empty"><span>NO MATCH</span><h3>조건에 맞는 오퍼레이터가 없어요.</h3><p>소속이나 컨셉 태그를 하나씩 해제해 보세요.</p><button onClick={reset}><span className="btn-icon" aria-hidden>↻</span>전체 보기</button></div>
          )}
        </div>
      </section>}
...
function FilterGroup({ title, items, selected, onToggle, rows = 1, countForItem }: { title: string; items: string[]; selected: string[]; onToggle: (value: string) => void; rows?: number; countForItem: (item: string) => number }) {
  ...
  return (
    <fieldset>
      ...
      {(hiddenCount > 0 || expanded) && (
        <button className="more-filter" type="button" onClick={() => setExpanded((current) => !current)} aria-expanded={expanded}><span className="btn-icon" aria-hidden>{expanded ? "▴" : "▾"}</span>{expanded ? "접기" : `더보기 +${hiddenCount}`}</button>
      )}
    </fieldset>
  );
}
...
function NicknameForm({ operator, onSubmit }: { operator: Operator; onSubmit: (opId: string, name: string) => Promise<string> }) {
  ...
  return (
    <div className="nickname-row">
      <span className="nickname-label">별명 제보</span>
      <form className="nickname-form" onSubmit={submit}>
        <input value={draft} maxLength={16} placeholder="이 오퍼의 별명 (16자 이내)"
          onChange={(event) => { setDraft(event.target.value); setMessage(""); }} />
        <button type="submit" disabled={busy || !draft.trim()}>{busy ? "전송 중…" : <><span className="btn-icon" aria-hidden>✎</span>제보</>}</button>
      </form>
