...
export type ChangeArea = "infra" | "archive" | "enemy" | "recruit" | "farm" | "upgrade" | "story" | "rogue" | "site";
...
export const CHANGE_AREA_LABEL: Record<ChangeArea, string> = {
  infra: "인프라", archive: "오퍼 백과사전", enemy: "적 도감", recruit: "공채", farm: "파밍",
  upgrade: "육성", story: "스토리", rogue: "통합전략", site: "사이트",
};

export const CHANGE_AREAS: ChangeArea[] = [
  "infra", "archive", "enemy", "recruit", "farm", "upgrade", "story", "rogue", "site",
];

/** 바로가기 경로 → 영역 (area가 비어 있는 옛 행의 폴백) */
const AREA_BY_PATH: [string, ChangeArea][] = [
  ["/infra", "infra"], ["/operators", "archive"], ["/enemies", "enemy"], ["/recruit", "recruit"],
...
//    묶음을 넣을 때는 seq 0부터 시작해 **최신 → 오래된 순**으로 매기고, 그 날짜에 이미
//    항목이 있으면 기존 것들의 seq를 밀어 올리거나 새 항목을 전부 seq 0으로 넣는다
//    (같은 seq는 created_at 최신순으로 갈리므로 0으로 넣어도 새 것이 위에 온다).
