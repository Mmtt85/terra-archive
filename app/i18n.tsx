...
const D: Record<string, Pair> = {
  ...
  "훈련실 정원 2는 트레이너석 1 + 훈련대상석 1이라, 플래너가 쓰는 근무석은 1석입니다. 다만 훈련 속도는 자원 생산과 단위가 달라 총점 계산에서는 빼둡니다 — 그래서 생산 방(제조소·무역소·발전소·사무실·응접실·제어 센터)을 먼저 다 채운 뒤 남는 인원 중 훈련 속도가 가장 높은 오퍼로 훈련실을 채웁니다. 자원 생산은 한 톨도 양보하지 않고, 훈련실은 교대 시계에 들어가는 방이 아니라 지속 시간도 줄지 않습니다. 다른 오퍼의 조건이 훈련실 배치로만 충족되는 경우(외드레르의 W가 앉을 작업 시설이 훈련실밖에 안 남은 경우)에는 그쪽이 우선입니다.": ["The Training Room's two slots are one trainer seat and one trainee seat, so the planner treats it as a single work seat, and it is filled rather than kept empty. Training speed isn't measured in the same unit as resource output, so it's excluded from the plan score — the production rooms (factories, trading posts, power plants, office, reception room, control center) are filled first, and the Training Room then takes whoever is left with the highest training speed. Not a single unit of resource output is given up, and since the Training Room isn't part of the shift clock, uptime doesn't drop either. When another operator's condition can only be met there (Hoederer's W with no other Work Facility left), that takes priority.", "訓練室の定員2は「トレーナー席1＋訓練対象席1」で、プランナーが使う勤務席は1席です。空けずに配置します。ただし訓練速度は資源生産と単位が異なるため総合スコアからは除外しています — 生産施設（製造所・貿易所・発電所・事務所・応接室・制御中枢）を先にすべて埋めたうえで、残ったオペレーターのうち訓練速度が最も高い者で訓練室を埋めます。資源生産は一切犠牲にせず、訓練室は交代時計の対象施設ではないため稼働時間も減りません。他オペレーターの条件が訓練室配置でしか満たせない場合（ヘドリーのWが座れる作業施設が訓練室しか残っていない等）はそちらが優先です。"],
