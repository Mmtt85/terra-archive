...
const D: Record<string, Pair> = {
  ...
  "한 번 누르면 이후엔 캡처만 반복하면 됩니다 — 에뮬레이터 목록을 클립보드로 캡처(맥 ⌃⌘⇧4 · 윈도우 Win+Shift+S)하고 이 탭으로 돌아오면 자동으로 인식돼 아래에 쌓입니다. 브라우저가 클립보드 접근을 물으면 '허용'을 눌러주세요.": ["Press once, then just keep capturing — capture the emulator list to the clipboard (Mac ⌃⌘⇧4 · Windows Win+Shift+S), return to this tab, and it's recognized automatically and stacks below. Click 'Allow' if the browser asks for clipboard access.", "一度押せば、あとはキャプチャを繰り返すだけ — エミュレーターの一覧をクリップボードにキャプチャ（Mac ⌃⌘⇧4・Windows Win+Shift+S）してこのタブに戻ると自動認識され、下に蓄積されます。ブラウザがクリップボードへのアクセスを尋ねたら「許可」を押してください。"],
