...
const D: Record<string, Pair> = {
  ...
  "훈련실 정원 2는 트레이너석 1 + 훈련대상석 1이며, 인프라 스킬 효과는 트레이너 1명만 반영됩니다 — 그래서 플래너가 쓰는 훈련실 근무석은 1석뿐입니다. 그 한 석도 실제 스킬 특화 훈련에 쓰도록 기본적으로 비워 둡니다. 다만 비움이 절대 규칙은 아닙니다 — 훈련실도 '작업 시설'이라, 다른 오퍼의 조건이 훈련실 배치로만 충족될 때(외드레르의 W가 앉을 작업 시설이 훈련실밖에 안 남은 경우)는 그쪽 이득이 실제로 더 클 때만 배치합니다.": ["The Training Room's two slots are one trainer seat and one trainee seat, and only the trainer's base skill takes effect — so the planner treats it as a single work seat. Even that seat is normally kept empty for actual mastery training, though emptiness is not an absolute rule: the room also counts as a Work Facility, so when another operator's condition can only be met there (Hoederer's W with no other Work Facility left), someone is stationed there — and only when the gain elsewhere genuinely outweighs it.", "訓練室の定員2は「トレーナー席1＋訓練対象席1」で、基地スキルの効果はトレーナー1名のみ反映されます — そのためプランナーが使う勤務席は1席だけです。その1席も実際のスキル特化訓練用に基本的に空けておきますが、空けること自体は絶対的な規則ではありません。訓練室も「作業施設」なので、他オペレーターの条件が訓練室配置でしか満たせない場合（ヘドリーのWが座れる作業施設が訓練室しか残っていない等）には、他所の利益が実際に上回るときだけ配置します。"],
