...
const D: Record<string, Pair> = {
  ...
  "아직 완성 안 한(정예화를 낮춰 둔) 오퍼를 완성했다고 가정해 자동편성을 다시 돌리고, 방 %효율이 실제로 얼마나 오르는지로 이득을 증명합니다. 숫자는 그 방 %효율 변화의 합계(%p)이며, A조(주력)를 우선해 정렬합니다. '적용'은 그 오퍼를 완성 처리하고 편성에 반영합니다.": ["Assuming each not-yet-completed operator (elite lowered) is finished, the auto-planner is re-run and the benefit is proven by how much room %-efficiency actually rises. The number is the sum of those room %-efficiency changes (%p), sorted by A-team (main crew) first. 'Apply' marks the operator complete and reflects it in the plan.", "まだ完成していない（昇進を下げた）オペレーターを完成させたと仮定して自動編成を回し直し、部屋の%効率が実際にどれだけ上がるかで利得を証明します。数値はその部屋の%効率変化の合計（%p）で、A班（主力）を優先して並べます。「適用」はそのオペレーターを完成扱いにして編成に反映します。"],
  ...
  "적용": ["Apply", "適用"],
  "이 오퍼를 완성 처리하고 자동편성에 반영합니다": ["Marks this operator complete and reflects it in the auto-plan", "このオペレーターを完成扱いにして自動編成に反映します"],
  "전체 적용": ["Apply all", "全て適用"],
  "추천 오퍼 전부를 완성으로 반영하고 자동편성을 다시 돌립니다": ["Marks all recommended operators complete and re-runs the auto-plan", "おすすめオペレーター全てを完成として反映し、自動編成を回し直します"],
