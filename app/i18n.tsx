...
const D: Record<string, Pair> = {
  ...
  // ── 에뮬레이터 화면 연동 (화면 캡처 — 오퍼 인식 스캐너) ──
  "에뮬레이터 연동": ["Link emulator", "エミュレーター連携"],
  "에뮬레이터 화면 연동": ["Link emulator screen", "エミュレーター画面連携"],
  "에뮬레이터(또는 명일방주) 창을 캡처해 보유 오퍼를 인식합니다 — 화면은 이 브라우저 안에서만 처리됩니다": ["Capture the emulator (or Arknights) window to recognize your operators — the screen is processed only inside this browser", "エミュレーター（またはアークナイツ）ウィンドウをキャプチャして所持オペレーターを認識します — 画面はこのブラウザ内でのみ処理されます"],
  "이 브라우저는 화면 캡처를 지원하지 않습니다.": ["This browser does not support screen capture.", "このブラウザは画面キャプチャに対応していません。"],
  "화면 선택이 취소되었습니다.": ["Screen selection was canceled.", "画面の選択がキャンセルされました。"],
  "화면 캡처를 시작하지 못했습니다: {msg}": ["Failed to start screen capture: {msg}", "画面キャプチャを開始できませんでした: {msg}"],
  "다시 선택": ["Reselect", "選び直す"],
  "연동 해제": ["Disconnect", "連携解除"],
  "화면 선택창에서 에뮬레이터(또는 명일방주) 창을 고르세요.": ["Choose the emulator (or Arknights) window in the screen picker.", "画面選択ウィンドウでエミュレーター（またはアークナイツ）を選んでください。"],
  "검은 화면만 잡힙니다 — 게임 화면 보호(DRM)로 캡처가 막혔을 수 있어요. 에뮬레이터 창을 고르거나 '화면 전체'를 선택해 보세요.": ["Only a black screen is captured — the game's screen protection (DRM) may be blocking capture. Try choosing an emulator window or 'Entire screen'.", "黒い画面しかキャプチャされません — ゲームの画面保護(DRM)でキャプチャがブロックされている可能性があります。エミュレーターのウィンドウか「画面全体」を選んでみてください。"],
  "연동됨 · {w}×{h}": ["Linked · {w}×{h}", "連携済み · {w}×{h}"],
  "연동됨 · 프레임 대기 중…": ["Linked · waiting for frames…", "連携済み · フレーム待機中…"],
  "연동 대기 중": ["Waiting for link", "連携待機中"],
  "이 영상은 이 브라우저 안에서만 처리되며 어디에도 전송되지 않습니다. 다음 단계에서 이 화면으로 보유 오퍼를 자동 인식합니다.": ["This video is processed only inside this browser and is never sent anywhere. In the next step, your operators will be recognized automatically from this screen.", "この映像はこのブラウザ内でのみ処理され、どこにも送信されません。次のステップでこの画面から所持オペレーターを自動認識します。"],
  "이미지로 테스트": ["Test with image", "画像でテスト"],
  "현재 화면 정지": ["Freeze current frame", "現在の画面を静止"],
  "라이브로": ["Back to live", "ライブに戻る"],
  "열": ["Cols", "列"],
  "행": ["Rows", "行"],
  "초상화 상단": ["Portrait top", "立ち絵 上端"],
  "초상화 하단": ["Portrait bottom", "立ち絵 下端"],
  "인식": ["Recognize", "認識"],
  "다시 인식": ["Recognize again", "再認識"],
  "인식 중…": ["Recognizing…", "認識中…"],
  "화면 전체에서 오퍼 이름을 읽는 중…": ["Reading operator names from the whole screen…", "画面全体からオペレーター名を読み取り中…"],
  "연동됨 · {w}×{h} · '인식'을 누르세요": ["Linked · {w}×{h} · press 'Recognize'", "連携済み · {w}×{h} ·「認識」を押してください"],
  "인식 중… {i}/{n}": ["Recognizing… {i}/{n}", "認識中… {i}/{n}"],
  "이름 상단": ["Name top", "名前 上端"],
  "이름 하단": ["Name bottom", "名前 下端"],
  "OCR 엔진 준비 중… {s}": ["Loading OCR engine… {s}", "OCRエンジン準備中… {s}"],
  "이름 밴드를 카드 이름 위치에 맞추고 '인식'을 누르세요. 격자는 드래그로 다시 그릴 수 있어요.": ["Fit the name band to the card name position and press 'Recognize'. Drag to redraw the grid.", "名前バンドをカードの名前位置に合わせて「認識」を押してください。グリッドはドラッグで描き直せます。"],
  "{n}명 인식됨 (초록=정확, 노랑/빨강=애매)": ["{n} recognized (green=confident, yellow/red=uncertain)", "{n}名認識 (緑=確実, 黄/赤=あいまい)"],
  "인식 영역을 오퍼 카드 격자에 맞추고 '인식'을 누르세요. 드래그로 영역을 다시 그릴 수 있어요.": ["Fit the recognition area to the operator card grid and press 'Recognize'. Drag to redraw the area.", "認識エリアをオペレーターカードのグリッドに合わせて「認識」を押してください。ドラッグでエリアを描き直せます。"],
  "이 영상은 이 브라우저 안에서만 처리되며 어디에도 전송되지 않습니다. 다음 단계에서 스크롤하며 자동으로 누적 인식합니다.": ["This video is processed only inside this browser and is never sent anywhere. In the next step, scrolling will accumulate recognitions automatically.", "この映像はこのブラウザ内でのみ処理され、どこにも送信されません。次のステップではスクロールしながら自動的に累積認識します。"],
  "결과 JSON 저장": ["Save result JSON", "結果JSONを保存"],
  "학습 {n}명": ["Learned {n}", "学習 {n}名"],
  "이미지로 식별 {n}건": ["{n} identified by image", "画像識別 {n}件"],
