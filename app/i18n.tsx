...
const D: Record<string, Pair> = {
  ...
  "사용법": ["How to use", "使い方"],
  "📷 버튼을 눌러 자동인식을 켭니다 (다시 누르면 꺼지고, 설정은 브라우저에 저장됩니다).": ["Press the 📷 button to turn auto-recognition on (press again to turn off; the setting is saved in your browser).", "📷ボタンを押して自動認識をオンにします（もう一度押すとオフ。設定はブラウザに保存されます）。"],
  "게임 화면을 클립보드로 캡처합니다 — 맥은 ⌃⌘⇧4, 윈도우는 Win+Shift+S. 화면 일부만 잘라 찍어도 됩니다.": ["Capture the game screen to the clipboard — ⌃⌘⇧4 on Mac, Win+Shift+S on Windows. Cropping just part of the screen works too.", "ゲーム画面をクリップボードにキャプチャします — Macは⌃⌘⇧4、WindowsはWin+Shift+S。画面の一部だけを切り取ってもOKです。"],
  "이 탭으로 돌아오면 자동으로 인식됩니다. 처음 한 번은 브라우저가 클립보드 접근을 물어보니 '허용'을 눌러주세요.": ["Return to this tab and it's recognized automatically. The first time, the browser will ask for clipboard access — click 'Allow'.", "このタブに戻ると自動で認識されます。初回はブラウザがクリップボードへのアクセスを尋ねるので「許可」を押してください。"],
  "인식되면 해당 정보로 바로 이동합니다 — 인식 중에는 상단 알림에 진행 상태와 이미지가 표시됩니다.": ["Once recognized, you jump straight to that info — while recognizing, the top toast shows progress and the image.", "認識されると該当情報へ直接移動します — 認識中は上部の通知に進行状況と画像が表示されます。"],
