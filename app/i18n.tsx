...
const D: Record<string, Pair> = {
  ...
  "현재 보유·정예화 상태로 자동편성을 돌린 결과와, 후보만 완성했을 때의 기지 총점 차이로 실제 이득을 증명합니다 — 최고 효율 상위 20개. 방 %효율 변화와 완성 비용은 실제 편성·게임 데이터 기준입니다.": ["The benefit is proven by the difference in total base score between the current plan and the plan with only the candidate completed — top 20 by efficiency. Room %-efficiency changes and completion cost are based on the real plan and game data.", "現在の所持・昇進状態で自動編成した結果と、候補だけを完成させたときの基地総合スコア差で実際の利得を証明します — 効率上位20件。部屋の%効率変化と完成コストは実際の編成・ゲームデータ基準です。"],
  "시너지": ["Synergy", "シナジー"],
