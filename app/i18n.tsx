...
const D: Record<string, Pair> = {
  ...
  "기지 치비": ["Base Chibi", "基地ちびキャラ"],
  "아직 준비된 치비 렌더가 없습니다 — 최신 오퍼레이터는 소스 레포에 렌더가 올라오면 추가됩니다.": ["No chibi render available yet — newer operators are added once the source repo renders them.", "まだ用意されたちびレンダーがありません — 最新オペレーターはソースレポにレンダーが上がり次第追加されます。"],
  "이 브라우저는 투명 WebM(VP9) 재생을 지원하지 않아 치비를 보여줄 수 없습니다.": ["This browser can't play transparent WebM (VP9), so the chibi can't be shown.", "このブラウザは透過WebM（VP9）の再生に対応していないため、ちびを表示できません。"],
  "{name} 기지 치비 애니메이션": ["{name} base chibi animation", "{name} 基地ちびアニメーション"],
  "기지 대기 모션 · 렌더 출처 ArknightsAssets/ArknightsSpines — 게임 내 연출과 다를 수 있습니다.": ["Base idle motion · renders from ArknightsAssets/ArknightsSpines — may differ from in-game presentation.", "基地待機モーション · レンダー出典 ArknightsAssets/ArknightsSpines — ゲーム内演出と異なる場合があります。"],
