...
const D: Record<string, Pair> = {
  ...
  "라운드마다 전장이 정해져 있습니다. 1~13라운드는 난이도와 상관없이 같고, 리더 라운드만 어느 리더가 나오느냐에 따라 갈립니다. 배치 칸과 적이 오는 길을 보고 편성을 맞춰 보세요.": [
    "Each round has a fixed battlefield. Rounds 1-13 are the same on every difficulty; only the leader round changes with which leader shows up. Check the deployable tiles and the paths enemies take, then plan your line-up around them.",
    "ラウンドごとに戦場が決まっています。1〜13ラウンドは難易度に関係なく同じで、リーダーラウンドだけどのリーダーが出るかで変わります。配置マスと敵の進路を見て編成を合わせてみてください。"],
  "전투 맵을 불러오는 중…": ["Loading battle maps…", "戦闘マップを読み込み中…"],
  "고정 라운드": ["Fixed rounds", "固定ラウンド"],
  "모든 난이도 공통": ["same on every difficulty", "全難易度共通"],
