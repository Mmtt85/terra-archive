...
const D: Record<string, Pair> = {
  ...
  "무역소 2 · 제조소 5(순금 2+작전기록 3) · 발전소 2 — 전력이 모자라 숙소·훈련실·응접실·사무실 Lv1 권장": ["Trading Posts ×2 · Factories ×5 (Gold 2 + Records 3) · Power Plants ×2 — short on power, so Lv1 dorms/training/reception/office recommended", "貿易所2 · 製造所5（純金2+作戦記録3）· 発電所2 — 電力不足のため宿舎・訓練室・応接室・事務室Lv1推奨"],
  ...
  "전력·시설 레벨: 발전소가 전력을 공급하고(레벨 3 기준 1기 270, 3기 810) 나머지 시설이 소비합니다. 방을 눌러 시설 레벨을 바꾸면 전력 소비·근무 슬롯(제조소·무역소 Lv1/2/3 = 1/2/3인)·레벨 연동 스킬(숙소 레벨 합·응접실·훈련실 레벨 등)이 함께 재계산됩니다. 243·153은 전부 만렙일 때 소비가 정확히 810이라 여유가 0이고, 252는 발전소가 2기(540)뿐이라 숙소·훈련실·응접실·사무실을 Lv1로 낮춰야 성립합니다(기본값으로 적용됨). 상단 ⚡ 전력이 음수면 게임에서 지을 수 없는 구성입니다.": ["Power & facility levels: power plants supply electricity (270 each at Lv3, 810 for three) and every other facility consumes it. Click a room to change its level — power draw, work slots (factory/trading Lv1/2/3 = 1/2/3 seats), and level-scaled skills (dorm level sum, reception/training level, etc.) all recompute. At full levels 243/153 consume exactly 810 with zero headroom; 252 has only two plants (540), so dorms, training room, reception and office must drop to Lv1 (applied as its default). If the power figure is negative, the base cannot exist in game.", "電力・施設レベル：発電所が電力を供給し（Lv3で1基270、3基810）、他の施設が消費します。部屋をクリックしてレベルを変えると、電力消費・勤務スロット（製造所・貿易所 Lv1/2/3 = 1/2/3人）・レベル連動スキル（宿舎レベル合計・応接室・訓練室レベルなど）が再計算されます。全施設最大時の消費は243・153でちょうど810（余裕0）、252は発電所2基（540）のみのため宿舎・訓練室・応接室・事務室をLv1に下げる必要があります（デフォルトで適用）。上部の電力表示がマイナスならゲーム内で成立しない構成です。"],
  ...
  "레벨별 효과": ["Effects by level", "レベル別効果"],
  "{n}인": ["{n} seats", "{n}人"],
