...
const D: Record<string, Pair> = {
  ...
  "프레임 저장": ["Save frame", "フレーム保存"],
  "현재 캡처 프레임을 PNG로 저장 — 오인식 제보에 첨부하면 재현·수정에 쓰입니다": ["Save the current captured frame as PNG — attach it to a misrecognition report so it can be reproduced and fixed", "現在のキャプチャフレームをPNG保存 — 誤認識の報告に添付すると再現・修正に使えます"],
  "캡처가 흐릿해 인식률이 낮습니다 — 화면 공유를 껐다 다시 시작하거나, 스크린샷 파일 인식을 사용해 보세요": ["Capture looks blurry so recognition is degraded — restart the screen share, or use screenshot-file recognition instead", "キャプチャがぼやけて認識率が低下しています — 画面共有を再開するか、スクリーンショットファイル認識をお試しください"],
  "화면 공유가 흐릿하게 잡히는 환경에서는 오퍼 목록 스크린샷 파일(PNG 여러 장)을 인식하는 쪽이 정확합니다.": ["If screen sharing captures blurry frames on your setup, recognizing operator-list screenshot files (multiple PNGs) is more accurate.", "画面共有がぼやけて取り込まれる環境では、オペレーター一覧のスクリーンショットファイル（PNG複数枚）を認識する方が正確です。"],
  "스크린샷 파일로 인식": ["Recognize from screenshot files", "スクリーンショットファイルで認識"],
  ...
  "에뮬레이터 스크린샷(⌘⇧4+스페이스로 창 캡처 등)을 계속 추가하면 오퍼가 누적됩니다.": ["Keep adding emulator screenshots (e.g. ⌘⇧4+Space window capture) and operators accumulate.", "エミュレーターのスクリーンショット（⌘⇧4+スペースのウィンドウキャプチャなど）を追加し続けるとオペレーターが蓄積されます。"],
