...
const D: Record<string, Pair> = {
  ...
  "훈련실은 두 자리 모두 인프라 스킬·시너지가 적용됩니다 (실제 기지 검증 — 한때 트레이너 1석만 쓴다고 봤던 것을 정정했습니다). 다만 두 자리 다 실제 스킬 특화 훈련에 쓰도록 기본적으로는 비워 둡니다. 비움이 절대 규칙은 아닙니다 — 훈련실도 '작업 시설'이라, 다른 오퍼의 조건이 훈련실 배치로만 충족될 때(외드레르의 W가 앉을 작업 시설이 훈련실밖에 안 남은 경우)는 그쪽 이득이 실제로 더 클 때만 배치합니다.": ["Both Training Room seats apply base skills and synergies (verified in an actual base — we previously treated only the trainer seat as usable and have corrected that). Both are still normally kept empty for actual mastery training, though emptiness is not an absolute rule: the room also counts as a Work Facility, so when another operator's condition can only be met there (Hoederer's W with no other Work Facility left), someone is stationed there — and only when the gain elsewhere genuinely outweighs it.", "訓練室は2席とも基地スキル・シナジーが適用されます（実際の基地で検証 — 以前はトレーナー席1つだけとみなしていたものを訂正しました）。ただし2席とも実際のスキル特化訓練用に基本的には空けておきます。空けること自体は絶対的な規則ではありません — 訓練室も「作業施設」なので、他オペレーターの条件が訓練室配置でしか満たせない場合（ヘドリーのWが座れる作業施設が訓練室しか残っていない等）には、他所の利益が実際に上回るときだけ配置します。"],
