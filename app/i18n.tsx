...
const D: Record<string, Pair> = {
  ...
  "이성": ["sanity", "理性"],
  "회수 {n}일": ["{n}d payback", "回収 {n}日"],
  "회수 환산 제외": ["Payback n/a", "回収換算対象外"],
  "이 오퍼가 올리는 방(발전소·사무실·응접실)은 산출을 이성으로 환산할 근거가 없어 회수일을 내지 않습니다": ["The rooms this operator improves (power plant, HR office, reception) have no basis for converting output into sanity, so no payback is shown", "このオペレーターが上げる部屋（発電所・人事室・応接室）は産出を理性に換算する根拠がないため、回収日数は表示しません"],
  "완성 비용 {cost} 이성 ÷ 하루 이득 {daily} 이성 = {days}일. 환산 기준은 목록 아래 각주 참고": ["Completion cost {cost} sanity ÷ daily gain {daily} sanity = {days} days. See the footnote below the list for the conversion basis", "完成コスト{cost}理性 ÷ 1日あたりの利益{daily}理性 = {days}日。換算基準はリスト下の注釈を参照"],
