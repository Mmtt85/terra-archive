...
const D: Record<string, Pair> = {
  ...
  "공개모집 태그 조합으로 확정·고성급 탐색": ["Guaranteed picks from your tag combos", "公開求人タグの組み合わせから確定・高レアを探索"],
  ...
  "생존연산": ["Reclamation Algorithm", "生息演算"],
  ...
  // 이벤트 상세 모달의 공식 카페 버튼 (2026-09-23) — 공지를 못 찾은 이벤트는 카페 이벤트 게시판으로
  "공식 카페": ["Official Cafe", "公式カフェ"],
  "공식 카페 이벤트 게시판 보기": ["Open the official Cafe event board", "公式カフェのイベント掲示板を開く"],
  // 듀얼 채널 상세 — 이벤트 도감 모달의 듀얼 채널 탭 (app/event-duel.tsx, 2026-09-23). 용어는 일·글섭 게임 문구에 맞춘다(様子見·ALL·失敗保障 / Skip·All-in·Spectator Shield)
  "듀얼 채널": ["Duel Channel", "デュエルチャンネル"],
  "듀얼 채널 상세": ["Duel Channel details", "デュエルチャンネル詳細"],
  "이 서버엔 아직 없는 회차라 게임 문구를 한국어판으로 보여 줍니다.": ["This season hasn't reached the Global server yet, so in-game text is shown from the Korean version.", "この回はまだ日本サーバーにないため、ゲーム内の文言を韓国版で表示しています。"],
  "개요": ["Overview", "概要"],
  "선수 {n}": ["Roster {n}", "選手 {n}"],
  "라운드": ["Rounds", "ラウンド"],
  "게임 안내": ["How to play", "遊び方"],
  "관객·기타": ["Viewers & more", "観客・その他"],
  "진행 단계": ["Phases", "進行段階"],
  "지금": ["Now", "現在"],
  "보상 수령은 {at}까지 (한국 시간)": ["Rewards can be claimed until {at} (KST)", "報酬受取は{at}まで（韓国時間）"],
  "관전 모드": ["Viewing modes", "観戦モード"],
  "{n}라운드": ["{n} rounds", "{n}ラウンド"],
  "최대 {n}라운드": ["Up to {n} rounds", "最大{n}ラウンド"],
  "시작 선물 {n}": ["Starting gifts {n}", "初期ギフト{n}"],
  "선택 {n}초": ["{n}s to pick", "選択{n}秒"],
  "마지막 {n}초 응원 비공개": ["others' picks hidden in the last {n}s", "残り{n}秒は他の選択が非表示"],
  "관객 보호 {n}라운드까지": ["Spectator Shield until round {n}", "失敗保障 {n}ラウンドまで"],
  "NPC 관객과 {n}명": ["{n} seats with NPC viewers", "NPC観客と{n}人"],
  "매칭": ["Matchmake", "マッチング"],
  "단체방": ["Group Watch", "グループ"],
  "게임 속 중계 채널": ["In-game broadcast channel", "ゲーム内の配信チャンネル"],
  "경기장": ["Arena", "会場"],
  "보상 프로그램": ["Reward program", "報酬プラン"],
  "{item} 누적 수량에 따라 단계별 보상": ["Rewards by total {item} collected", "{item}の累計で段階報酬"],
  "{n}단계 합계": ["Total of {n} levels", "全{n}段階の合計"],
  "프로필 아바타": ["Profile avatar", "プロフィールアイコン"],
  "분위기 +{n}": ["Ambience +{n}", "快適度+{n}"],
  "{at}부터": ["From {at}", "{at}から"],
  "Lv.{lv} 이후 단계는 {at}에 열립니다 (한국 시간)": ["Levels from Lv.{lv} unlock at {at} (KST)", "Lv.{lv}以降は{at}に解放（韓国時間）"],
  "순위 추가 보상": ["Rank bonus", "順位ボーナス"],
  "매칭 한 판의 최종 순위에 따라": ["By final rank in one matchmade game", "マッチング1試合の最終順位に応じて"],
  "{n}위": ["#{n}", "{n}位"],
  "{a}~{b}위": ["#{a}–{b}", "{a}～{b}位"],
  "일일 활약": ["Daily Activity", "デイリー"],
  "메달 {n}": ["Medals {n}", "勲章 {n}"],
  "라운드마다 이 명단에서 양쪽 팀을 뽑습니다. 큰 글씨가 듀얼 채널 전용 이름, 작은 글씨가 원본 적입니다. 강조된 수치는 원본과 다른 값입니다.": ["Each round, both teams are drawn from this roster. The large name is the Duel Channel name, the small one the original enemy. Highlighted values differ from the original.", "ラウンドごとにこの名簿から両チームが選ばれます。大きい名前がデュエルチャンネルでの名前、小さい名前が元の敵です。強調された数値は元と異なる値です。"],
  "앞 회차에 없던 선수": ["Not in earlier seasons", "前回までにいなかった選手"],
  "{pool} 풀": ["{pool} pool", "{pool}枠"],
  "게임 순서": ["Game order", "ゲーム内の順"],
  "여러 개체가 한 팀으로 나온다": ["Appears as a group of several units", "複数の個体で1チーム"],
  "다수": ["Group", "複数"],
  "모든 풀의 가중치가 0 — 지금은 대진에 뽑히지 않습니다": ["Weight 0 in every pool — not drawn for now", "全枠の重みが0 — 現在は選ばれません"],
  "대기": ["Standby", "待機"],
  "풀에는 올라 있지만 게임 데이터에 아직 없는 선수 {n}명은 뺐습니다.": ["{n} fighters listed in a pool but not yet in the game data are left out.", "枠には載っているがゲームデータにまだない選手{n}人は除いています。"],
  "거대 보스": ["Giant boss", "巨大ボス"],
  "거대 보스 상대": ["Vs. giant boss", "巨大ボスの相手"],
  "기습 없음": ["No surprise", "奇襲なし"],
  "소형": ["Small", "小型"],
  "음악": ["Music", "音楽"],
  "양쪽 팀은 각 칸의 풀에서 그 수만큼 뽑힙니다. 풀에 누가 있는지는 '선수' 탭의 풀 필터로 볼 수 있습니다.": ["Each team is drawn from the pool shown, in that number. See who is in each pool with the pool filter on the Roster tab.", "両チームは各欄の枠から、その人数だけ選ばれます。枠の中身は「選手」タブの枠フィルターで見られます。"],
  "왼쪽 팀": ["Left team", "左チーム"],
  "오른쪽 팀": ["Right team", "右チーム"],
  "선택 시간": ["Pick time", "選択時間"],
  "관망": ["Skip", "様子見"],
  "올인": ["All-in", "ALL"],
  "최대 {n}라운드 — 게임 데이터에는 {k}라운드까지만 따로 정의돼 있습니다.": ["Up to {n} rounds — the game data defines only the first {k} individually.", "最大{n}ラウンド — ゲームデータで個別に定義されているのは{k}ラウンドまでです。"],
  "인게임 안내": ["In-game guide", "ゲーム内ガイド"],
  "누르면 크게 볼 수 있습니다": ["Click to enlarge", "クリックで拡大"],
  "이전": ["Previous", "前へ"],
  "다음": ["Next", "次へ"],
  "안내 {n}": ["Guide {n}", "案内 {n}"],
  "팁 {n}": ["Tips {n}", "ヒント {n}"],
  "공통": ["All modes", "共通"],
  "관객 NPC {n}": ["NPC viewers {n}", "NPC観客 {n}"],
  "혼자 즐기기에서 함께 예측하는 가상 관객": ["Virtual viewers who predict alongside you in solo play", "一人観戦で一緒に予想する仮想観客"],
  "항상 왼쪽": ["Always left", "常に左"],
  "다수를 따라감": ["Follows the majority", "多数派に乗る"],
  "소수를 따라감": ["Follows the minority", "少数派に乗る"],
  "적이 홀수인 쪽": ["Side with an odd number of enemies", "敵が奇数の側"],
  "적이 짝수인 쪽": ["Side with an even number of enemies", "敵が偶数の側"],
  "승리 예상 쪽": ["Picks the expected winner", "勝ちそうな方"],
  "정산 코멘트": ["Result comments", "リザルトコメント"],
  "판이 끝나면 결과에 따라 붙는 한마디": ["A one-liner shown after each game based on the result", "試合後、結果に応じて付くひとこと"],
  "듀얼 채널 전용 개체의 수치입니다 (원본: {name}).": ["Stats of the Duel Channel variant (original: {name}).", "デュエルチャンネル専用個体の数値です（元：{name}）。"],
  "강조된 수치는 원본 적과 다른 값입니다.": ["Highlighted values differ from the original enemy.", "強調された数値は元の敵と異なる値です。"],
  "원본 적이 도감에 없어 듀얼 채널 수치만 보여 줍니다.": ["The original enemy isn't in the dex, so only the Duel Channel stats are shown.", "元の敵が図鑑にないため、デュエルチャンネルの数値のみ表示します。"],
  "능력·면역·설명은 원본 적 기준입니다.": ["Abilities, immunities and description are those of the original enemy.", "能力・耐性・説明は元の敵のものです。"],
  ...
  "발전소 공급 − 시설 소비. 음수면 게임에서 못 짓는 구성": [
    "Power plant supply − facility draw. Negative means the layout cannot be built in game.",
    "発電所の供給 − 施設の消費。マイナスならゲーム内では建てられない構成です。"],
  "발전소 공급 {p} − 시설 소비 {c} = 여유 {n}. 음수면 게임에서 지을 수 없는 구성이니 방을 눌러 시설 레벨을 낮추세요.": [
    "Power plant supply {p} − facility draw {c} = {n} to spare. If it is negative the layout cannot exist in game — click a room to lower its level.",
    "発電所の供給{p} − 施設の消費{c} = 余裕{n}。マイナスならゲーム内では成立しない構成なので、部屋をクリックして施設レベルを下げてください。"],
  "하루 평균 용문폐는 이렇게 계산했습니다": ["How the daily LMD average is calculated", "1日平均の龍門幣の計算方法"],
  "하루 평균 작전기록은 이렇게 계산했습니다": ["How the daily records average is calculated", "1日平均の作戦記録の計算方法"],
  ...
  "빈 자리에 추가 — 클릭 시 즉시 배치·📌 고정 (기여 예상):": ["Add to an empty slot — click to place and 📌 pin (estimated gain):", "空きスロットに追加 — クリックで即配置・📌固定（貢献予想）："],
  "이 방에 고정 — 자동편성이 A조·B조 모두 이 자리에 유지합니다 (교대 휴식 없음, 컨디션 관리는 직접)": ["Pin to this room — auto-planning keeps them here on both shift A and B (no shift rest; you manage their morale yourself)", "この部屋に固定 — 自動編成はA班・B班の両方でこの枠に残します（交代休憩なし。体力管理は自分で行います）"],
  "방 고정 해제 — 다음 자동편성이 이 자리를 다시 짤 수 있습니다": ["Unpin — the next auto-plan may reassign this slot", "固定解除 — 次の自動編成でこの枠は組み直される可能性があります"],
  "{name} — 방 고정 (A·B 양조 근무, 자동편성이 유지)": ["{name} — pinned to this room (works both shifts; kept by auto-planning)", "{name} — この部屋に固定（A・B両班勤務、自動編成でも維持）"],
  "**📌 고정된 오퍼는 자동편성이 이 방 A조·B조 모두에 그대로 앉힙니다** — 교대 휴식이 없어지므로 컨디션(지속시간) 관리는 피아메타 등으로 직접 해 주세요. 카드의 📌로 잠그거나 풀고, ✕로 빼면 고정도 풀립니다.": ["**📌 Pinned operators are seated by auto-planning on both shift A and B of this room** — they get no shift rest, so manage their morale (uptime) yourself, e.g. with Fiammetta. Use 📌 on a card to pin or unpin, and ✕ removes them, which also unpins.", "**📌固定したオペレーターは、自動編成がこの部屋のA班・B班の両方にそのまま配置します** — 交代休憩がなくなるため、体力（持続時間）の管理はフィアメッタなどで自分で行ってください。カードの📌で固定・解除、✕で外すと固定も一緒に解けます。"],
  "오퍼를 방에 고정했습니다": ["Operator pinned to this room", "オペレーターをこの部屋に固定しました"],
  "고정은 다음 전체 자동편성부터 편성 전체에 반영됩니다 — 지금 재편성할까요? 더 고정할 오퍼가 있으면 '나중에'를 누르고 이어서 고정하세요.": ["Pins take effect base-wide from the next full auto-plan — rebuild now? If you have more operators to pin, choose \"Later\" and keep pinning first.", "固定は次の全体自動編成から編成全体に反映されます — 今すぐ再編成しますか？ まだ固定したいオペレーターがいる場合は「あとで」を押して先に固定を続けてください。"],
  "지금 재편성": ["Rebuild now", "今すぐ再編成"],
  "나중에": ["Later", "あとで"],
  ...
  "조건에 맞는 이벤트가 없어요.": ["No events match those filters.", "条件に合うイベントがありません。"],
  "수록 내용": ["Contains", "収録内容"],
  "사이드 스토리": ["Side Story", "サイドストーリー"],
  "막간 이야기": ["Intermezzi", "幕間"],
  "기타": ["Other", "その他"],
  "교환 재화": ["Exchange currency", "交換素材"],
  "보상 오퍼": ["Reward operator", "報酬オペレーター"],
  "이벤트 오퍼레이터": ["Event operators", "イベントオペレーター"],
  "이벤트 오퍼": ["Event operator", "イベントオペ"],
  "맵에서 나오는 상위 재료": ["Top-tier materials from its maps", "マップで出る上位素材"],
  "상위 재료": ["Top-tier materials", "上位素材"],
  "상위 재료 {n}": ["{n} top-tier materials", "上位素材{n}種"],
  "미실장": ["Unreleased", "未実装"],
  "한국 서버 {ym} 예정": ["Expected on KR around {ym}", "韓国サーバー{ym}予定"],
  "보상": ["Reward", "報酬"],
  "신규": ["New", "新規"],
  ...
  "적 {n}": ["{n} enemies", "敵{n}種"],
  "재화 {n}": ["{n} currencies", "素材{n}種"],
  // ── 아이템 도감 (/items, 2026-09-16) ────────────────────────────────────────
  "아이템 도감": ["Item Handbook", "アイテム図鑑"],
  "아이템 도감 - 명일방주 아이템 정보 | 테라 아카이브": [
    "Item Handbook - Arknights Item Database | Terra Archive",
  ...
  "교환권·모집권": ["Vouchers & tickets", "引換券・求人券"],
  "소장품·기타": ["Collectibles & misc", "コレクション・その他"],
  "작전 드랍": ["Stage drop", "作戦ドロップ"],
  "제조·가공": ["Factory / workshop", "製造・加工"],
  "효율표 있음": ["Has efficiency table", "効率表あり"],
  "제조소에서 생산": ["Produced in the factory", "製造所で生産"],
  ...
  "생산방(무역소·제조소·발전소 등)도 같은 방식으로 오퍼를 📌 고정할 수 있습니다 — 다만 근무 방 고정은 A조·B조 양쪽에 같은 자리로 들어가 교대 휴식이 없어지므로, 컨디션(지속시간) 관리는 피아메타 등으로 직접 해야 합니다 (프로바이조를 무역소에, 미즈키를 제조소에 상주시키는 운용). 고정 오퍼는 다른 방·숙소 후보에서 빠져 이중 배치되지 않습니다.": ["Work rooms (trading posts, factories, power plants, …) support 📌 pinning the same way — but pinning to a work room seats the operator in the same slot on both shift A and B, removing their shift rest, so you manage their morale (uptime) yourself, e.g. with Fiammetta (the \"keep Proviso in a trading post and Mizuki in a factory\" playstyle). Pinned operators are excluded from other rooms' and dormitory candidates, so they are never double-assigned.", "生産部屋（貿易所・製造所・発電所など）も同じ方法でオペレーターを📌固定できます — ただし勤務部屋への固定はA班・B班の両方に同じ枠で入るため交代休憩がなくなり、体力（持続時間）の管理はフィアメッタなどで自分で行う必要があります（プロヴァイゾを貿易所に、ミヅキを製造所に常駐させる運用）。固定オペレーターは他の部屋・宿舎の候補から外れるため二重配置にはなりません。"],
  ...
  "'하루 평균 작전기록'은 제조소 기본 생산 속도 1포인트/초 기준입니다 — 중급작전기록 10,800pt = 1,000exp(Lv3), 초급 4,800pt = 400exp(Lv2), 기초 2,700pt = 200exp(Lv1). 제조소 레벨이 낮으면 만들 수 있는 등급도 낮아지므로(252 배치의 Lv2 제조소) 그 레벨의 레시피로 계산합니다.": [
    ...
    "「1日平均の作戦記録」は製造所の基本生産速度1ポイント/秒が基準です — 中級作戦記録10,800pt = 1,000exp（Lv3）、初級4,800pt = 400exp（Lv2）、入門2,700pt = 200exp（Lv1）。製造所のレベルが低いと作れる等級も下がるため（252配置のLv2製造所）、そのレベルのレシピで計算します。",
  ],
  ...
  // 계층 목록 안내 (2026-08-16) — 종전 "고를수록 아래에 하위 조건이 생깁니다"를 대체.
  // ⚠ 조작이 기기마다 다르지만(마우스=올리면 열림, 터치=누르면 펼침) 문구는 **한 문장**으로
  //   둔다 — 이 줄은 첫 화면에 그려지므로 기기별로 갈라 쓰면 하이드레이션이 어긋난다.
  "항목을 눌러 값을 고르세요 · › 표시가 있는 항목은 하위로 좁힐 수 있습니다": ["Tap a field to choose a value · entries marked › can be narrowed further", "項目を押して値を選択 · ›が付いた項目はさらに絞り込めます"],
  // 하위 목록 맨 위 — 하위를 좁히지 않고 부모 값만 고른다 (터치에선 부모를 고르는 유일한 통로)
  "{name} 전체": ["All of {name}", "{name} すべて"],
  ...
  // 생존연산 가이드 /ra (사용자 확정 2026-08-12)
  "생존연산 가이드": ["Reclamation Algorithm Guide", "生息演算ガイド"],
  "생존연산 가이드 - 명일방주 생존연산 공략 | 테라 아카이브": [
    "Reclamation Algorithm Guide - Arknights RA Guide | Terra Archive",
    "生息演算ガイド - アークナイツ生息演算攻略 | テラアーカイブ"],
  "생존연산 상설 「사막 이야기」의 요리 조합, 제작·설치물 재료, 지역과 날씨, 조우 선택지, 균열 목표를 게임 데이터에서 그대로 정리했습니다.": [
    "Everything from the permanent Reclamation Algorithm “Tales Within the Sand”, straight from game data — food recipes, crafting materials, areas & weather, encounter choices and rift objectives.",
    "生息演算常設「砂中の遺聞」の料理レシピ、製作・設置物の素材、エリアと天候、遭遇の選択肢、裂け目の目標をゲームデータから整理しました。"],
  "중국 서버 선행 신시즌 「재기동 앵커」는 헤더의 '미래시 데이터 포함'을 켜면 보입니다.": [
    "The CN-first new season is shown when “Include future data” is enabled in the header.",
    "中国サーバー先行の新シーズンは、ヘッダーの「未実装データを含む」をオンにすると表示されます。"],
  "생존연산 보기": ["Reclamation Algorithm views", "生息演算の表示"],
  "무작위 지형": ["Random terrain", "ランダム地形"],
  "게임 데이터에 지형 이름이 따로 없어, 그 지형을 쓰는 시나리오 코드(없으면 지형 풀)를 이름 대신 싣고 내부 id를 작게 덧붙였습니다.": [
    "The game data gives these tiles no names, so we label them with the scenario code that uses them (or their terrain pool) and keep the internal id as a small note.",
    "ゲームデータに地形名がないため、その地形を使うシナリオコード（なければ地形プール）を名前代わりに表示し、内部idを小さく併記しています。"],
  // 생존연산 확장 (2026-08-12 — 전체 지도 구역·조우 묶음·신시즌 전투 지형/시나리오)
  "구역 경계를 지도 위에 표시했습니다 — 구역을 누르면 그 구역의 정보가 열립니다. 노드 {n}개는 매 판 이 구역들 안에 새로 뽑혀 배치되므로 게임 데이터에 고정 좌표가 없습니다 (좌표 대신 최소 간격만 들어 있습니다).": [
    "Zone borders are drawn over the map — tap a zone to see its details. The {n} nodes are re-rolled inside these zones every run, so the game data holds no fixed coordinates for them (only a minimum spacing).",
    "エリア境界を地図上に表示しています — エリアを押すとその情報が開きます。ノード{n}個は毎回このエリア内で抽選配置されるため、ゲームデータに固定座標がありません（座標の代わりに最小間隔のみ）。"],
  "지도에 배치되는 노드 종류입니다 — 어느 지역이 어느 노드에 놓이는지는 게임이 매번 새로 뽑기 때문에 데이터에 고정 매핑이 없습니다. 대신 아래에서 자원으로 지역을 걸러 보세요.": [
    "Node types placed on the map. Which area lands on which node is re-rolled every run, so there is no fixed mapping in the data — filter areas by resource below instead.",
    "地図に配置されるノード種別です。どのエリアがどのノードに置かれるかは毎回抽選されるため固定の対応はありません — 代わりに下の資源でエリアを絞ってください。"],
  "거점 강화": ["Outpost upgrades", "拠点強化"],
  "노드에 시설을 세우면 그 거점에 붙는 상시 효과입니다.": [
    "Permanent effects a node gains once you build facilities on it.",
    "ノードに施設を建てるとその拠点に付く常時効果です。"],
  "구역 고유 효과": ["Zone effect", "エリア固有効果"],
  "이 구역에는 고유 효과가 없습니다.": ["This zone has no special effect.", "このエリアに固有効果はありません。"],
  "이 구역 안에 놓이는 노드는 판마다 새로 뽑히므로 어떤 지역이 나올지는 고정돼 있지 않습니다.": [
    "Nodes inside this zone are re-rolled every run, so which areas appear is not fixed.",
    "このエリア内のノードは毎回抽選されるため、どのエリアが出るかは固定されていません。"],
  "박사 추천 지역": ["Doctor’s pick", "ドクターのおすすめ"],
  "이어지는 장면과 갈래는 한 조우로 묶었습니다 — 카드를 누르면 갈래를 골라 장면 순서대로 볼 수 있습니다.": [
    "Follow-up scenes and branches are grouped into one encounter — open a card to pick a branch and read the scenes in order.",
    "続く場面と分岐は一つの遭遇にまとめました — カードを押すと分岐を選び、場面を順番に読めます。"],
  "{n}갈래": ["{n} branches", "{n}分岐"],
  "장면 {n}": ["{n} scenes", "場面{n}"],
  "갈래": ["Branch", "分岐"],
  "갈래 {n}": ["Branch {n}", "分岐{n}"],
  "이어지는 장면 {n}": ["Follow-up {n}", "続く場面{n}"],
  "함께 쓰이는 재료": ["Used together with", "一緒に使う材料"],
  "누르면 지도에서 이 자원만 남깁니다": ["Tap to show only this resource on the map", "押すと地図でこの資源だけを表示します"],
  "산출물이 없는 레시피는 재료를 소비해 번영도를 올리는 레시피입니다.": [
    "Recipes with no output consume materials to raise prosperity instead.",
  ...
  "프리뷰 없음": ["No preview", "プレビューなし"],
  "이 지형은 프리뷰 이미지가 없습니다.": ["This terrain has no preview image.", "この地形にはプレビュー画像がありません。"],
  "이 지형은 경로 데이터가 없습니다.": ["This terrain has no route data.", "この地形には経路データがありません。"],
  "게임 데이터의 격자 좌표 그대로 그린 지도입니다. 노드를 누르면 그 시나리오 상세가 열립니다.": [
    "Drawn from the grid coordinates in the game data. Tap a node to open that scenario.",
    "ゲームデータのグリッド座標そのままで描いた地図です。ノードを押すとそのシナリオが開きます。"],
  "난이도 0~{n}": ["Difficulty 0–{n}", "難易度0〜{n}"],
  "목표 점수": ["Target score", "目標スコア"],
  "지형 배치": ["Terrain layout", "地形配置"],
  "이 시나리오를 이루는 전투 지형입니다 — 누르면 도면·경로 상세가 열립니다.": [
    "The battle terrain this scenario is made of — tap for blueprint and route details.",
    "このシナリオを構成する戦闘地形です — 押すと図面・経路の詳細が開きます。"],
  "지형 풀": ["Terrain pool", "地形プール"],
  "환경 압력": ["Environmental pressure", "環境圧力"],
  "최초 클리어 보상": ["First-clear reward", "初回クリア報酬"],
  "같은 이야기의 1·2단계와 보스 판은 한 카드로 묶었습니다 — 상세에서 골라 봅니다.": [
    "Stage 1, stage 2 and the boss version of the same story share one card — pick inside.",
    "同じ話の1・2段階とボス版は一枚のカードにまとめました — 詳細で選べます。"],
  "1단계": ["Stage 1", "1段階"],
  "2단계": ["Stage 2", "2段階"],
  "도면": ["Blueprint", "図面"],
  "요리·음료": ["Food & Drink", "料理・飲料"],
  "제작·설치물": ["Crafting & Structures", "製作・設置物"],
  "지역·날씨": ["Areas & Weather", "エリア・天候"],
  "균열·원정": ["Rifts & Expeditions", "裂け目・遠征"],
  "테크트리": ["Tech Tree", "テクノロジー"],
  "신시즌": ["New Season", "新シーズン"],
  "이름·재료 검색": ["Search name or material", "名前・素材で検索"],
  "생존": ["Survival", "生存"],
  "채집": ["Gathering", "採集"],
  "재배치": ["Redeploy", "再配置"],
  "코스트": ["Cost", "コスト"],
  ...
  "균열": ["Rift", "裂け目"],
  "상점": ["Shop", "ショップ"],
  "재료 조합이 같으면 같은 요리가 나옵니다. α·β는 보조 재료의 속성에 따라 갈리는 상위 변형입니다.": [
    "The same ingredient combination always cooks the same dish. α/β are upgraded variants decided by the secondary ingredient's attribute.",
    "同じ素材の組み合わせからは同じ料理ができます。α・βは副材料の属性で分かれる上位変種です。"],
  "정해진 조합이 없는 요리입니다 — 조건이 안 맞는 조합에서 나옵니다.": [
    "No fixed recipe — this is what you get from combinations that don't match anything.",
    "決まったレシピのない料理です — 条件に合わない組み合わせから生まれます。"],
  "요리 재료 효과": ["Ingredient effects", "食材の効果"],
  "보조 재료의 속성이 α(공격)·β(방어) 변형을 결정합니다.": [
    "The secondary ingredient's attribute decides the α (attack) / β (defense) variant.",
    "副材料の属性がα（攻撃）・β（防御）変種を決めます。"],
  "구분": ["Role", "区分"],
  "변형": ["Variant", "変種"],
  "효과": ["Effect", "効果"],
  "보조": ["Secondary", "副"],
  "주재료": ["Main", "主材料"],
  "음료 재료": ["Drink materials", "飲料素材"],
  "음수대에 넣으면 수분으로 바뀌는 재료와 환산량입니다.": [
    "Materials that convert to water at the drinking station, and how much you get.",
    "給水機に入れると水分に変わる素材と換算量です。"],
  "수분": ["Water", "水分"],
  "설치물": ["Structure", "設置物"],
  "해금": ["Unlock", "解禁"],
  "분류": ["Category", "分類"],
  ...
  "설명": ["Description", "説明"],
  "위험 {n}": ["Danger {n}", "危険 {n}"],
  "행동력은 이동 1회 소모량, ⚔는 적습 조우 시의 소모량입니다.": [
    "AP is the cost per move; ⚔ is the cost when an enemy raid triggers.",
    "行動力は移動1回の消費量、⚔は敵襲遭遇時の消費量です。"],
  "행동력 {n}": ["AP {n}", "行動力 {n}"],
  "균열 주 목표": ["Rift main objectives", "裂け目のメイン目標"],
  "목표": ["Objective", "目標"],
  "내용": ["Details", "内容"],
  "기한": ["Deadline", "期限"],
  "대상": ["Target", "対象"],
  "{n}일": ["{n}d", "{n}日"],
  "난이도 (환경압력)": ["Difficulty (environmental pressure)", "難易度（環境圧力）"],
  "원정": ["Expeditions", "遠征"],
  "인원": ["Team", "人数"],
  "{n}명": ["{n} ops", "{n}名"],
  "정예화 {n}+": ["Elite {n}+", "昇進{n}+"],
  "기간": ["Duration", "期間"],
  "주둔지 연구 노트로 해금하는 상시 강화입니다. 숫자는 필요한 토큰입니다.": [
    "Permanent upgrades unlocked with research notes at your base. The number is the token cost.",
    "拠点の研究ノートで解禁する常時強化です。数字は必要トークンです。"],
  "기술": ["Tech", "技術"],
  "계열": ["Branch", "系統"],
  "토큰": ["Tokens", "トークン"],
  ...
  "요리·음료에 쓰이는 모든 재료입니다. 누르면 효과와 쓰이는 요리, 얻는 곳이 나옵니다.": [
    ...
    "料理・飲料に使うすべての素材です。クリックすると効果・使う料理・入手先が出ます。"],
  "이 변형이 되는 보조 재료": ["Secondary ingredients for this variant", "この変種になる副材料"],
  "보조 재료": ["Secondary", "副材料"],
  "요리 효과": ["Cooking effect", "料理効果"],
  "얻는 곳": ["Obtained from", "入手先"],
  "쓰이는 요리": ["Used in", "使う料理"],
  "오브젝트 채집": ["Object gathering", "オブジェクト採集"],
  "사막 이야기": ["Tales Within the Sand", "砂中の遺聞"],
  "재기동 앵커": ["Reboot Anchor (CN)", "重启锚点"],
  ...
  "전술 아이템": ["Tactical item", "戦術アイテム"],
  "일반 균열": ["Standard rift", "通常の裂け目"],
  "사냥 균열": ["Hunt rift", "狩猟の裂け目"],
  "고정 균열": ["Fixed rift", "固定の裂け目"],
  "환경압력 0~{n}": ["Pressure 0-{n}", "環境圧力 0〜{n}"],
  "압력 {n}": ["Pressure {n}", "圧力 {n}"],
  "환경압력": ["Environmental pressure", "環境圧力"],
  "위험도": ["Danger level", "危険度"],
  "쾌청": ["Clear", "快晴"],
  "주 목표": ["Main objectives", "メイン目標"],
  "추가 목표": ["Sub objectives", "サブ目標"],
  "파견 팀 특성": ["Team specialties", "派遣チーム特性"],
  "특수 효과": ["Special effects", "特殊効果"],
  "등장 세력": ["Factions", "登場勢力"],
  "보상 자원": ["Rewards", "報酬資源"],
  "처치 보상": ["Kill reward", "撃破報酬"],
  "등장 지역": ["Appears in", "出現エリア"],
  "적 도감에서 자세히 보기": ["View in the Enemy Handbook", "敵図鑑で詳しく見る"],
  "{n}개 지역": ["{n} areas", "{n}エリア"],
  "{n}가지 편성": ["{n} team setups", "{n}通りの編成"],
  "정해진 조합 없음": ["No fixed recipe", "決まったレシピなし"],
  "조합": ["Recipe", "レシピ"],
  "변형별 효과": ["Variant effects", "変種ごとの効果"],
  "선택지": ["Choices", "選択肢"],
  "제작 재료": ["Materials", "製作素材"],
  "업그레이드": ["Upgrade", "アップグレード"],
  "수리 비용": ["Repair cost", "修理コスト"],
  "회수율": ["Refund", "回収率"],
  "에너지음료": ["Energy drink", "エナジードリンク"],
  "정예화": ["Elite", "昇進"],
  "직군 조건": ["Class requirement", "クラス条件"],
  "종류": ["Type", "種類"],
  "편성": ["Team setup", "編成"],
  "시즌": ["Season", "シーズン"],
  ...
  "지역·날씨": ["Areas & Weather", "エリア・天候"],
  "지역을 누르면 등장 적과 타일 도면·이동 경로 상세가 열립니다. 아래에서 획득 자원으로 걸러 볼 수 있습니다.": [
    "Click an area for its enemies, tile layout and routes. Filter by the resources it yields below.",
    "エリアをクリックすると出現する敵・タイル図・移動ルートが開きます。下の獲得資源で絞り込めます。"],
  "생존연산에 나오는 적입니다. 스탯은 가장 강화된 등장 기준이며, 누르면 등장 지역과 처치 보상이 나옵니다.": [
    "Enemies that appear in Reclamation Algorithm. Stats are from the most reinforced appearance; click for areas and kill rewards.",
    "生息演算に出現する敵です。ステータスは最も強化された出現基準で、クリックすると出現エリアと撃破報酬が出ます。"],
  "환경압력(난이도)은 카드 안에서 골라 봅니다. 보상은 그 난이도에서 나오는 자원입니다.": [
    "Pick the environmental pressure inside the card — rewards are what that level yields.",
    "環境圧力（難易度）はカード内で選びます。報酬はその難易度で得られる資源です。"],
  "같은 성과의 원정은 한 카드로 묶었습니다 — 편성(인원·정예화·기간)은 상세에서 고릅니다.": [
    "Expeditions with the same outcome share one card — pick the team setup inside.",
    "同じ成果の遠征は1枚のカードにまとめました — 編成は詳細で選びます。"],
  "재료 조합이 같으면 같은 요리가 나옵니다.": [
    "The same ingredient combination always cooks the same dish.",
    "同じ素材の組み合わせからは同じ料理ができます。"],
  "주재료로 요리를 정하고, 보조 재료를 함께 넣으면 그 재료의 종류에 따라 α·β·γ 변형이 됩니다 — α는 능력치가 하나 더 붙고, β(조미료)는 지속 시간이 늘며, γ는 공격 계열 효과가 크게 붙습니다.": [
    "Main ingredients decide the dish; adding a secondary ingredient turns it into an α/β/γ variant — α adds one more stat, β (seasoning) extends the duration, and γ adds a large attack-side bonus.",
    "主材料で料理が決まり、副材料を加えるとその種類に応じてα・β・γの変種になります — αは能力値が1つ増え、β（調味料）は持続時間が延び、γは攻撃系の効果が大きく付きます。"],
  "보조 재료의 종류가 변형을 정합니다 — α 재료(알·버섯·후추·설탕·뿔 등)는 능력치를 하나 더 붙이고, β 재료(조미료)는 지속 시간을 늘리며, γ 재료(양조주·파울 파우더·아이스 퓌레·플라워 슈거)는 공격 계열 효과를 크게 붙입니다.": [
  ...
  // 통로 타일 — 키가 빠져 EN·JA 범례에 한글로 떴다 (사용자 지적 2026-09-23)
  "통로 입구": ["Tunnel entrance", "通路入口"],
  "통로 출구": ["Tunnel exit", "通路出口"],
  "적이 여기로 들어가 통로 출구로 순간이동합니다": ["Enemies go in here and teleport to the tunnel exit", "敵はここに入り、通路出口へ瞬間移動します"],
  "통로 입구로 들어간 적이 여기서 나옵니다": ["Enemies that went into the tunnel entrance come out here", "通路入口に入った敵がここから出てきます"],
  "파괴 가능 바위": ["Breakable rock", "破壊できる岩"],
  "유적지 잔해": ["Ruin rubble", "遺跡の残骸"],
  "보물 상자": ["Treasure chest", "宝箱"],
  // 생존연산 자원 — 게임 공식 표기 (sandbox.en/ja.json 의 sandbox_1_* 이름. 명징석은 종전 'Crystal / 澄亮石'
  // 이었는데 澄亮石은 중섭 이름이다. 목재는 키가 빠져 EN·JA 에 한글로 떴다 — 사용자 지적 2026-09-23)
  "석재": ["Stone", "石材"],
  "철광석": ["Iron Ore", "鉄鉱石"],
  "명징석": ["Lighting Ore", "透珀石"],
  "목재": ["Wood", "木材"],
  "물": ["Water", "水"],
  "오브젝트": ["Object", "オブジェクト"],
  "식생": ["Vegetation", "植生"],
  "보물": ["Treasure", "宝物"],
  "더 크게": ["Even larger", "さらに拡大"],
  "원래 크기": ["Original size", "元のサイズ"],
  // 이동 경로 지도의 타일 범례 (라벨 자체가 설명이 되게 — 사용자 지적 2026-08-10)
  "적 출현": ["Spawn", "出現地点"],
  "방어 지점": ["Objective", "防衛地点"],
  "도로": ["Road", "道路"],
  "이동 가능·배치 불가": ["Walkable · no deploy", "通行可・配置不可"],
  "지상 배치 가능·이동 불가": ["Melee deploy · impassable", "地上配置可・通行不可"],
  "고지대(원거리 배치)": ["Highland (ranged deploy)", "高台(遠距離配置)"],
  "고지형(이동·배치 불가)": ["High terrain (impassable)", "高地形(通行・配置不可)"],
  "장애물(이동·배치 불가)": ["Obstacle (impassable)", "障害物(通行・配置不可)"],
  "구멍(비행만 통과)": ["Pit (flying only)", "穴(飛行のみ)"],
  "적이 나타나는 곳입니다": ["Where enemies spawn", "敵が出現する場所です"],
  "적이 도달하면 안 되는 곳입니다": ["Where enemies must not reach", "敵を到達させてはいけない場所です"],
  "적 이동 가능 · 지상 오퍼레이터 배치 가능": ["Enemies walk here; melee operators can be deployed", "敵が通行し、地上オペレーターを配置できます"],
  "적은 지나가지만 오퍼레이터는 배치할 수 없습니다": ["Enemies walk here, but operators can't be deployed", "敵は通行しますがオペレーターは配置できません"],
  "적은 못 지나가지만 지상 오퍼레이터는 배치할 수 있습니다": ["Enemies can't pass, but melee operators can be deployed", "敵は通れませんが地上オペレーターを配置できます"],
  "적 이동 불가 · 원거리 오퍼레이터 배치 가능": ["Impassable; ranged operators can be deployed", "敵は通れません・遠距離オペレーターを配置できます"],
  "높은 지형 — 적 이동도 오퍼레이터 배치도 불가합니다": ["High terrain — impassable and undeployable", "高い地形 — 通行も配置もできません"],
  "적 이동도 오퍼레이터 배치도 불가합니다": ["Impassable and undeployable", "通行も配置もできません"],
  "비행 적만 지나갈 수 있습니다": ["Only flying enemies can cross", "飛行の敵のみ通過できます"],
  ...
  "진입 조건": ["How to reach", "進入条件"],
  "엔딩 후 해금 스토리": ["Stories unlocked after the ending", "エンディング後に解禁されるストーリー"],
  "읽기": ["Read", "読む"],
  "기록": ["Record", "記録"],
  "기록 불러오는 중…": ["Loading record…", "記録を読み込み中…"],
  "기록을 불러오지 못했습니다.": ["Failed to load the record.", "記録を読み込めませんでした。"],
  ...
  "↻ 앞의 단계로 돌아갑니다": ["↻ Returns to an earlier step", "↻ 前のステップに戻ります"],
  "랜덤 전투 — 고정된 전투 맵 없이, 현재 층의 일반 전투 맵 중 하나에서 벌어집니다.": ["Random battle — no fixed map; it takes place on one of the current floor's regular battle maps.", "ランダム戦闘 — 固定マップはなく、現在の階層の通常戦闘マップのいずれかで発生します。"],
  ...
  "클릭하면 화면 크기로 확대됩니다": ["Click to enlarge", "クリックで画面サイズに拡大"],
  ...
  // 전투 노드 모달의 등장 적 절·적 상세 스탯표 (작전 도감과 같은 짜임으로 바꾼 2026-09-23)
  "강조된 수치는 난이도·긴급 배율이 반영된 값입니다.": ["Highlighted numbers include difficulty/emergency multipliers.", "強調された数値は難易度・緊急倍率を反映した値です。"],
  "난이도 {n}": ["Difficulty {n}", "難易度{n}"],
  ...
  "제안": ["Feedback", "提案"],
  "푸터 펼치기": ["Expand footer", "フッターを開く"],
  "푸터 접기": ["Collapse footer", "フッターを折りたたむ"],
  ...
  // 관리자 모드 뱃지 — 같은 자리에 '새 제안' 수를 센다 (2026-09-06)
  "새 제안 {n}개": ["{n} new suggestions", "新着提案{n}件"],
  ...
  // 이벤트 기록 — 사용자 제보 2026-08-22. 스토리 상세의 세 번째 보기 (사용자 확정 2026-08-23)
  "이벤트 기록": ["Event records", "イベント記録"],
  "이벤트를 진행하면서 미니게임·수집 요소로 하나씩 풀리던 글입니다 — 본편 스토리에는 없고, 이벤트가 끝나면 게임에서도 다시 볼 수 없습니다.": [
    "Writing that unlocked piece by piece through this event's minigames and collectibles — none of it is in the main story, and once the event closed it was gone from the game too.",
    "イベントを進める中でミニゲームや収集要素によって一つずつ解放されていた文章です — 本編ストーリーにはなく、イベントが終わるとゲーム内でも読めなくなります。"],
  "이벤트 기록을 불러오지 못했습니다.": ["Could not load the event records.", "イベント記録を読み込めませんでした。"],
  "＋기록 {n}": ["+{n} records", "＋記録{n}"],
  "이 이벤트에는 미니게임·수집으로 풀리던 이벤트 기록이 함께 있습니다": [
    "This event also has records unlocked through its minigames and collectibles",
    "このイベントには、ミニゲームや収集要素で解放された記録も収録されています"],
  "글 {n}편": ["{n} pieces", "{n}編"],
  "{n}편": ["{n} found", "{n}編"],
  ...
  "초대 문구 또는 방 ID": ["Invite text or room ID", "招待文または部屋ID"],
  "박사 {n}": ["Doctor {n}", "ドクター{n}"],
  "입장": ["Enter", "入室"],
  "잘못된 방 ID입니다 — 방 ID는 영문·숫자 14자입니다 (예: kmk0im89g02bli).": [
    "Invalid room ID — a room ID is exactly 14 letters/digits (e.g. kmk0im89g02bli).",
    "部屋IDが正しくありません — 部屋IDは英数字14文字です（例: kmk0im89g02bli）。"],
  "방은 처음 들어오는 사람이 자동으로 만들고, 모두 나가면 1분 뒤 사라집니다. 남는 것은 전략·맹약 선택뿐입니다.": [
    "The first person to enter creates the room automatically; it disappears 1 minute after everyone leaves. Only strategy and alliance picks are kept.",
    "部屋は最初に入った人が自動で作り、全員が退室すると1分後に消えます。残るのは戦略・盟約の選択だけです。"],
  "방": ["Room", "部屋"],
  "링크 복사": ["Copy link", "リンクをコピー"],
  "게임 초대 문구와 이 방의 링크를 함께 복사합니다": ["Copies the game's invite text together with this room's link", "ゲームの招待文とこの部屋のリンクをまとめてコピーします"],
  "연결 중": ["Connecting", "接続中…"],
  "다시 연결 중": ["Reconnecting", "再接続中"],
  "자리가 없음": ["Room full", "満員"],
  "나가기": ["Leave", "退室"],
  "이 방은 이미 4명이 있습니다. 자리가 나면 다시 시도해 주세요.": [
    "This room already has 4 people. Try again when a seat frees up.",
    "この部屋はすでに4人います。席が空いたら再試行してください。"],
  "빈 자리": ["Empty seat", "空席"],
  ...
  "오늘 만들어진 방 {n}개": ["{n} rooms created today", "本日作成された部屋 {n}件"],
  "날짜별 방 생성 수": ["Rooms created per day", "日付別の部屋作成数"],
  "{n}개": ["{n}", "{n}件"],
  "{n}회": ["{n}x", "{n}回"],
  "아직 닉네임을 읽은 방이 없습니다.": ["No room has a readable nickname yet.", "まだニックネームを読めた部屋がありません。"],
  "초대 문구를 못 읽어 만든 사람을 모르는 방 {n}개 — 한국어 클라 문구만 읽습니다.": ["{n} rooms whose invite text could not be read — only the Korean client wording is parsed.", "招待文を読めず作成者が不明な部屋 {n}件 — 韓国語クライアントの文言のみ読み取ります。"],
  "이 날 만들어진 방이 없습니다.": ["No rooms were created on this day.", "この日に作られた部屋はありません。"],
  "알 수 없음": ["Unknown", "不明"],
  "만들어진 방": ["rooms created", "作成された部屋"],
  "만든 사람 모름 {n}": ["{n} unknown creator", "作成者不明 {n}"],
  "시간대별 방 생성 수": ["Rooms created by hour", "時間帯別の部屋作成数"],
  "막대를 누르면 그 날 누가 언제 만들었는지 볼 수 있습니다.": ["Click a bar to see who created rooms that day, and when.", "棒を押すと、その日の作成者と時刻が見られます。"],
  "이 날 기록은 집계만 남아 있습니다 — 방별 기록을 남기기 전에 만들어진 방입니다.": ["Only totals remain for this day — these rooms predate per-room logging.", "この日は集計のみ残っています — 部屋ごとの記録を始める前の部屋です。"],
  ...
  "방 목록을 불러오지 못했습니다.": ["Could not load the room list.", "部屋一覧を読み込めませんでした。"],
  "방 {n}개": ["{n} rooms", "部屋{n}"],
  "지금 열린 파티 공유 방": ["Party rooms open right now", "現在開いているパーティ部屋"],
  "닫힌 방": ["Room closed", "閉じた部屋"],
  "방이 만들어진 지 6시간이 지나 닫혔습니다. 같은 ID로 다시 입장하면 새 방이 열립니다.": [
    "This room was closed 6 hours after it was created. Entering the same ID again opens a fresh room.",
    "この部屋は作成から6時間が経過したため閉じられました。同じIDで再入室すると新しい部屋が開きます。"],
  "입장 화면으로": ["Back to entry", "入室画面へ"],
  "방이 정리되어 닫혔습니다. 같은 ID로 다시 입장하면 새 방이 열립니다.": [
    "This room was cleaned up and closed. Entering the same ID again opens a fresh room.",
    "この部屋は整理のため閉じられました。同じIDで再入室すると新しい部屋が開きます。"],
  "나": ["Me", "自分"],
  "아직 안 골랐음": ["Not chosen yet", "未選択"],
  "맹약 고르기": ["Pick alliances", "盟約を選ぶ"],
  "선택 해제": ["Clear", "選択解除"],
  "완료": ["Done", "完了"],
  "함께 고른 맹약": ["Alliances picked by more than one", "複数人が選んだ盟約"],
  "최대 {n}개까지 선택 가능": ["Up to {n} can be selected", "最大{n}つまで選択可能"],
  // 신호 — 채팅 대신 미리 정한 한마디 (사용자 요청: "연결이 끊겼어요 죄송해요 라는 느낌의 의사를 전달 가능한 버튼")
  "신호": ["Signal", "シグナル"],
  "신호 내리기": ["Withdraw signal", "シグナルを下げる"],
  "연결이 끊겼어요, 죄송해요": ["I got disconnected, sorry", "接続が切れました、すみません"],
  "잠시만요": ["One moment", "少し待って"],
  "준비됐어요": ["Ready", "準備OK"],
  "방금": ["just now", "たった今"],
  "{n}분 전": ["{n} min ago", "{n}分前"],
  ...
  "정예화(골든) 전환 — 판을 비워도 남습니다": ["Toggle Golden — kept when you clear the board", "精鋭化（ゴールデン）切替 — 盤をクリアしても残ります"],
  "빼기": ["Remove", "外す"],
  ...
  "정비구역 {n} 포함": ["incl. {n} in standby", "整備エリア{n}を含む"],
  "정비구역 {n} — 아직 안 셈": ["{n} in standby — not counted yet", "整備エリア{n} — まだ数えません"],
  "발동": ["Active", "発動"],
  "미발동": ["Inactive", "未発動"],
  ...
  "아래에서 라운드를 고르면 그 라운드의 적과 오는 길이 이 지도에 그려집니다.": [
    "Pick a round below and its enemies and their paths are drawn on this map.",
    "下でラウンドを選ぶと、そのラウンドの敵と進路がこの地図に描かれます。"],
  ...
  // 위수 협의 전장 모달 캡션의 모드 이름 — 경로 파일(autochess-routes.json)이 한국어로만 싣는다.
  // 게임 공식 표기 (autochess.en/ja.json modes). 종전엔 EN·JA 에 한글로 떴다 (사용자 지적 2026-09-23)
  "입문 협의": ["Entry Protocol", "指導協定"],
  "표준 시뮬레이션": ["Standard Simulation", "標準"],
  "험지 시뮬레이션": ["Perilous Simulation", "逆境"],
  "극한 시뮬레이션": ["Dire Simulation", "死地"],
  "초월 시뮬레이션": ["Ultimate Simulation", "究極"],
  "일반 라운드 전장": ["Normal-round battlefield", "通常ラウンドの戦場"],
  "리더 라운드 전장": ["Leader-round battlefield", "リーダーラウンドの戦場"],
  "2·3라운드는 같은 전장입니다": ["Rounds 2 and 3 share one battlefield", "2・3ラウンドは同じ戦場です"],
  "적 {a}종 · 오는 길 {b}갈래": ["{a} enemy types · {b} paths", "敵{a}種・進路{b}本"],
  "전투 맵을 불러오는 중…": ["Loading battle maps…", "戦闘マップを読み込み中…"],
  "일반 라운드": ["Normal rounds", "通常ラウンド"],
  "모든 난이도 공통 — 라운드가 오를수록 적이 늘고 오는 길이 많아집니다": [
    "same on every difficulty — later rounds bring more enemies down more paths",
    "全難易度共通 — ラウンドが進むほど敵が増え、進路も多くなります"],
  "리더 라운드": ["Leader rounds", "リーダーラウンド"],
  "14R · 15R — 표준 시뮬레이션은 9R": ["R14 · R15 — R9 in Standard Simulation", "14R・15R — 標準シミュレーションは9R"],
  "{n}R": ["R{n}", "{n}R"],
  "중첩 {n} 기준": ["at {n} stacks", "重ね掛け{n}基準"],
  "자금": ["Funds", "資金"],
  "무료 갱신": ["Free refresh", "無料更新"],
  "쓰러질 시": ["On defeat", "撃破された時"],
  "획득할 때 1회": ["once, on acquisition", "獲得時に1回"],
  "전투마다": ["every battle", "戦闘ごと"],
  "라운드마다": ["every round", "ラウンドごと"],
  "가장 많이 중첩된 맹약": ["Most-stacked alliance", "最も重なった盟約"],
  "자신 몫은 위 합계에 포함, 이웃 몫은 대상에 따라 다릅니다": [
    "own share counted above; the neighbor's share depends on who stands there",
    "自分の分は上の合計に含まれます。隣の分は相手によって変わります"],
  ...
  // 맹약 중첩 수치 (2026-08-24) — 게임 설명문이 "(중첩 수에 따라 변경)"으로 뭉갠 실제 값
  // 기간 한정 바로가기의 남은 시간 (2026-08-24)
  "{n}일 남음": ["{n} days left", "残り{n}日"],
  "{n}시간 남음": ["{n} hours left", "残り{n}時間"],
  "{n}분 남음": ["{n} min left", "残り{n}分"],
  "중첩당": ["per stack", "加算ごとに"],
  "/중첩": ["/stack", "/加算"],
  "최대 {v}": ["Max {v}", "最大{v}"],
  "최대 {n}중첩": ["Max {n} stacks", "最大{n}スタック"],
  "탄약 1발당 공격력": ["ATK per ammo spent", "弾薬1発ごとの攻撃力"],
  "장비 장착 시 공격력": ["ATK with gear", "装備装着時の攻撃力"],
  "승급 장비면 추가": ["Upgraded gear adds", "強化装備なら追加"],
  "방어력·마법 저항 무시": ["DEF / RES ignored", "防御力・術耐性を無視"],
  "재배치 시간": ["Redeploy time", "再配置時間"],
  ...
  "트루 대미지": ["True damage", "真ダメージ"],
  "마법 대미지": ["Arts damage", "術ダメージ"],
  "대미지 배율": ["Damage multiplier", "ダメージ倍率"],
  "냉기·빙결 적 대미지 배율": ["Damage multiplier vs Chilled/Frozen", "冷気・凍結の敵へのダメージ倍率"],
