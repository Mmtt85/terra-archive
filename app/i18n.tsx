...
const D: Record<string, Pair> = {
  ...
  "중첩 수치": ["Per-stack values", "加算数の実数値"],
  // 기간 한정 바로가기의 남은 시간 (2026-08-24)
  "{n}일 남음": ["{n} days left", "残り{n}日"],
  "{n}시간 남음": ["{n} hours left", "残り{n}時間"],
  "{n}분 남음": ["{n} min left", "残り{n}分"],
  "중첩당": ["per stack", "加算ごとに"],
  "{n}명 배치": ["{n} deployed", "{n}名配置"],
  "상위 단계": ["Higher tier", "上位段階"],
  "/중첩": ["/stack", "/加算"],
  "중첩 {n} → {v}": ["{n} stacks → {v}", "{n}加算 → {v}"],
  "게임 설명문이 '(중첩 수에 따라 변경)'으로만 적어 둔 실제 계수입니다. 중첩 수는 판이 도는 동안 오퍼레이터 능력·이벤트로 쌓이는 맹약별 누적 값입니다. 'N명 배치' 상위 단계 효과도 중첩을 타면 여기 함께 있습니다.": [
    "The real coefficients behind the game's \"(varies with stack count)\". Stack count is a per-alliance total that builds up over a run through operator abilities and events. Higher-tier effects (\"N deployed\") are listed here too when they scale with stacks.",
    "ゲームの説明文が「(加算数によって変動)」としか書いていない実際の係数です。加算数は、周回中にオペレーターの能力やイベントで盟約ごとに積み上がる累計値です。「N名配置」の上位段階効果も、加算で変わるものはここに含まれます。"],
  "공격 속도": ["ASPD", "攻撃速度"],
  "지속 시간": ["Duration", "持続時間"],
