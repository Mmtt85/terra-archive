...
const D: Record<string, Pair> = {
  ...
  "자동": ["Auto", "自動"],
  "클립보드 자동인식 — 켜두면 모달을 열지 않아도 캡처만 하면 바로 인식·적용됩니다": ["Clipboard auto-recognition — while on, captures are recognized and applied instantly without opening the modal", "クリップボード自動認識 — オンにするとモーダルを開かなくてもキャプチャするだけで即認識・適用されます"],
  "스샷 자동인식 켜짐 — 게임 화면을 캡처하고 이 탭으로 돌아오면 바로 이동합니다": ["Screenshot auto-recognition on — capture the game screen and return to this tab to jump instantly", "スクショ自動認識オン — ゲーム画面をキャプチャしてこのタブに戻ると即移動します"],
  "스샷 자동인식 켜짐 — 게임 화면을 캡처하고 이 탭으로 돌아오면 바로 적용됩니다": ["Screenshot auto-recognition on — capture the game screen and return to this tab to apply instantly", "スクショ自動認識オン — ゲーム画面をキャプチャしてこのタブに戻ると即適用されます"],
  "테마를 특정하지 못했습니다 — 📷 버튼을 눌러 테마를 선택하세요": ["Couldn't determine the theme — press the 📷 button to pick one", "テーマを特定できませんでした — 📷ボタンを押してテーマを選んでください"],
  ...
  "통합전략 화면(분대 선택·전리품·작전 노드·조우 등)을 클립보드로 캡처(맥 ⌃⌘⇧4 · 윈도우 Win+Shift+S)하고 이 탭으로 돌아오면, 가이드의 해당 정보로 바로 이동합니다. 파일 드롭이나 ⌘V 붙여넣기도 됩니다.": ["Capture an Integrated Strategies screen (squad select, loot, operation nodes, encounters…) to the clipboard (Mac ⌃⌘⇧4 · Windows Win+Shift+S) and return to this tab — you'll jump straight to that info in the guide. File drop and ⌘V paste also work.", "統合戦略の画面（分隊選択・戦利品・作戦ノード・遭遇など）をクリップボードにキャプチャ（Mac ⌃⌘⇧4・Windows Win+Shift+S）してこのタブに戻ると、ガイドの該当情報へ直行します。ファイルドロップや⌘V貼り付けも使えます。"],
  "공개모집 화면을 클립보드로 캡처(맥 ⌃⌘⇧4 · 윈도우 Win+Shift+S)하고 이 탭으로 돌아오면, 인식된 태그가 자동으로 선택됩니다. 파일 드롭이나 ⌘V 붙여넣기도 됩니다.": ["Capture the recruitment screen to the clipboard (Mac ⌃⌘⇧4 · Windows Win+Shift+S) and return to this tab — the recognized tags are selected automatically. File drop and ⌘V paste also work.", "公開求人画面をクリップボードにキャプチャ（Mac ⌃⌘⇧4・Windows Win+Shift+S）してこのタブに戻ると、認識されたタグが自動選択されます。ファイルドロップや⌘V貼り付けも使えます。"],
  ...
  "클립보드 접근이 막혀 있습니다 — ⌘V 붙여넣기나 파일 드롭을 이용하세요": ["Clipboard access is blocked — use ⌘V paste or file drop", "クリップボードへのアクセスがブロックされています — ⌘V貼り付けかファイルドロップをご利用ください"],
  "클립보드 확인 중…": ["Checking clipboard…", "クリップボードを確認中…"],
