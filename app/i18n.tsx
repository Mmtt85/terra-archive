...
const D: Record<string, Pair> = {
  ...
  "현재 보유·정예화 상태로 자동편성을 돌린 결과와, 후보만 완성했을 때의 기지 총점 차이로 실제 이득을 증명합니다. 방 %효율 변화와 완성 비용(재료·용문폐)은 정확하며, 일일 용문폐·작전기록과 회수일은 기준 상수 미확정 근사치(β)입니다.": ["The benefit is proven by the difference in total base score between the current plan and the plan with only the candidate completed. Room %-efficiency changes and completion cost (materials/LMD) are exact; daily LMD/EXP and payback days are approximations (β) pending finalized constants.", "現在の所持・昇進状態で自動編成した結果と、候補だけを完成させたときの基地総合スコア差で実際の利得を証明します。部屋の%効率変化と完成コスト（素材・龍門幣）は正確で、日当たり龍門幣・作戦記録と回収日数は基準定数が未確定の近似値（β）です。"],
  ...
  "완성": ["Complete", "完成"],
  "기지 +{n}": ["Base +{n}", "基地 +{n}"],
  "현재와 완성 후의 기지 자동편성 총점 차이 — 추천 순위 기준": ["Difference in total base score before/after completion — the ranking basis", "完成前後の基地自動編成の総合スコア差 — おすすめ順位の基準"],
  "{room} · {shift}에 배치됩니다": ["Placed in {room} · {shift}", "{room} · {shift} に配置されます"],
  "/일": ["/day", "/日"],
  "무역 오더 효율 개선 — 순금 공급에 따라 실질 용문폐가 달라져 원화 환산은 생략": ["Improves trade order efficiency — actual LMD depends on gold supply, so no daily figure is shown", "取引の受注効率が向上 — 実際の龍門幣は純金の供給次第のため、日当たり換算は省略"],
  "제조소(용문폐·작전기록) 외 방이라 일일 자원 환산은 생략": ["Outside the factory (LMD/EXP) lines, so no daily-resource figure", "製造所（龍門幣・作戦記録）以外の部屋のため、日当たり資源換算は省略"],
  "완성 비용": ["Completion cost", "完成コスト"],
  "작전기록": ["Battle Records", "作戦記録"],
  "경험치": ["EXP", "経験値"],
  "이성 환산": ["Sanity equiv.", "理性換算"],
  "회수 ≈ {d}일": ["Payback ≈ {d} days", "回収 ≈ {d}日"],
  "회수일 산출 생략": ["Payback n/a", "回収日数の算出省略"],
