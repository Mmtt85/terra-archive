...
const HELP_SECTIONS: { title: string; items: string[] }[] = [
  { title: "교대 정책", items: [
    ...
    "훈련실 정원 2는 트레이너석 1 + 훈련대상석 1이라, 플래너가 쓰는 근무석은 1석입니다. 다만 훈련 속도는 자원 생산과 단위가 달라 총점 계산에서는 빼둡니다 — 그래서 생산 방(제조소·무역소·발전소·사무실·응접실·제어 센터)을 먼저 다 채운 뒤 남는 인원 중 훈련 속도가 가장 높은 오퍼로 훈련실을 채웁니다. 자원 생산은 한 톨도 양보하지 않고, 훈련실은 교대 시계에 들어가는 방이 아니라 지속 시간도 줄지 않습니다. 다른 오퍼의 조건이 훈련실 배치로만 충족되는 경우(외드레르의 W가 앉을 작업 시설이 훈련실밖에 안 남은 경우)에는 그쪽이 우선입니다.",
