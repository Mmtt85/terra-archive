...
// product: 생산품 한정 오라(플레임테일 — 작전기록 제조소에만 적용, 피드백 2026-07-14)
type AmbientAura = { kind: string; value: number; gateFaction?: string | null; gateCount?: number | null; belowThreshold?: number | null; product?: string | null };
  ...
  for (const op of controlTeam) {
    ...
    for (const skill of b.skills) {
      ...
      if (skill.gateFaction || skill.belowThreshold != null) {
        list.push({ kind: skill.kind, value: skill.value, gateFaction: skill.gateFaction, gateCount: skill.gateCount ?? 1, belowThreshold: skill.belowThreshold, product: skill.product });
      } else {
        list.push({ kind: skill.kind, value: skill.perFaction ? b.auras[skill.kind] ?? 0 : skill.value, product: skill.product });
      }
    }
  }
  return list;
...
function ambientFor(room: string, team: InfraOp[], ambient?: AmbientAura[], roomEfficiency = 0, product?: string): number {
  if (!team.length || !ambient) return 0;
  const target = AURA_TARGET[room] ?? "";
  let best = 0;
  for (const aura of ambient) {
    if (aura.kind !== target) continue;
    // 생산품 한정 오라(플레임테일: 작전기록류만) — 다른 생산품 방엔 적용하지 않는다
    if (aura.product && aura.product !== "any" && product && aura.product !== product) continue;
...
function breakdown(op: InfraOp, room: string, team: InfraOp[], ctx: Ctx): OpBreakdown {
  ...
  for (const skill of activeSkills(op, room, ctx.product)) {
    ...
    // 제어센터 원격 오라(야하타 "무역소 내"·플레임테일 "제조소에 배치된")의 기지 전체 근사는
    // 같은 제어센터에 앉은 동일 진영 인원(플레임테일 본인 등)을 빼서 과산정을 줄인다
    if (skill.perFaction && skill.perSkillTag == null) {
      const count = skill.perScope === "room"
        ...
        : Math.max(0, (ctx.factionCounts?.[skill.perFaction] ?? 0)
            - (skill.kind in AURA_WEIGHT ? team.filter((member) => factionsOf(member).includes(skill.perFaction!)).length : 0));
...
function bestTeam(room: string, slots: number, pool: Map<string, InfraOp>, ctx: Ctx, seedOps: InfraOp[] = []): InfraOp[] {
  ...
  // 솔로·제로아웃 스킬 오퍼가 상위에 있으면 그리디가 '혼자 45~50%'에 갇혀 '셋이 85%' 조합을
  // 놓친다 — 첫 픽이 위디(자동화 45)면 일반 +N% 오퍼는 전부 0이 돼 아무도 추가되지 못하는
  // 함정. 이런 오퍼를 뺀 대안 팀도 만들어 방 점수로 비교한다 (automation/override 누락은
  // 위디 단독 제조소 배치 원인 — 사용자 피드백 2026-07-18).
  const trapKinds = new Set(["solo", "automation", "automation_crew", "override"]);
  const noTrap = ranked.filter((op) => !op.skills.some((skill) => trapKinds.has(skill.kind) && skillApplies(skill, room, ctx.product)));
  if (noTrap.length !== ranked.length) {
    const alt = fill(seedOps, noTrap);
...
// 감사 하네스 전용 내부 노출 — UI에서는 쓰지 않는다. 방별 통째 비교(시뮬) 검증 시
// esbuild 번들 → node로 bestTeam/teamScore를 직접 호출한다 (피드백 검증 절차 2026-07-19).
export const __audit = {
  bestTeam, teamScore, opSolo,
  buildPlan: (...args: Parameters<typeof buildPlan>) => buildPlan(...args),
  optimize: (...args: Parameters<typeof optimize>) => optimize(...args),
};
...
function buildPlan(packageTokens: string[], roster: InfraOp[], factionSets = false, priority: ProdPriority = "gold"): Plan {
  ...
  for (let shift = 0; shift < SHIFT_COUNT; shift += 1) {
    ...
    // 피누스 실베스트리스 세트 (피드백 2026-07-14): 플레임테일(제어센터, 제조소 기사단
    // 1명당 작전기록 +10% 오라) 보유 시 **회복 교대(B조)**에 기사단 제조 요원 + 플레임테일을
    // 결집한 후보안. 그리디는 제조소를 제어센터보다 먼저 채워 "오라는 인원이 있어야 켜지고,
    // 인원은 오라가 켜져야 이긴다"는 닭-달걀로 세트를 못 연다 — 쉐라그 세트처럼 시드로 열고
    // optimize()가 세트 없는 안과 기지 총점을 비교해 이득일 때만 채택한다. B조인 이유:
    // A조 제어센터·제조소는 화식 세트와 상위 생산 오퍼 몫이라 기회비용이 크다.
    if (shift === 1 && factionSets) {
      for (const auraOp of roster) {
        if (used.has(auraOp.id)) continue;
        for (const skill of auraOp.skills) {
          if (skill.room !== "CONTROL" || !(skill.kind in AURA_WEIGHT) || !skill.perFaction || !skill.product || skill.product === "any") continue;
          const bodies = roster
            .filter((op) => op.id !== auraOp.id && !used.has(op.id) && factionsOf(op).includes(skill.perFaction!)
              && op.skills.some((s) => skillApplies(s, "MANUFACTURE", skill.product)))
            .sort((a, b) => opSolo(b, "MANUFACTURE", 3, { tokenPoints: {} }) - opSolo(a, "MANUFACTURE", 3, { tokenPoints: {} }))
            .slice(0, 3);
          if (bodies.length < 2) continue;
          const cell = LAYOUT.find((c) => c.room === "MANUFACTURE" && c.product === skill.product);
          if (!cell) continue;
          seeds[cell.key] = [...(seeds[cell.key] ?? []), ...bodies].slice(0, 3);
          seeds["CONTROL"] = [...(seeds["CONTROL"] ?? []), auraOp].slice(0, 5);
          // 예약 등록 — B조 전수 감사(seedKeep)가 세트를 쓸어내지 않도록. shift 0 풀은
          // 이미 지나갔으므로(루프 순서) A조 배치에는 영향이 없다
  ...
  const strategy = (packageTokens.length ? `${packageTokens.join(" + ")} 패키지` : "기본 편성") + (factionSets ? " + 진영 세트" : "");
...
function optimize(roster: InfraOp[], priority: ProdPriority = "gold"): Plan {
  ...
  // 진영 세트 후보(쉐라그 게이트 오라·플레임테일 생산품 한정 진영 오라)가 있으면
  // 세트 포함안을 만들어 총점이 높을 때만 채택
  const hasGatedAura = roster.some((op) => op.skills.some((skill) => skill.room === "CONTROL" && skill.gateFaction));
  const hasPerFactionSet = roster.some((op) => op.skills.some((skill) =>
    skill.room === "CONTROL" && skill.perFaction && skill.kind in AURA_WEIGHT && skill.product && skill.product !== "any"));
  if (!hasGatedAura && !hasPerFactionSet) return base;
...
const HELP_SECTIONS: { title: string; items: string[] }[] = [
  ...
  { title: "포인트 시너지 (시설 간)", items: [
    ...
    "기억 조각(위스퍼레인): 사무실에서 모집 인원마다 +10(초기 제외 = 20)이 쌓이고, 정예화 2 '추억 회상'이 이를 감지 정보로 1:1 전환합니다 — 로즈몬티스(생각의 사슬)·에벤홀츠(공명) 체인의 공급원이 되어 사무실 배치가 추천에 반영됩니다.",
  ...
  { title: "제어 센터", items: [
    ...
    "플레임테일(피누스 실베스트리스 기사)은 제조소의 기사단 인원 1명당 작전기록 생산력 +10% 오라입니다 — 작전기록 방에만 적용되고(귀금속은 감산이라 배치를 피함), 애쉬락·와일드메인·파투스(각 +25%)와 함께 두면 작전기록방이 최대 105%까지 오릅니다. 정예 로스터에선 주로 회복 교대(B조)에 결집합니다.",
