...
type AmbientAura = { kind: string; value: number; gateFaction?: string | null; gateCount?: number | null };
...
function aurasOf(controlTeam: InfraOp[], ctx: Ctx): AmbientAura[] {
  ...
  for (const op of controlTeam) {
    ...
    for (const skill of b.skills) {
      ...
      if (skill.gateFaction) list.push({ kind: skill.kind, value: skill.value, gateFaction: skill.gateFaction, gateCount: skill.gateCount ?? 1 });
      else list.push({ kind: skill.kind, value: skill.perFaction ? b.auras[skill.kind] ?? 0 : skill.value });
    }
  }
  return list;
}

// 이 방이 실제로 받는 오라 (동종 최고만) — 조건부 오라는 방 구성원이 조건을 채울 때만
function ambientFor(room: string, team: InfraOp[], ambient?: AmbientAura[]): number {
...
function teamScore(team: InfraOp[], room: string, ctx: Ctx): number {
  ...
  return efficiency + quality + payout + auras + ambientFor(room, team, ctx.ambient);
}
  ...
  const fillGaps = () => {
    ...
    for (let shift = 0; shift < SHIFT_COUNT; shift += 1) {
      ...
      for (const key of [...PRODUCTION_KEYS, ...SUPPORT_KEYS]) {
        ...
        if (key === "MEETING" && shift > 0) continue; // 응접실은 상시 편성
        const cell = cellByKey.get(key)!;
...
function RoomModal({ cell, plan, allAssigned, roster, opMap, initialShift, onClose, onShowOperator, onUpdateTeam, onNotify, eliteById, onSetElite }: { cell: { key: string; room: string; label: string; product?: string }; plan: Plan; allAssigned: Set<string>; roster: InfraOp[]; opMap: Map<string, InfraOp>; initialShift: number; onClose: () => void; onShowOperator?: (id: string) => void; onUpdateTeam?: (cellKey: string, shiftIdx: number, ids: string[]) => void; onNotify?: (message: string) => void; eliteById: Map<string, Elite>; onSetElite: (id: string, elite: Elite) => void }) {
  ...
  const agg = team.reduce((acc, op) => {
    ...
    return acc;
  }, { "스킬 효율": 0, "시설 기반": 0, "자동화": 0, "품질 기대치": 0, "오더 수익": 0, "효율 오버라이드": 0, "동료 보너스": 0, "제어 오라(가중)": 0, "제어센터 오라 수신": ambientFor(cell.room, team, ambient) } as Record<string, number>);
