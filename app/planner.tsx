          ...
          <>
            ...
            {(["gold", "exp", "balance", "endless", "perpetual"] as const).map((mode) => (
              <label key={mode} className={priority === mode ? "on" : ""}
                title={mode === "endless" ? t("A조를 컨디션 소모·회복까지 계산해 최대한 오래가게 짭니다(기지 전체 교대 최소·효율 겸비) — B조는 A조가 쉬는 동안의 생산 교대")
                  : mode === "perpetual" ? t("효율을 따지지 않습니다 — 제어센터는 무조건 무한동력(순소모 0·양조 고정) 조합으로 만들고, 나머지 방도 최대한 오래가게 짭니다") : undefined}>
...
const HELP_SECTIONS: { title: string; items: string[] }[] = [
  { title: "교대 정책", items: [
    ...
    "훈련실 정원 2는 트레이너석 1 + 훈련대상석 1이며, 인프라 스킬 효과는 트레이너 1명만 반영됩니다 — 그래서 플래너가 쓰는 훈련실 근무석은 1석뿐입니다. 그 한 석도 실제 스킬 특화 훈련에 쓰도록 기본적으로 비워 둡니다. 다만 비움이 절대 규칙은 아닙니다 — 훈련실도 '작업 시설'이라, 다른 오퍼의 조건이 훈련실 배치로만 충족될 때(외드레르의 W가 앉을 작업 시설이 훈련실밖에 안 남은 경우)는 그쪽 이득이 실제로 더 클 때만 배치합니다.",
