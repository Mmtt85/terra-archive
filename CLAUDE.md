...
- **점검일에는 스킬을 쓴다** — 손으로 꿰면 매번 빠뜨린다 (CI가 축약해 도는 무거운 단계인
  ...
  중섭은 **미래시(선행 정보) 전용**이라 17표만 받는다. 한섭에 언젠가 넘어올
  오퍼·재료·모듈·스킨이 주된 수확이지만, **2026-09-17부터 `activity`·`stage`·`zone`이
  더해져** 이벤트 도감이 미래시 이벤트의 작전·등장 적·교환 재화까지 중섭에서 미리 뽑는다
  (그 전까지의 "중섭 이벤트가 열려도 사이트는 그대로"는 이제 맞지 않는다). 다만 중섭
  콘텐츠를 한섭 화면에 그대로 싣지는 않는다.
- **`ci-refresh.sh`는 `SKIP_FETCH=1`로 돌린다** (CDN에서 받아 온 뒤라면). 안 그러면 맨 앞에서
  네 서버를 CDN에서 다시 받는데, 하나라도 실패하면 `fetch-gamedata.py`(클뜯 레포판)로 물러나
  **방금 받은 최신 데이터를 덮어쓴다** — 레포는 며칠씩 밀려서(실측 11일) 조용히 옛 데이터로
  사이트가 만들어진다. 무인 CI는 기본값(CDN 우선 수신) 그대로 둔다.
