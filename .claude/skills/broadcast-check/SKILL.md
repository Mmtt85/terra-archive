...
- 사이트는 정적(static)이라 실시간 API가 없다. **예정 방송 일정만 JSON에 넣어두면**, 헤더의 `BroadcastBadges`(app/page.tsx)가 현재 시각과 비교해 상태를 **자동 계산**한다:
  - `now < start` → `📅 예약됨` (D-N / N시간 후 / 곧 시작)
  - `start ≤ now ≤ start+durationMin` → `● 생방송 중` (빨간색 깜빡임)
  - 종료 후 **이틀 이내** → `✓ 오늘/어제 방송`
  - 그 이후 → 자동으로 숨김 (JSON에서 지울 필요 없음)
- 따라서 상태 전환은 시간이 알아서 처리한다. **새 방송이 확정될 때만** 이 파일을 갱신하면 된다.
