...
> **순서 절대룰 (사용자 확정 2026-08-01, 재확인 2026-09-16)**: 신규 오퍼가 걸려 있으면
> `SKIP_FETCH=1 bash scripts/ci-refresh.sh fast`(~40초)를 먼저 돌려 **커밋·푸시·배포까지
> 끝낸 뒤** 나머지(`rest`, ~7분)를 한다. 스토리 요약·번역처럼 오래 걸리는 것은 **오퍼가
> 배포된 다음에** 손댄다. 개방 당일에 순서를 뒤집으면 그 몇십 분 동안 사람들이 갱신 안 된
> 화면을 보고 나간다 (2026-09-16 실측). 중간에 "할까요?"로 멈추지 말고 끝까지 진행한다.
