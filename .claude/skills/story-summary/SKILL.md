...
— 이벤트별로 따로 켤 필요 없다.** 판정 기준: '읽는 중' 영역을 화면 하단(45%~90%)으로
잡아, 스크롤을 내리며 방금 나타난 **아래쪽 문단부터** 엔티티를 고르고(아래쪽 우선,
정렬 `b-a`), 위로 올라간 문단(45% 위)은 빠르게 제외한다 (2026-07 사용자 확정 —
StoryDetail의 IntersectionObserver rootMargin·active 정렬). 좁은 화면(≤1180px)
