...
"act48side": {
  ...
  "chars": [  // 인물 카드 — 스탠딩 CG는 --chars로 수집, 없으면 img 생략(텍스트 카드)
    { "name": "티티", "desc": "스포일러 없는 한 줄", "img": "/story/char/….png", "alias": ["메제티케티"] }
  ],
...
보이는 문단에 이름(또는 alias)이 언급된 카드가 오른쪽에 떠오른다(name 부분 문자열 매칭).
그래서 desc는 반드시 그 시점 기준 스포일러가 없어야 하고, 정체가 반전인 인물·용어는
아예 넣지 않는다. 좁은 화면에선 레일 대신 상단 인물 갤러리(img 있는 chars만)가 보인다.
