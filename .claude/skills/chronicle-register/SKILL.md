...
- **미출시(CN 미래시, `unreleased: true`) 이벤트는 연대기에 넣지 말 것.** 연대기(테라 연대기 뷰)는
  미래시 토글과 무관하게 항상 표시되므로, 미출시 이벤트를 넣으면 토글 꺼도 노출되고 미래시 목록과
  중복된다. KR 정식 출시(unreleased 해제)된 뒤에 등록한다. (ci-report 감지도 미출시는 제외한다.)
- 연대기는 **선별 큐레이션**이다. 모든 잔이벤트를 넣을 필요는 없지만, 스토리 요약이 있는
  (출시된) 사이드/메인 이벤트는 연표에 포함하는 게 기본. 애매하면 사용자에게 편입 여부를 묻는다.
