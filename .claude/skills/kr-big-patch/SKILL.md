...
## 2. 결정론 레인 — **두 번에 나눠 내보낸다** (사용자 확정 2026-08-01, 재확인 2026-09-16)

오퍼 도감은 40초면 끝나는데 인프라·회귀검증·파밍·스토리 뒤에 줄을 세우면 몇십 분 늦는다.
**신규 오퍼부터 배포하고, 무거운 건 그 뒤에 따로 배포한다.** 개방 당일엔 그 몇십 분 동안
사람들이 갱신 안 된 화면을 보고 나간다 (2026-09-16 실측).

```bash
SKIP_FETCH=1 bash scripts/ci-refresh.sh fast   # 오퍼·다국어·아바타·스킬레벨·프로필 (~40초)
# → 여기서 바로 커밋·푸시하고 배포한다.
