...
- **한섭에 이미 나온 오퍼의 스킬·재능·모듈 설명** — 게임 데이터에 공식 한국어가 있고
  **그쪽이 여기 번역문보다 정확합니다.** 게임에서 직접 얻으세요. `op-fut.json`에 든 것은
  **아직 한섭에 오지 않은** 오퍼뿐입니다.
- **아이템·적·작전 정보** — 위와 같은 이유로 번역 대상이 아니었습니다 (`op-past.json`에
  아이템이 34건 섞여 있지만 그때그때 남은 것이라 온전한 목록이 아닙니다). 여기 있는
  적·작전은 통합전략 것(`is*.json`의 `battles`)뿐입니다.
