...
화면 밖으로 나가면 다시 떨어진다. **320px 넘게 떨어지면** 앞으로 철퍼덕 3단계 — 자세가
무너져야 넘어짐으로 읽힌다(직립 회전만은 "문짝"으로 보임, 사용자 지적 ×2): fall(move 클립이
재생되는 채로 발끝 축 92° 고꾸라짐 0.35s) → sprawl(웅크린 sit 클립을 88° 눕혀 구겨진 더미,
착지 스쿼시+💫, 0.95s) → kneel(회전 복귀 = 무릎 꿇기 0.26s) → situp 전환으로 일어나
Interact + 💢(짜증) → relax. **등반**: 배회 틱의 30%로 머리 위 30~190px에서 그려진 요소의
