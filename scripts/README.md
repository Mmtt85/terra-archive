...
## 5.6 화면 인식 스캐너 아바타 템플릿

```bash
python3 scripts/build-avatar-templates.py   # → app/data/avatar-templates.json
```

에뮬레이터 화면 연동(인프라 플래너 '에뮬레이터 연동' 버튼)의 오퍼 인식이 쓰는 아바타
템플릿 인덱스. `public/avatars/<id>.webp`를 중앙 크롭(투명 가장자리 제외) → 그레이 24×24로
줄여 ZNCC(정규화 상관) 매칭용 벡터로 만든다. 런타임(app/scanner-core.ts)이 캡처 프레임의
카드 초상화를 로컬 서치로 미세정렬해 이 템플릿과 매칭한다(크롭 어긋남·저해상도에 강건).
⚠ scanner-core.ts와 그레이 L601·중앙 크롭 비율·크기가 **동일해야** 매칭된다. operators.json
아바타가 바뀌면 재실행. (avatars가 안 바뀌면 재생성 불필요.)
