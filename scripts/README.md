...
한섭 점검은 두 종류다 (사용자 확정 2026-08-13). **개별 스크립트를 손으로 꿰지 말고
점검 종류에 맞는 스킬을 부르면 아래 절차가 순서대로 엮여 있다.**

| | 큰 점검 (10:00~16:00) | 작은 점검 (16:00~16:10) |
|---|---|---|
| 성격 | 몇 달치 데이터가 통째로 클라에 들어온다 | 들어와 있던 걸 하나씩 해금만 한다 |
...
python3 scripts/whatsnew-gamedata.py [--since <sha>] [--no-rogue] [--future-only]
```

`whatsnew-gamedata.py`는 "몇 달치 미래시가 들어왔다"는 소문을 확인/반증하는 용도이기도 하다 —
2026-08-13 큰 점검은 전수 조사 결과 미래 일정 **0건**이었고 실제 변경은 이벤트 1·통합전략
확장팩 1·스킨 4종뿐이었다.
...
#     이벤트마다 에셋 번들 이름이 제각각이고([uc]guntask · [uc]informant · deepsea · siracusamap …)
#     접두어도 다르다 (마리아 니어 신문 사진은 파일이 act13side_ 로 시작한다). 새 이벤트는
#     ArknightsAssets2 트리를 받아 이름으로 대조해 짝을 더한다. --no-icons로 생략.
#     ⚠ 원본이 없는 그림은 데이터에 남기지 않는다 — have()가 받아 둔 파일만 참조하게 거른다
...
`app/data/rogue*.json`뿐이라 게임데이터를 안 읽는다.
...
열어 봐도 뭐가 든 파일인지 알 수 없어졌다. 다시 조각내지 말 것.

받는 쪽은 `manifest.json` 하나만 주소로 박는다. 나머지 파일 주소는 `files[].url`에
**해시를 `?v=`로 박아** 들어 있다 — 내용이 바뀌면 주소가 바뀌므로 중간 캐시·CDN 엣지가
옛 바이트를 돌려줄 창이 닫힌다 (`app/rogue.tsx`의 `?v=2`와 같은 처방). R2 워커는 쿼리
파라미터를 무시하고 같은 오브젝트를 준다. **형식 버전 필드는 두지 않는다** — 깨는 변경이
필요하면 숫자를 올리지 말고 주소를 새로 판다(`assets/tl2/`). 옛 앱이 옛 주소에서 계속
돌아가는 편이, 검사해 줄지도 모르는 숫자보다 낫다.
...
`npm run build`의 **첫 단계**다 — 실패하면 빌드가 멈춘다.

잡는 것: 단축 속성에 CSS 전역 키워드를 섞은 선언 (`font: 700 11px/1 inherit` 같은 것).
`inherit`·`initial`·`unset`·`revert`는 **값 전체**로만 쓸 수 있고 구성요소 자리에는 못 온다.
브라우저는 에러 대신 그 선언을 통째로 버리므로 빌드는 통과하고 글자만 기본값(16px)으로 남는다.

왜 검사로 막나: `font: inherit`은 버튼 리셋의 정석이라 globals.css에 30곳 넘게 정당하게
쓰인다. 그래서 "이 버튼 글자만 줄이자"며 그 줄에 크기를 얹는 수정이 자연스러워 보이는데,
그 순간 선언이 죽는다. **조용히 실패**해서 리뷰도 통과한다 — 주석으로 두 번 경고했는데도
한 달 동안 재발해서(사용자 지적 2026-08-23) 검사로 바꿨다. 고치는 법은 롱핸드로 나눠 쓰기
(`font-size` / `font-weight` / `line-height`).

## 구조화 데이터 검사 (`check-jsonld.mjs`, 2026-08-26~)

`npm run build`의 **마지막 단계**다 (`fix-html-lang.mjs` 뒤 — 그때가 최종 HTML이다).
실패하면 빌드가 멈춘다. 정상 출력은 이렇다:

```
check-jsonld: 8,517개 페이지 · 블록 8,517개 · 오류 0 — BreadcrumbList 8,514 · Article 8,475 · WebSite 42 · WebApplication 15
```

잡는 것: ① JSON 파싱 실패 ② 이스케이프 안 된 `<` ③ `@context`가 schema.org가 아님
④ `@type` 없음 ⑤ `BreadcrumbList`의 position 어긋남·name/item 누락 ⑥ `Article` headline 누락
⑦ url·item·`@id`·image가 상대 경로 ⑧ image를 뺀 URL이 정본 도메인이 아님(pages.dev 유출).
페이지 수가 8,000 미만이면 출력 구조가 바뀐 것으로 보고 중단한다(`fix-html-lang`의 0개 가드와 같은 취지).

왜 검사로 막나: 8,500여 장 전부가 JSON-LD를 싣는데 **깨져도 알려주는 사람이 없었다** —
빌드는 통과하고, 화면에도 안 보이고, 검색엔진만 조용히 무시한다. 게다가 내용이 게임
데이터에서 조립되므로 **코드를 안 고쳐도 클뜯 텍스트가 바뀌는 것만으로** 깨질 수 있다.
잘못된 구조화 데이터는 단순 오류가 아니라 구글 스팸 정책(마크업과 콘텐츠 불일치) 대상이라
리치 결과 자격을 통째로 잃을 수 있어서, 틀린 채로 나가느니 실패가 낫다.

⚠ **검사기도 음성 테스트를 거쳐야 한다.** 첫 판은 URL 검사가 최상위 노드만 훑어
