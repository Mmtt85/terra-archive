...
# 이 규칙 덕분에 '침묵' 태그(2026-08-08 추가)도 이름 오탐 없이 붙는다 — 아래 tag() 참조.
def units(o):
...
POTS = ["어비설팟", "쉐이팟", "쉐라그팟", "카시미어팟", "미노스팟", "아베무팟", "소각팟",
        "라테라노팟", "탄약팟", "라인랩팟", "라이오스 파티"]
# 판정: 앵커 진영 소속이거나, 전투 텍스트(스킬·재능·특성·모듈)에 시너지 언급이 있을 때.
# 출신지만으로 붙이지 않는다 (예: 에기르 ≠ 어비설팟 — terra-archive 도메인 규칙).
POT_BY_FACTION = {
    "어비설팟": {"어비설 헌터스"},
    "쉐이팟": {"염-쉐이"},
    "쉐라그팟": {"쉐라그", "카란 무역회사"},
...
TAG_ORDER = POTS + ["공격 회복", "피격 회복", "아군 치유", "자가 회복", "SP 배터리", "기절", "수면",
             "냉기·빙결", "감속·정지", "속박", "공포", "침묵", "취약", "방어력 감소", "마법 저항 감소",
...
def tag(o):
    ...
    # 침묵 (사용자 요청 2026-08-08: "라플란드 같은애들이 침묵 가지고 있음").
    # ⚠ KR 데이터는 '침묵'이라는 낱말을 **한 번도 쓰지 않는다** — 실제 표기는
    # "특수 능력 무효화"다(라플란드 재능 '정신 파괴': "공격 시 적의 특수 능력 무효화").
    # '침묵'은 커뮤니티 용어라 태그 이름으로만 쓰고, 판정은 게임 문구로 한다.
    # 사이에 시간이 끼는 변형까지 받는다("특수 능력을 5초간 무효화" 코로세럼,
    # "특수 능력 7초간 무효화" 스카이박스). 대미지·은신 무효화는 걸리지 않는다.
    if re.search(r"특수 능력[^.]{0,10}무효화", T): tags.add("침묵")
