...
def tag(o):
    ...
    # 반사(가시): 피격 시 공격자/목표에게 대미지를 되돌리는 메커니즘 (무에나 "반사", 페넌스·테킬라
    # "피격될 때마다 …에게 대미지"). 스킬명 "반격"(듀나 방패 반격 등 능동 역공격)은 제외한다
    if re.search(r"대미지[^.]{0,6}반사|반사[^.]{0,10}대미지", T) or \
       re.search(r"피격(?:당)?(?: 시| 시마다|될 때마다|할 때마다|하면)[^.]{0,70}?(?:상대|목표|적|공격자)[^.]{0,30}?대미지[^.]{0,16}?(?:입힌|입힘|가한|가함)", T):
        tags.add("반사")
