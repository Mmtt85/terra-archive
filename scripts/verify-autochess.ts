// 위수 협의 PRTS 시뮬레이션 회귀 하네스 — 네 코어를 한 번에 돈다 (2026-09-07 재가동).
//
...
//   npx tsx scripts/verify-ac/<grid|face|text|band>.ts   ← 하나만
//
...
async function main() {
  let pass = 0, fail = 0;
  for (const [label, run] of [["grid", grid], ["face", face], ["band", band], ["text", text]] as const) {
    const t0 = Date.now();
    try {
      const r = await run();
