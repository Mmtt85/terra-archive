...
- `app/data/autochess.json` / `.en` / `.ja` — 위수 협의(오토체스) 가이드 (build-autochess.py,
  ...
  ⚠ **같은 오퍼가 두 기물로 실린 8건**(레코드키퍼·가비알 디 인빈서블·틴맨·쉐라·프틸롭시스·
  시빌라이트 에테르나·와파린·님프)은 맹약도 능력도 글자까지 같고 티어만 다르다. 한쪽에
  `isHidden`이 붙어 있으며 그쪽이 지난 시즌 잔재다 — **짝이 있을 때만** 숨김 쪽을 버린다
  (사용자 제보 2026-08-23: "레코드키퍼는 T5고 가비알 디 인빈서블은 T4임").
  ⚠ 짝 없이 혼자 `isHidden`인 9건(비즈왁스 T4·비그나·어스스피릿·클리프하트·아코르트·인포서·
  샤마르·로즈솔트·맹약 서포터)은 **그대로 싣는다** — isHidden은 '상점 진열에서 뺀다'는 뜻이지
  없는 기물이 아니다(전부 특수 지급 PRESET). 숨김을 통째로 거르면 이 9명이 사라진다.
