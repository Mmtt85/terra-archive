...
3. **분류 힌트** — "쉐이록라" = 이름(쉐이) + 분류어(록라). 내장 사전(`HINT_TOKENS`: 록라/로라/
...
4. **오탈자 근사** — 한글을 **자모로 분해**해 2-그램 Dice 유사도 ≥0.55면 후보로 올린다
   ('첸 더 더스트릭'→첸 더 던스트릭, '네스티'→내스티. 둘 다 자모 한 개 차이라 음절 단위론 못 잡는다).
   1~3단의 최고점이 72 미만일 때만 돌고, 점수는 30~85라 완전일치(120)를 절대 못 넘는다.
   목록에 '비슷한 이름' 배지가 붙는다. 자모 4개(2음절) 미만 검색어에는 돌리지 않는다(잡음).

**색인 대상**: 오퍼(이름·다국어·별칭·커뮤니티 별명 3표↑·코드·이격 별칭) → `#op-` 모달 /
...
**선택 학습 — 전역(사람들) 우선** (`app/omni-picks.ts` + `docs/supabase-omni-picks.sql`):
되묻기에서 무엇을 골랐는지 모아 같은 검색어의 1순위를 정한다. 사용자 확정 2026-07-25:
**"자기 자신한테만 적용되면 안 된다 — 모든 사람 데이터를 수집해 가중치를 둔다."**

- 원장 `omni_pick` = 클릭 1행. 컬럼: `q`(정규화 검색어, `~토큰`이면 은어 힌트 행) ·
  `uid`(고른 항목 id / `hint:<종류>`) · kind·name·locale · **session**(브라우저 익명 랜덤 id) ·
  rank(고른 후보의 순위) · candidates(후보 수) · fuzzy · hinted ·
  **source**(`pick`=직접 클릭 / `trail`=실패한 검색의 목적지 추론) · steps(추론까지 걸린 행동 수).
- 공개 집계 뷰 `omni_pick_counts(q, uid, picks, voters, trail_voters)` — **voters = 서로 다른
  사람 수**가 가중치의 정본이고(추론은 `trail_voters`로 분리해 절반만 인정),
  **2명 이상 합의한 조합만** 노출한다(한 사람이 전역 순위를 못 흔든다).
  `omni_query_top`은 인기 검색어·평균 순위(검색 품질 점검)용, 아직 사이트가 읽지는 않는다.
- 최종 가중 = `voters + 0.5×trail_voters + (내가 고른 적 있으면 2, 추론이면 1)`, 보너스 = `min(45, 18+9·log2(1+가중))`.
  → 내 기기에선 한 번만 골라도 32점이라 즉시 1순위(확신 문턱 30 초과), 남의 화면은 그대로.
  상한 45라 학습이 **비슷한 후보의 순서만** 바꾸고 관련 없는 항목을 끌어올리지 못한다.
- 표 3 이상이면 '자주 선택' 배지. 테이블 **미설치면 404를 한 번 확인한 뒤 서버 전송을 끊고**
  로컬 학습만 돈다(사이트 정상). 스팸은 /admin과 같은 `x-admin-key`로 삭제.

**실패한 검색의 최종 목적지 추적** (`app/trail.ts`, 사용자 요청 2026-07-25):
결과가 0건이던 검색어를 sessionStorage에 쌓아 두고, 그 사람이 **10번의 행동·10분 안에**
실제 컨텐츠에 도착하면 그 검색어에 이어 붙인다.

> "날시" 검색 → 아무것도 없음 → 검색창을 닫고 백과사전에서 "켈시" → Kal'tsit·Esperanta 클릭
> → **날시 = 그 오퍼**. "뱅제"→없음→"은재"→…→실버애쉬 더 레인프로스트 클릭도 같은 방식.

- **미스**를 남기는 곳: 유니버셜 서치(통합전략 자동 확장까지 끝난 뒤 0건일 때만)·오퍼
  백과사전·재료 파밍 검색. **행동 1회**로 세는 것: 탭 이동, 새 검색어.
- **도착**을 잡는 곳: `home.tsx`의 해시 감시(오퍼 `#op-`·스토리 `#story-`·통합전략
  `#rg-…~타입~id`) + 파밍 재료 상세 모달. uid 형식이 검색 색인과 같아 그대로 가중치가 된다.
- 직접 클릭(`source='pick'`)이 아니라 **추론(`source='trail'`)**이라 무게는 절반
  (내 기기 1표 / 사람들 0.5표). 추론이 쌓여도 완전일치를 뒤집지 못한다.
- **글자가 하나도 안 맞아도 결과에 넣는다** — "날시"는 어떤 이름과도 안 겹치지만 학습된
  별명이면 `LEARNED_BASE(55) + 표 보너스`로 후보에 올라간다(searchSmart의 주입 단계).
