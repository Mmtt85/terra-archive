...
|---|---|---|---|
| 밴 카드 격자·티어 | '사용 제한 오퍼레이터' 카드 우상단 **빨간 금지 표식** | `acvision.findAcRows` — 표식 질량으로 열 피치·위상, 열마다 표식 y-런으로 행, 배지는 색상(T3~T6)·회색 글리프 면적(T1/T2) | 온전한 행 87/88, 카드 티어 276/276, 오탐 0 |
| 밴 기물 정체 | 카드 얼굴 | `acface` — `skin/portrait/<op>_2` 를 s 0.85·dx 0.0625·dy −0.0625 로 카드에 놓은 템플릿 ↔ 10% 인셋 48px **HOG** 코사인. 행의 맹약은 OCR 없이 "행 카드들이 공유하는 맹약" 으로 풀고(`solveBanRow`), (맹약,티어) 후보 안에서 1위 | 후보 제한 1위 100% (294카드), 전체 121종 97.6% |
...
회귀 하네스: `npx tsx scripts/verify-autochess.ts` — 픽스처는 `fixtures/lens/ac-frames/v1|v2/*.png`(녹화 프레임, ffmpeg
