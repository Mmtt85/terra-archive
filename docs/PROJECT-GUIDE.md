...
### 위수 협의 PRTS 시뮬레이션 — 한 판을 화면에서 받아 적는다 (2026-09-06 · **폐기**)

> 🚫 **폐기됐다 (2026-09-06 사용자 확정 "이건 폐기하자").** 같은 날 제목 줄의
> `PRTS 시뮬레이션` 버튼을 지웠고, **다시 달지 않는다.** 인식 배선
> (`lens/acvision`·`acsolve`·`acmatch`, `lens/run.ts` 의 `autochess` 분기, `autochess-run.ts`,
> `scripts/verify-autochess.ts`)은 만들면서 같이 고친 것들이 섞여 있어 **일부러 남겨 뒀지만**,
> 들어가는 문이 없어 동작하지 않는다. 아래는 되살릴 일이 생겼을 때를 위한 기록이다.
...
제목 줄의 `PRTS 시뮬레이션` 버튼이 게임 창을 물려(록라와 같은 브리지) 대신 읽어 줬다.

| 무엇 | 어디서 | 어떻게 |
|---|---|---|
| 맹약 중첩 수 | 맹약 원형 안의 숫자 | 이름 줄을 OCR 로 찾고 **그 위**를 숫자 전용 워커로 (`acmatch.planAcStacks`) |
| 배치 가능 인원 | `n/8` · `n/9` | 원시 라인의 분수. **분모 9 = 인사부 파일** → 9번째 칸 해금 |
| 독립 / 연합 | 전략 정보 화면 참가자 카드 | `#1234` 번호를 센다 — 1명 독립, 2명 이상 연합 |
| 새 판 | '시뮬레이션 정보' 화면 | 보이면 지난 판 기록을 비운다 |
| 밴 목록 | 사용 제한 오퍼레이터 | 아래 참조 |
...
화면과 어긋난 계산이 나온다. 전략 드롭다운·노드 이동 토글도 그동안 사라진다.

#### 밴 목록은 **얼굴이 아니라 조합으로** 푼다

밴 화면은 이름 없이 얼굴만 나온다. 초상·아바타 템플릿 ZNCC 로 맞히려 했으나 후보 121종
중 **순위 2·25·9·31위**(컬러 ZNCC 로도 1위 적중 0/4)로 못 쓸 수준이었다. 대신 성질을 쓴다 —
**밴은 기물 단위이고, 밴된 기물은 자기가 속한 모든 맹약 행에 동시에 나타난다**
(시=염국+아케인 → 두 행 모두 T5). 행끼리 서로를 구속하므로 (맹약, 티어) 관측만으로
조합이 정해진다. `app/lens/acsolve.ts` 가 exact cover 로 푼다 (실측 1~2ms).

- 티어 배지는 **색**으로 갈린다 (실측 편차 ±0.3°):
  T6 H=31° · T5 H=46° · T4 H=189° · T3 H=161°, 무채색 T1/T2 는 흰 글리프 면적 (0.07 vs 0.30).
- 카드 격자는 밝기 투영 (`acvision.findAcCards`). **상수로 박으면 안 된다** — 전체화면
  스샷은 카드 179px, 창모드 에뮬레이터는 55px. 임계는 오츠로 화면마다 다시 잡는다.
- **잘린 행은 행 간격으로 걸러낸다.** 배지가 잘리면 티어가 전부 T1 로 읽혀 조용히 틀린
  밴이 나오는데, 카드 폭으로는 안 걸린다(잘린 행도 폭 중앙값 정상). 대신 위치가 어긋난다
  (265·264·265·265 뒤에 227). 스크롤 중엔 위아래가 늘 잘려 있어 이게 핵심 관문이다.
- 맹약 이름은 **아이콘 아래를 잘라 이진화 후 다시 읽는다** — 전체 프레임 PSM11 은 5행 중
  2행만 잡았다. `bondOfLine` 은 OCR 이 붙이는 숫자 잡음을 떼고 본다 ("아케인11").
- 해가 여럿이면 **교집합만 확정**으로 내고 나머지는 점선 후보 칩으로 — 틀린 밴을 사실처럼
  보여 주느니 모른다고 두는 편이 낫다. 화면을 끝까지 스크롤할수록 후보가 확정으로 옮겨 간다.
  그래서 UI 가 "끝까지 스크롤해 주세요"라고 안내한다 — **그 안내가 정확도의 일부다.**

회귀 하네스: `npx tsx scripts/verify-autochess.ts` (픽스처는 `fixtures/lens/screenshots/`,
git 미추적 로컬 전용 — 없으면 조용히 건너뛴다).
