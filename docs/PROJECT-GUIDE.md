...
### 헤더 만능검색 (2026-07-25, 사용자 요청)

헤더 1줄(햄버거 왼쪽)의 `⌕ 검색` 버튼 — **헤더를 접어도 남는다**. ⌘K/Ctrl+K, `/`로도 열린다.
단어를 넣고 **[바로가기]**(또는 ⏎)를 누르면 1위가 확실하면 곧장 이동하고, 애매하면
"이 중에 무엇인가요?" 선택지를 띄운다. 목록은 타이핑 중에도 실시간으로 보인다.

- **판정 로직**: `app/omni.ts` — 완전일치(120) > 접두(≤100×덮는비율) > 부분(≤60×비율),
  별명·코드는 −8. 1위가 유일한 완전일치이거나 2위와 30점 이상 벌어지면 즉시 이동,
  아니면 되묻기. `/rogue` 페이지에서는 **현재 테마를 사전확률로** 써서 테마마다 있는
  동명 유물(뱀파이어의 침대 등)의 5중 동점을 그 테마로 확정한다(렌즈의 CTX_BOOST와 같은 판단).
- **색인 대상**: 오퍼(이름·다국어·별칭·커뮤니티 별명 3표↑·코드) → `#op-` 모달 /
  스토리(요약·전문 있는 것만) → `#story-` / 재료(은어 별칭 포함) → 파밍 탭 상세 모달 /
  공채 태그 → 태그 선택 / 통합전략 테마 → `?topic=isN` / 기능 탭(동의어 riic·base·is3 …).
  **미래시 토글이 꺼져 있으면 미실장 오퍼·재료·CN 선행 테마는 색인에서 빠진다.**
- **통합전략 세부 항목**(유물·조우·작전·구역·분대·시스템)은 스샷 레이더 인덱스(2.9MB)를
  그대로 재사용하는데 무거워서 기본 색인엔 없다 — 가벼운 색인이 아무것도 못 찾았을 때
  자동으로, 또는 푸터의 `통합전략 세부 항목까지 찾기` 버튼으로 지연 로드한다(세션 1회).
- **선택 학습** (`app/omni-picks.ts`, 사용자 요청 2026-07-25): 되묻기에서 고른 항목을 기억해
  같은 검색어의 정확도를 올린다. ① **내 선택** = localStorage `ta-omni-picks`, 가중 3 →
  한 번만 골라도 다음엔 되묻지 않고 바로 이동(보너스 36 > 확신 문턱 30). ② **사람들의 선택**
  = Supabase `omni_pick`(1클릭 1행), 공개는 **2표 이상** 집계 뷰 `omni_pick_counts`만, 가중 1.
  보너스는 `min(45, 18+9·log2(1+표))`로 **상한이 있어** 학습이 비슷한 후보의 순서만 바꾸고
  관련 없는 항목을 끌어올리지 못한다(완전일치 120 > 부분일치 60+45). 표 3 이상은 '자주 선택' 배지.
  설치 SQL은 `docs/supabase-omni-picks.sql` — **미실행이면 404를 한 번 확인한 뒤 서버 전송을
  끊고 로컬 학습만** 돈다(사이트는 정상). 기록 값은 정규화된 검색어 + 항목 id뿐.
- **이동은 새 라우팅을 만들지 않고 기존 경로를 탄다**: 해시 딥링크(`#op-`/`#story-`),
  스샷 레이더 핸드오프(`sessionStorage["ta:lens-handoff"]` + `ta:rogue-topic` 이벤트),
  탭 내부 상태는 `app/handoff.ts` 우편함(`ta:omni-handoff` + 커스텀 이벤트 —
  파밍 재료 모달·공채 태그). 새 목표를 추가할 땐 `home.tsx`의 `runOmni`에 한 줄 붙인다.
- UI 문구는 `app/i18n.tsx` 사전(`// 헤더 만능검색` 블록)에 EN/JA가 함께 있다.

### 검색 입력 공통 규칙 — 1초 디바운스 (사용자 확정 2026-07-25)

**사이트의 모든 검색 입력은 타이핑이 멈춘 뒤 1초에만 검색한다.** 한 글자마다 목록을
다시 그리던 것이 입력 지연의 원인이었다 — 실측으로 계산 비용은 1~8ms인데 프레젠테이션
딜레이가 130~380ms였다(목록 재렌더·재페인트, 전체화면 `backdrop-filter`가 특히 나쁨).
정본은 `app/search.ts`의 `useDebounced(value, SEARCH_DEBOUNCE_MS=1000)`:

| 화면 | 상태 → 디바운스 값 |
|---|---|
| 헤더 만능검색 | `query` → `term` (자체 타이머 — [바로가기]·⏎는 **즉시** 검색) |
| 오퍼 백과사전 | `query` → `searchTerm` |
| 재료 파밍 / 육성 비용 계산기 | `query` → `searchTerm` · `draft` → `draftTerm` |
| 인프라 플래너 (보유 설정·후보 검색) | `query` → `searchTerm` · `benchQuery` → `benchTerm` |
| 통합전략 (맵·적·소장품) | `mapQ`/`enemyQ`/`relicQ` → `mapTerm`/`enemyTerm`/`relicTerm` |
| 스토리 검색 | `query` → `searchTerm` |
| 공채 태그 빠른 입력 | `quick` → `quickTerm` (태그 자동선택·조합 계산도 함께 지연) |
| 스샷 스캐너 (빠진 오퍼 추가) | `addQuery` → `addTerm` |

- 입력란에는 **raw 값**을 그대로 보여 준다(타이핑은 즉각 반응). 필터·목록만 지연된다.
- **지우기(빈 문자열)는 기다리지 않는다** — 초기화는 즉시 반영(`useDebounced` 특례).
- 새 검색 입력을 추가하면 이 훅을 쓸 것. 즉시 검색으로 되돌리지 말 것(사용자 확정 규칙).
