...
- 공개 집계 뷰 `omni_pick_counts(q, uid, picks, voters)` — **voters = 서로 다른 사람 수**가
  가중치의 정본이고, **2명 이상 합의한 조합만** 노출한다(한 사람이 전역 순위를 못 흔든다).
  `omni_query_top`은 인기 검색어·평균 순위(검색 품질 점검)용, 아직 사이트가 읽지는 않는다.
- 최종 가중 = `voters + (내가 고른 적 있으면 2)`, 보너스 = `min(45, 18+9·log2(1+가중))`.
