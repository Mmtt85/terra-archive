...
- ⚠ **발행된 요약의 사실 오류 제보가 오면 KO·EN·JA 세 파일을 그 자리에서 고친다.**
  `scripts/story-i18n-merge.py`는 **지금 돌리면 안 된다** — KO 원본과 번역본의 alias 배열이
  어긋나 있어 EN 24건·JA 39건이 한국어로 폴백된다(2026-07-29 확인, 스크립트 헤더에 상세).
  한 블록만 고칠 땐 `story-summaries.{en,ja}.json`의 그 블록만 직접 편집한다.
