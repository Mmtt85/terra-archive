...
- **공채 추가 지연 보충**: KR에 공채 풀 추가가 공지·적용됐으나 데이터마인 `recruitDetail`이
  아직 갱신 안 됐을 때는 `build-recruit.py`의 `RECRUIT_SUPPLEMENT`(이름 리스트)에 넣는다.
  성급은 character_table에서 자동 판정, 데이터마인이 따라잡으면 id로 중복 제거됨.
  (2026-07-12: 카넬리안·키라라·인디고를 나무위키 공개모집 문서 근거로 보충 — 데이터마인
  반영되면 리스트에서 빼도 됨.)
