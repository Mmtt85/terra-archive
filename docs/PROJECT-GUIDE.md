...
- **스킬 공격 범위**: 스킬이 범위를 바꾸면(`skill_table` levels의 `rangeId`, KR 228개) 스킬 카드에
  ...
  ⚠ 표시되는 범위가 본체가 아니라 **소환물** 것인 경우가 있다(메이어 '교란 장치', 도로시 '고속 공진
  제거' 등 14건) — 클뜯에 구분 플래그가 없어 **소환물이 같은 이름의 스킬을 갖고 있으면** 그쪽으로
  보고 "소환물 ○○의 범위"로 라벨을 바꾼다 (사용자 지적 2026-08-01).
- **소환물(토큰)**: `displayTokenDict`가 정본 (KR 48 + CN 선행 5). 스탯·범위·재능·스킬을 오퍼와
  ...
  소환물 스킬의 범위 격자는 소환물 카드가 아니라 **같은 이름의 본체 스킬 카드**에 붙는다.
