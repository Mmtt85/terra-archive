...
- **썸네일은 서버판별** (CN판은 중국어 부제가 박혀 있어 사용 금지 — 사용자 확정 2026-07):
  기본(ko·en) = `ArknightsAssets2` **en 브랜치**(글로벌판) → `/story/<id>.jpg`,
  ja = `555me/ArknightsAssets2` **jp 브랜치** → `/story/ja/<id>.jpg` (stories.json `thumbJa`,
  없으면 UI가 기본판 폴백). KR 서버판은 언팩 공개 레포가 없어 글로벌판으로 대체.
  경로: `assets/dyn/arts/ui/storyreview/hubs/activity/storyentrypic_*.png`.
