...
- **게시판 관리자 모드** (사용자 확정 2026-08-17: "admin페이지 말고 제안 모달에서 답변 +
  ...
  (.supabase-admin-key 내용)를 넣으면 잠금 해제: 전체 제안 목록 + 스레드 모달에서 답변
  등록·삭제. 키는 그 브라우저 localStorage(`ta-feedback-admin`)에 남아 x-admin-key 헤더로
  Supabase REST에 직접 실린다 (틀린 키는 열람 코드 형식 오류와 같은 문구 — 존재를 안
  드러냄). ⚠ 공개 사이트 localStorage에 키가 놓이므로 **관리자 본인 기기에서만** 쓸 것 —
  '관리자 해제'가 키를 지운다. /admin 페이지의 상태 관리(대응중·확인완료·삭제)는 종전대로.
