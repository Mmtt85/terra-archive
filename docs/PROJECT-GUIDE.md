...
- **보유 오퍼 설정**: 전원 표시하되 1~5성은 기본 보유 체크, 6성은 기본 미보유.
  ...
  **화면에서 보유 오퍼 스캔(app/scan/, v4 2026-07-23)**: 에뮬레이터 창을 getDisplayMedia로 공유해
  오퍼 목록 화면을 인식. 핵심 원리 = 카드 아트는 **장착 스킨의 초상(portrait) 에셋**이므로
  KR 전 초상(스킨 포함, `build-scan-templates.py` → app/scan/portrait-templates.json 2.4MB)과
  masked ZNCC로 대조해 식별하고, 정예화는 카드 엠블럼(E0=무표시/E1=✕/E2=날개)을 인-도메인
  템플릿으로 3-way 분류. 이름 OCR(tesseract, 상한 84%)은 제거됨. 픽스처 138셀 식별·정예화
  100% — 회귀는 `npx tsx scripts/verify-scan.ts` (fixtures/scanner/ 로컬 전용).
...
- **인물·용어 참조 레일**: summary JSON의 `chars`(스탠딩)·`terms`(용어)가, 읽는 중인
  문단에 이름/alias가 언급되면 오른쪽에 세로 중앙 정렬로 떠오른다. 인물이 오퍼레이터면
  `op: <char id>`로 오퍼 상세 모달과 연결. 상단 갤러리는 제거됨(레일로 일원화) — **스토리 본명과 오퍼명이 다르면 desc에
  오퍼명을 병기** (예: 크산토스 = 바르카리스, 레나 = 퍼퓨머 더 디스틸트).
