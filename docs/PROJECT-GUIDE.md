...
2. **조합 별칭(완전일치 전용, 키 앞에 `=`)** — 부분일치로 새면 잡음이라 정확히 쳤을 때만 맞는다.
   ...
   - **통합전략 테마 별명**: `TOPIC_NICKS`(팬텀·미즈키·사미·살카즈·쉐이·흑류수해 …) ×
...
- **스킬·재능 '이름'은 판정에 쓰지 않는다** (2026-07 사용자 규칙). `corpus()`는 특성·설명·모듈
  ...
  명칭이 실제 효과로 오인되던 문제 제거. 이름을 빼자 **실제 침묵 효과 오퍼가 0명 → "침묵" 컨셉
  자체 삭제**(TAG_ORDER·CONCEPT_I18N에서 제거).
...
- **오퍼 상세 모달**: 내용 순서 = 잠재→스탯→스킬→재능→특성→모듈→인프라→복장→프로필→보이스 대사.
  뒤 셋(복장·프로필·보이스)은 **모달을 열 때만 오퍼당 파일 1개를 받아오는 지연 로딩**이다
  (`public/{skins,profiles,voice}/<locale>/<id>.json`, R2 서빙). 목록 화면에 통째로 실리는
...
- **테라 연대기 (베타)**: `app/data/chronology.json` — 손수 큐레이트하는 스캐폴드.
  ...
  - UI: 스토리 탭 상단 "요약 ↔ 테라 연대기" 토글 → 한 줄 연혁 바 + 테마별/종류별 그룹 목록.
