...
- **효과음은 있고, 음악은 없다** (2026-08-25). 대본의 `[PlaySound]`·`[PlayMusic]` 지시를
  `build-story-scripts.py` 가 **`au` 트랙**으로 싣고(줄 번호 + 키 + 음량 + 크로스페이드),
  효과음만 `build-story-sfx.py` 가 구워 `public/story/sfx/<키>.m4a` 로 낸다 (1,260종 57MB).
  - **조달은 KR 공식 CDN** — GitHub 미러(ArknightsAssets2)의 `dyn/audio` 는 21개 파일뿐이라
    못 쓴다. CDN 매니페스트엔 음원 번들 2,619개가 있고 **대본 키가 파일 경로와 그대로 이어진다**
    (`$d_gen_walk_n` → `audio/sound_beta_2/avg/d_gen_walk_n`). 접두가 붙는 것이 있어 접미 일치로 찾는다.
    언팩은 `--kr-thumbs` 와 같은 UnityPy + lz4inv 경로, 인코딩은 macOS 내장 `afconvert` (AAC 96kbps
    — iOS 사파리까지 되는 무난한 선택). **macOS 전용 단계라 CI 에서 돌리지 않는다.**
  - ⚠ **BGM 은 재호스팅하지 않는다** (판단 근거를 남긴다). 명일방주 음악은 Monster Siren Records 가
    **별도 상품으로 파는 것**이라, 10시간치를 통째로 공개 스트리밍하면 그 상품을 대체한다 —
    스프라이트 섬네일과 대체성이 다르다. 저작권 표시를 붙여도 재배포가 적법해지지 않는다.
    `au` 트랙에 BGM 키는 남겨 두되(`bgmAt()`) **소리로 내지 않고**, 안내문이 "음악은 포함되어
    있지 않습니다"라고 밝힌다. 곡명 표시도 포기했다 — `audio_data.json` 의 `bgmBanks`×`musics`
    로 이어 봐도 대본 키 258종 중 이름이 나오는 건 28종(11%)뿐이다 (스토리 BGM 은 뱅크를
    거치지 않고 파일 경로로 직접 재생된다).
  - **재생은 장면 모드에서만** (`app/story-audio.ts` 의 `useStorySfx`). 기본 **꺼짐** —
    브라우저 자동재생 정책상 제스처 전에는 `play()` 가 거부되므로, 무대 위 🔊 버튼을 누르는
    그 클릭이 첫 제스처가 된다. 설정은 `localStorage: ta-story-sfx`.
  - ⚠ **KR 스크립트를 재생성하면 `au` 트랙도 다시 나온다** — 효과음 키가 늘면
    `build-story-sfx.py` 를 다시 돌려야 새 클립이 생긴다 (있는 것은 건너뛴다).
