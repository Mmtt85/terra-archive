...
|---|---|---|---|
| 밴 화면 여부 | 우하단 teal '준비 완료' 버튼의 **자리·크기** | `acvision.isAcBanScreen` — `findAcRows` 가 이미 낸 `button` 을 `x∈[0.72,0.84] ∧ w∈[0.14,0.21]` 로 본다 | 정보 42/42 · 그 외 0/75 (117프레임) · 0.18ms |
...
| 밴 기물 정체 | 카드 얼굴 | `acface` — `skin/portrait/<op>_2` 를 s 0.85·dx 0.0625·dy −0.0625 로 카드에 놓은 템플릿 ↔ 13% 인셋 48px **HOG** 코사인. 행의 맹약은 위 아이콘 매칭이 주므로 `solveBanRow(…, hint)` 로 넘겨 (맹약,티어) 후보 안에서 1위를 뽑는다 | 후보 제한 1위 100% (294카드), 전체 121종 97.6% |
...
#### 밴 인식은 **밴 화면에서만** 돈다 — 게이트 세 겹 (2026-09-07 오후)
...
게이트는 값이 싼 순서로 세 겹이고, 셋 다 117프레임에서 오탐 0 이다 (하나가 흔들려도 나머지가 남는다):

| # | 무엇 | 비용 | 오탐 행 | 잃는 온전한 행 |
|---|---|---|---|---|
| ① | `isAcBanScreen` — teal '준비 완료' 버튼 창 | 0.18ms (격자가 이미 계산) | 22프레임 → 0 | 0 |
| ② | `acBanRows` — col 0 부터 연속·2장 이상·행 안 red 폭 ≤0.10·red ≤0.55 | 0 | 49행 → 0 | 0 |
| ③ | `acbond.matchBondIcon` — 맹약 아이콘 23종 ZNCC ≥0.62 | 행당 50ms (①②를 통과한 행만) | 149행 → 0 | 0 |

- ②의 주력은 **열 연속**이다 (오탐 49→5). 진짜 밴 행은 col 0 부터 빈칸 없이 이어지는데 오탐은 `c1 c2 c5 c6`
  처럼 끊긴다. '2장 이상' 이 한 장짜리 허깃 행을 쓸어내고(5→1), red 폭이 마지막 하나를 죽인다.
  ⚠ 밴 기물이 1명뿐인 맹약 행을 버리지 않도록, **같은 프레임에 2장 이상 행이 있으면 한 장 행도 받는다.**
- ⚠ **프레임 단위 하한(행 ≥2 · 이웃 행 간격 규칙성)은 쓰지 않는다.** 스크롤이 막 시작돼 한 행만 보이는
  순간을 통째로 버리고(v1/f015·f016 손실), 원본 해상도에서는 헤더 잡음 한 조각이 그 프레임의 진짜 행
  전부를 죽여 87→81 이 된다 — 위험이 비선형이다.
- ③은 게이트이면서 **행의 맹약 라벨**이다. 그래서 밴 리스트를 맹약별로 묶을 수 있다(아래). 양성 95행
  1순위 100%(점수 0.830~0.966), 음성 149행 최고 0.431 — 틈이 0.37 이라 문턱을 0.55~0.80 어디에 둬도 같다.
- **OCR 분류(`classifyAcScreen`)는 게이트로 쓰지 않는다.** 정확도는 같지만(info 42/42 · 오탐 0) 803ms 라
  밴 화면에서 OCR 을 건너뛰는 설계의 목적을 정면으로 버리고, `INFO_WORDS` 에 중국어가 없어 **CN 클라에서는
  통째로 닫힌다**. 대신 이미 OCR 이 돈 프레임에서는 확증으로 쓴다 — info 인데 ①이 거짓이면 `console.debug`
  로 남긴다 (미검증 상태를 실사용에서 잡는 공짜 계측).
- ⚠ ①의 미검증: '준비 완료' 를 **누른 뒤** 버튼 색이 바뀌는 상태 · 연합 다인 정보 화면 · 21:9 초광각 캡처.
