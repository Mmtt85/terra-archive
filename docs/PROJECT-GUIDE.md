...
- 만능검색은 **패널(omni-search.tsx)을 셸이 정적으로 싣고, 색인 엔진(omni.ts)만 지연 로드**한다 —
  트리거 호버·포커스·터치 때 미리, 늦어도 열 때. 패널째 `lazy()`로 되돌리면 첫 클릭에 청크 8개를
  기다리느라 패널이 한 박자 늦게 뜬다 (라이브 실측 380ms, 사용자 지적 2026-09-23).
...
⚠ **세 서버를 다 받아야 한다. 그리고 서버마다 리소스 버전이 따로 논다.**
2026-09-02 실측: kr `26-08-31-10-00-49` · jp `26-08-28-10-07-03` · en `26-08-17-16-42-26`
(클라 버전은 셋 다 36.7.22인데도 그렇다). 한섭만 새로 받으면 EN/JA가 낡은 데이터로 만들어져
**번역이 실제로 있는데도 한국어로 폴백한다** — 그날 일섭엔 신규 전략 4종이 이미 들어와 있었는데
한섭만 받는 바람에 일본어가 한국어로 나간 채 배포됐다.
스키마도 서버별로 따로 둔다(`scripts/fbs/<서버>/`) — `fbs-repair.py --server jp` 처럼 지정할 것.

⚠ CDN에서 못 뜯어 레포로 폴백하면 스크립트가 **"예정에 없던 레포 폴백"이라고 경고한다.**
`range_table` 같은 예정된 폴백과 구분되니, 경고가 뜨면 반드시 `fbs-repair.py`로 고치고 다시 받는다.
그냥 두면 그 표만 며칠 낡은 데이터로 사이트가 만들어지는데, 폴백이 성공이라 눈에 안 띈다.
...
- `app/data/autochess-s1.json` / `.en` / `.ja` + `autochess-seasons.json` — **지난 시즌**
  (2026-09-05, 사용자 요청 "예전 맹약 어땠는지 궁금해하는 사람들도 많더라"). 최신 시즌은
  파일명이 그대로고 지난 시즌만 `-s<N>`이 붙는다 — 새 시즌이 오면 옛 최신본이 그 자리로
  내려간다. 시즌 번호는 `autoChessData.versionInfoDict`가 정본이라 손으로 안 적는다
  (`scripts/acseason.py`). ⚠ **시즌1↔2는 같은 id인데 수치가 갈아엎어졌다** (밴드 29/29 ·
  맹약 18/18 · 기물 195/200 변경, 시즌2 신규 맹약 5: 아케인·카시미어·시라쿠사·독행·궁극기) —
  섞어 참조하면 틀린 숫자가 나가므로 화면도 시즌을 갈아탈 때 `key`로 통째로 새로 마운트한다.
  새 시즌 대응 절차는 `.claude/skills/autochess-season`. 시즌 문서는 **정적 임포트**다 —
  `/autochess/s1`이 프리렌더 정적 페이지라 지연 로드로 두면 그 HTML에 최신 시즌이 박힌다.
  ...
  ⚠ **`isHidden` = 보급센터(상점) 미진열, 그 이상이 아니다.** 붙은 17개를 "같은 charId가
  다른 티어에 살아 있나"로 가르면 성격이 정반대로 갈린다:
  ① **짝이 있는 8건 = 옛 티어 잔재 → 버린다.** 티어가 옮겨지면서 옛 항목이 숨김으로 남고
     새 티어 항목이 그 티어 목록 끝에 덧붙는다(레코드키퍼 T4→T5, 가비알 디 인빈서블 T5→T4,
     틴맨·쉐라·프틸롭시스·시빌라이트 에테르나·와파린·님프). 남기면 같은 오퍼가 두 번,
     하나는 **틀린 티어**로 뜬다.
  ② **짝이 없는 9건 = 상점에만 안 뜨는 실존 기물 → 남긴다**(`off: 1` + `ways`).
     비그나·어스스피릿·맹약 서포터·클리프하트·아코르트·인포서·샤마르·비즈왁스·로즈솔트.
  ⚠ **2026-09-02 정정** — 종전엔 17개를 통째로 버렸다. 2026-08-24에 ②도 "게임에 아예 없다"고
  판단했으나(사용자 "비즈왁스는 아예 없는데?"), 그건 **상점에서 안 보인다**는 관찰이었다.
  실사용 제보 2026-09-02: 퍼퓨머(조력·협동방어)에 〈의태 물질〉을 끼워 **로즈솔트**(협동방어)가
  나왔고 다른 사람은 **클리프하트**도 봤다 — 둘 다 ②다. 통째로 버리면 맹약 소속이 11곳에서
  모자란다(조력 8→11, 불굴 7→9, 협동방어 5→7 …). **시너지 판정에 쓰는 숫자라 표시만의 문제가
  아니다.** (옛 근거였던 "진영 맹약이 10명으로 떨어진다"는 우연이다 — ②에 진영 소속이 적을 뿐)
  ⚠ 그러므로 `bondInfoDict.chessIdList`가 숨김 기물을 품은 것은 찌꺼기가 아니라 **뽑기 풀
  그 자체**다. 맹약 소속 목록에서도 ②는 빼지 않는다 (①만 뺀다).
  **획득 경로(`ways`) 판정**: 효과 문구에 '보급센터'가 없고 맹약/풀에서 곧장 주는 것만 후보다.
  확정 2종은 〈의태 물질〉·〈간이 통신기〉(둘 다 착용자와 같은 맹약), 〈우등생〉은 맹약 서포터
  고정 지급. 진영 지원 8종(`allybuff_select_7_*`)은 조건이 맞지만 참조 컨테이너를 못 찾아
  `maybe: 1`로 내보내고 화면이 '(가능성)'으로 낮춰 쓴다. 보급센터를 거치는 경로(호출 모듈·
  긴급 차출권·비콘·상업 포장 계획·헤드헌터·구류의 인연·집단 행동·클리어 보상)는 제외 —
  진열이 막혀 있어 ②가 나올 수 없다.
  ⚠ `chessType`(PRESET/NORMAL)은 **상점 진열 여부가 아니다** — 둘 다 뜬다. 실제 축은 능력치
  출처다: PRESET은 모드가 고정 지급(charId==backupCharId, 4★15·5★38·6★6), NORMAL은 내 계정의
  ★6를 쓰되 미보유면 `backupCharId`의 예비 오퍼로 대체(전원 ★6). 라벨도 2026-09-02에
  '기본 지급'·'보유 시 본인 출전'으로 고쳤다 (옛 '특수 지급'/'상점 등장'은 거꾸로 읽혔다).
...
- `app/data/autochess-routes.json` (+ 지난 시즌 `autochess-s<N>-routes.json`) — 위수 협의
  전투 맵 (`scripts/build-autochess-routes.py`, 로케일 무관 45KB).
  **두 가지를 따로 싣는다** — 2026-08-30에 이 둘을 혼동해 두 번 갈아엎었다:
  · `maps` = 진짜 전장의 **지형**. `stageDatasDict` → `stage_table[stageId].levelId` 다
    (시즌2는 8장 = act1autochess_m01~m04 + act2autochess_m01~m04. act2 m03·m04는 KR 2단계
    2026-09-02에 열렸고, act1 m05~m07은 시즌2에서 weight 0. 시즌1은 m01~m07 7장 전부).
  ...
  · `rounds` = 라운드별 **경로와 등장 타이밍**. `battleDataDict` 의 levelId.
    ...
    ⚠ `still` = 첫 꼭짓점에서 **사실상 영원히 대기**하는 제자리 경로 (사미의 의지 등
    보스 5종이 cw `[[0, 999, 0]]` = 999초 고정 대기다). 데이터엔 갈 길이 적혀 있지만
    전투 내내 안 간다 — 화면은 출현 지점에 점으로 세우고 카드에 '제자리'를 단다.
...
### 관련 오퍼레이터 — 네 갈래 (2026-09-18 전수조사로 확정)

상세 맨 아래 `RELATED / 12`. 상세끼리 서로 링크해 크롤 깊이를 늘리려고 넣었다(2026-08-06).
순서는 **다른 버전 → 콜라보 → 진영(전부) → 세부 직군**, 갈래마다 최대 10명.
실측 페이지당 링크 평균 15개·최대 31개, 줄은 최대 4줄.

1. **다른 버전** — 같은 인물의 알트. 34묶음 69명.
   판정은 id 코드(`char_<번호>_<코드>`의 끝 숫자를 뗀다: `catap2` → `catap`)로 먼저 묶고,
   어긋나는 짝은 **영문 이름**으로 잇는다 — 알트의 영문 이름은 원본 이름을 통째로 앞이나
   뒤에 달고 있다 (`Kirin R Yato` ← `Yato`, `Ch'en the Dawnstreak` ← `Ch'en`).
   ⚠ **화면에 보이는 이름으로 맞추면 안 된다.** 일본어는 띄어쓰기가 없어 경계를 못 잡고
   `グレイ ← レイ` · `プリン ← リン` 같은 오탐이 쏟아진다 (3개 언어 실측). 언어판마다
   `name`과 `aliases`가 맞바뀌지만 **영문 표기는 세 판 모두에 있어** 이것만 언어 무관하다.
   id 코드만으로도 안 된다 — 키린R 야토는 `yato2`인데 원본 야토는 `nblade`다.
2. **콜라보** — 전용 진영이 있는 콜라보(`S.E.E.S.`·`Ave Mujica`·`라이오스 파티`)는 3번이
   알아서 묶는다. 없는 쪽은 **원본 오퍼의 진영을 물려받아** 서로 남남이 되므로
   (몬헌 6명이 작전팀 A4조와 예비작전팀 A6조로 찢어져 있었다) 별칭의 오퍼레이터 넘버
   접두로 따로 묶는다 — `COLLAB_CODES` 화이트리스트. **접두를 전면 적용하지 말 것**:
   `PA`(예비작전팀)·`YD`(염)처럼 대부분은 그냥 소속 코드고 이미 진영으로 묶여 있다.
3. **진영은 전부** 돈다. 종전엔 `factions[0]` 하나뿐이라 진영이 2개 이상인 111명(25%)의
   둘째 진영이 통째로 묻혔다 (컬럼비아 23명·염-용문 14명…).
   단 **로도스 아일랜드는 뺀다**(`GENERIC_FACTIONS`) — 사실상 전원 소속이라 59명에게
   똑같은 ★6 열 명이 뜨는 줄이 된다.
4. **미실장도 빼지 않는다** — 2026-09-04 규칙대로 흑백(`.fut-dim`)으로 보여준다.
   종전엔 걸러 내서, 소속이 전원 미실장인 진영(페르소나3 `S.E.E.S.` 넷)은 줄이 통째로
   사라졌다. 정렬만 실장 먼저로 둬 미실장 ★6이 앞자리를 먹지 않게 한다.
...
- **데이터는 스크립트 JSON 안의 `vn` 트랙 하나뿐** — `build-story-scripts.py`가 파싱하며
  ...
  **무대가 바뀐 줄에만** 스냅샷을 남긴다. 화면은 줄마다 직전 스냅샷을 펴서 그릴 뿐
  상태 기계가 없다.
- **`[charslot]` 읽는 법** (사용자 제보 2026-09-23 "천둥 속의 고요 — 스탠딩이 사라져야 할 때
  안 사라진다", `charslot()`):
  - 괄호 없는 `[charslot]`, 자리 없이 시간만 준 `[charslot(duration=1)]` = **전부 내리기**.
    종전 정규식은 괄호 있는 형태만 봐서 이걸 흘렸고, KR 대사 줄의 30%(111,215줄)에 앞 장면
    인물이 남아 있었다 — `[Image]` 컷씬 내리기(2026-08-29)와 같은 함정이다.
  - 이름 없이 자리만 짚은 것: 투명도를 내리면(`ato<0.5`) 그 자리를 비우고, 초점(`focus`)·
    이동(`posfrom/posto`)·흔들기(`action`)·확대(`poszoom/scale`)면 **인물을 그대로 둔다**.
    종전엔 무조건 비워서 말하는 사람을 비추기만 해도 인물이 사라졌다.
  - 자리 이름은 **첫 글자**로 읽는다 — 대본이 `left`/`right`/`middle` 도 쓴다(KR 6,686회).
  - charslot 무대는 **늘 세 자리**(좌 25%·중 50%·우 75%). 짚은 자리까지만 늘이면 가운데
    인물 혼자일 때 화면 2/3 지점에 선다. (`[Character]` 형은 이름 수만큼 벌려 세운다.)
  - 파서를 고쳤으면 스토리 3개 언어(`--lang en`·`--lang ja`)와 **기록(`build-records.py`)까지**
    다시 굽는다 — 기록도 같은 파서를 쓴다. 그다음 `build-story-vn.py`(+`--records`).
    ⚠ 인자 없이 도는 전체 재생성은 `story-script-ids.json` 을 **이번에 구운 것만으로 덮어써**
    중섭 번역본(main_17·오퍼 스토리 모음 등 21개, `--cn-merge` 산출)을 목록에서 뺀다 —
    목록은 `git checkout` 으로 되돌릴 것. 컷 그림(`public/story/cut`)도 다시 구워지니 되돌린다.
...
- ~~개발자 코멘트(dev_notes)~~ — **2026-09-05 기능째 제거** (사용자 지시: "개발자 코멘트도
  이제 필요 없을거 같음. 기능 자체를 지워버리자. 피드백 페이지에서도 없애고"). 게시판의
  공개 탭·`/admin` 편집 탭·`app/devnotes-api.ts`·`docs/supabase-devnotes.sql`·`#devnotes`
  딥링크·devnote-* CSS·i18n 사전을 모두 지웠다. 게시판은 이제 **탭 없이 '내 제안' 하나**.
  ⚠ **Supabase의 `dev_notes` 테이블과 그 안의 행은 지우지 않았다** — 사이트가 안 읽을 뿐이다.
  테이블이 남아 있는 동안은 `scripts/make-admin-rotate-sql.mjs`의 회전 목록에도 남겨 둔다
  (빼면 키 회전 때 옛 키가 그 테이블에 남는다). 정리하려면 Supabase에서 drop 하고 그 줄도 삭제.
- **목록 클릭 = 창모달** (사용자 지시 2026-08-17: "높이가 휙 늘어나지 말고 모달창 하나 더")
  — 제안 스레드는 인라인 확장 없이 두 번째 ModalWindow로 연다.
...
### 위수 협의 PRTS 시뮬레이션 — 한 판을 화면에서 받아 적는다 (2026-09-06 · 09-07 재가동)

> 2026-09-06 에 만들었다가 같은 날 접었고(버튼만 삭제), **2026-09-07 사용자가 실플레이 녹화 2편을 주며
> 재요청**("아무리 생각해도 포기하기 힘들다 … 원하는 사람들이 되게 많음")해 되살렸다. 그날 밴 카드가
> 아바타가 아니라 **스킨 초상의 윗부분**이라는 것을 찾아 얼굴 매칭이 풀렸다 (아래). 우선순위(사용자):
> ① 밴 목록 자동 인식 ② 다른 참가자의 전략 ③ 시뮬레이션 종류 ④ 배치·정비구역 기물과 맹약 중첩 동기화.

위수 협의는 **한 판에 필요한 정보가 화면에서 순식간에 사라진다.** 밴 목록은 시작 화면
25초(`enterStepList.INFO_CHECK`)뿐이고, 맹약 중첩은 라운드마다 바뀌며, 그 사이에 전략까지
골라야 해서 손으로 받아적을 틈이 없다. 그래서 **사람이 넣지 않고 읽는다** — `/autochess`
제목 줄의 `PRTS 시뮬레이션` 버튼이 게임 창을 물려(록라와 같은 브리지, `lens/bridge.ts`) 대신 읽어 준다.
프레임은 브리지가 **어두운 여백을 잘라 가로 900px JPEG** 로 넘기므로, 모든 인식은 그 해상도에서 실측했다
(원본 해상도가 이득인 항목은 없었다 — 상한을 올리지 않는다).

| 무엇 | 어디서 | 어떻게 (파일) | 실측 (2026-09-07, 900px) |
|---|---|---|---|
| 밴 행 여부 | 행의 **모양** — col 0 부터 빈칸 없이 연속 + 2장 이상 | `acvision.acBanRows` — 배율·선명도에 무관한 구조 조건만. 최종 확정은 아래 맹약 아이콘이 한다 | 밴 구간 밖 오탐 49행 → 1행 · 잃는 온전한 행 0 |
| 밴 카드 격자·티어 | '사용 제한 오퍼레이터' 카드 우상단 **빨간 금지 표식** | `acvision.findAcRows` — 표식 질량으로 열 피치·위상, 열마다 표식 y-런으로 행, 배지는 색상(T3~T6)·회색 글리프 면적(T1/T2). 받을 행은 `acBanRows` 가 고른다 (col 0 부터 연속·2장 이상·red 고름) | 온전한 행 87/89, 카드 티어 270/270, 오탐 0 |
| 밴 행의 맹약 | 행 왼쪽 원형 맹약 아이콘 | `acbond.matchBondIcon` — `public/ac/bond/<id>.webp` 23개를 **정사각 레터박스·inset 0** 으로 32px Sobel ZNCC, 글리프 변 0.47~0.53·d 3스케일 국소 탐색 | 1순위 100% (95행) · 밴 화면 밖 승인 0/149 · 행당 50ms |
| 밴 기물 정체 | 카드 얼굴 | `acface` — `skin/portrait/<op>_2` 를 s 0.85·dx 0.0625·dy −0.0625 로 카드에 놓은 템플릿 ↔ 13% 인셋 48px **HOG** 코사인. 행의 맹약을 `solveBanRow(…, hint)` 로 넘겨 **(맹약,티어) 후보만** 비교한다 (카드당 20.5 → 1.8명) | 후보 제한 1위 100% (294카드), 전체 121종 97.6% |
| 시뮬레이션 종류 | 로딩 화면 큰 글씨 / 정보·전략 화면 좌상단 붉은 배지 | `acmatch.parseAcMode` — 표준·험지·극한·초월·입문. 배지는 제목 줄 왼쪽을 **채널최댓값** 전처리로 잘라 읽는다(`ocr.maxCrop`). `AC-n` 코드·OPERATION 은 kor 모델이 못 읽어 안 쓴다 | 로딩 11/11 · 배지 51/51 |
| 내 전략 | '선택한 전략' 확정 화면 | `acmatch.parseAcBand` — 대표 오퍼 이름(`bands[].by`, 3자 이상만) 완전일치, 보조 `[전략 이름]`. '선택한전략·전략선택완료' 가 있어야 확정, 없으면 미리보기 | 확정 7/7, 탐색/확정 구분 9/9 |
| 상대 전략 (연합) | 참가자 줄의 전략 썸네일 — 줄은 **오른쪽 체크박스**로 찾는다 (`acband.participantRows`) | `acband.searchBand` — 전략 아이콘 Sobel(gx,gy) ZNCC 32px, 영역 3스케일 국소 탐색 | 타일 80/80 · 4인 화면 줄 수·순서·'나' 3/3 스크린샷 정확 일치(최소 점수 0.78) · 1인 화면 2/3 |
| 맹약 중첩 수 | 인게임 상단 맹약 원형 | `acmatch.findAcRings`(원둘레 밝기 차) → 링 아래 이름은 그레이 반전 크롭(`ocr.cropInverted`) → 링 안 숫자는 digits + O/Q/C→0 매핑 | 링 20/21 · 이름 34/35 · **숫자 0 은 10/35 로 약함, 1 이상은 표본 없음** |
| 남은 배치 칸 | HUD '배치 가능 인원:N' | `acmatch.parseDeployLeft` — 콜론 형식(분수 n/8 아님). **9 가 보이면 인사부 파일** → 9번째 칸 | 17/19 |
| 새 판 | '시뮬레이션 정보' 화면 | `acmatch.isAcInfoScreen` | 117/117 (오탐 0) |
| 목표 HP | HUD 상단 | `acmatch.parseAcHud` | 24/28 |
| 화면에서 본 기물 | 상점 카드·툴팁 이름 | `acmatch.matchPieceName` — chess/equips 정규화 완전일치, 4자 이상은 편집거리 1 | 정보용. 보드/정비구역 **확정 동기화는 아직 아님** (아래 '남은 것') |

⚠ **원 안의 숫자는 중첩 수다** (실측): 빅토리아가 툴팁 `중첩 수 0`·`👥 1/3`인데 원에는 0,
사르곤은 2였다. 인원수였다면 빅토리아가 1로 찍혔어야 한다.

⚠ **연결 중에는 편성이 잠긴다** (사용자 확정). 실제 판이 언제나 정답이라 손이 끼어들면
화면과 어긋난 계산이 나온다. 전략 드롭다운·노드 이동 토글도 그동안 사라진다. 읽은 전략은
`acrun.bands[seat 0]` 를 **파생값**으로 편성기에 넣고, 연결을 끊을 때 손 상태(`simBand`)로 옮겨 남긴다.

#### 밴 목록 — 얼굴을 **초상**으로 맞힌다 (지난번 실패의 원인)

2026-09-06 엔 `avatars/<op>.webp` 로 ZNCC 를 돌려 121종 중 순위 2·25·9·31위였다. 원인은 **템플릿이 틀렸던 것** —
밴 카드는 아바타가 아니라 `skin/portrait/<op>_2.webp`(180×360) 의 윗부분을 s≈0.85 로 확대해 살짝 왼쪽·아래로
민 그림이다 (50 카드로 정합, 편차 ±0.04). 같은 아바타 1:1 은 여전히 7.1%, 초상 템플릿 + 10% 인셋 HOG 는 97.6%.
초상이 없는 기물(`char_616_pithst`)만 아바타로 폴백한다.

- (맹약, 티어) 조합당 후보는 1~4명뿐이다 (1명 57조합 · 2명 52 · 3명 13 · 4명 1). 행의 맹약은 OCR 로 읽지 않는다 —
  900px 에서 맹약 이름 OCR 은 27~93%, 아이콘 템플릿은 79% 였다. 대신 **행의 카드들이 공유하는 맹약**을
  "각 카드의 (B,티어) 후보 중 최고 점수 합" 으로 고른다(`acface.solveBanRow`). 그러면 후보 제한 1위가 100%.
- 밴 표(`acrun.banVotes`)는 프레임마다 마진(1위−2위)을 표로 바꿔 더한다 — 마진 ≥ 0.12 면 1표(확정),
  후보 하나면 마진 1. 얼굴이 못 가른 자리만 `acsolve` 조합 풀이가 보태고, 해가 여럿이면 후보(점선)로.
- **잘린 행은 세지 않는다.** 위·아래로 잘리면 배지가 잘려 티어가 전부 T1 로 읽히므로 `cut` 행의 티어는 null.
  스크롤 중에는 늘 위아래가 잘려 있어 이게 오답을 막는 관문이다 — 그래서 UI 의 "끝까지 스크롤해 주세요"
  안내가 정확도의 일부다. 7장짜리 행은 7번째 카드가 아이콘 없이 다음 줄로 넘어간다 → `findAcRows` 가 위 행에 붙인다.
- 브라우저는 초상을 `asset()`(R2, CORS `*`)에서 받아 `acface-load.ts` 가 특징으로 만들어 캐시한다 — 티어 후보(≤ 20 op)만 받으므로 첫 밴 화면에서 수백 KB.

#### 밴 인식은 **밴 화면에서만** 돈다 — 게이트 두 겹 (2026-09-07 오후)

되살린 첫 판에는 구멍이 있었다. 격자가 카드 우상단의 **빨간 표식만** 보므로 전투 화면의 붉은 UI가 카드로
잡혔고, `run.ts` 가 그걸 밴 화면으로 단정해 (a) 없는 밴을 등록하고 (b) `screen="ban"` 이라 **OCR 을 통째로
건너뛰어** 그 프레임의 HP·맹약 링·남은 배치를 못 읽었다. 녹화 117프레임 실측 — 밴 구간 밖 **22프레임**에
'온전한 행' 이 나왔다 (전투 화면 9장은 전부). 사용자 신고: "밴 리스트 인식이 뭔 모든 상황에서 계속 되냐."
**원인은 하네스였다** — `verify-ac/grid.ts` 가 밴 프레임만 넣어 보고 다른 화면을 한 번도 안 쟀다.

게이트는 두 겹이고, **배율·선명도·창 비율에 무관한 것만** 조건으로 쓴다:

| # | 무엇 | 비용 | 오탐 행 (밴 구간 밖) | 잃는 온전한 행 |
|---|---|---|---|---|
| ② | `acBanRows` — col 0 부터 빈칸 없이 연속 + 카드 2장 이상 | 0 | 49행 → **1** | 0 |
| ③ | `acbond.matchBondIcon` — 맹약 아이콘 23종 ZNCC ≥0.62 | 행당 50ms (②를 통과한 행만) | 149행 → **0** | 0 |

- ②의 주력은 **열 연속**이다 (49→5). 진짜 밴 행은 col 0 부터 빈칸 없이 이어지는데 오탐은 `c1 c2 c5 c6`
  처럼 끊긴다. '2장 이상' 이 한 장짜리 허깃 행을 쓸어낸다(5→1). 남는 1행(v2/f001 메인 메뉴)은 ③이 죽인다.
  ⚠ 밴 기물이 1명뿐인 맹약 행을 버리지 않도록, **같은 프레임에 2장 이상 행이 있으면 한 장 행도 받는다.**
- ③은 게이트이면서 **행의 맹약 라벨**이다. 그래서 밴 리스트를 맹약별로 묶고(아래), 얼굴 후보도 그 맹약으로
  좁힌다. 양성 95행 1순위 100%(점수 0.830~0.966), 음성 149행 최고 0.431 — 틈 0.37 이라 문턱을 0.55~0.80
  어디에 둬도 결과가 같다.

**조건에서 뺀 것들** — 전부 "실측 오탐은 줄지만 실사용에서 진짜 행을 죽일 수 있는" 것이다. 비대칭이 결정적이다:
②가 진짜 행을 잘못 버리면 그 프레임만 잃고 다음 프레임이 잡지만(스토어가 프레임을 누적한다),
화면 단위 조건이 틀리면 **매 프레임 전부**를 잃는다.

- **`isAcBanScreen`(teal '준비 완료' 버튼 창)** — 정보 42/42 · 그 외 0/75 로 성적은 완벽했지만 셋 중
  유일하게 **캡처 창 모양에 의존**한다. 녹화 3종과 다른 창 비율·여백으로 캡처하면 버튼의 정규화 x 가
  대역을 벗어나 밴이 통째로 죽는다 (2026-09-07 오후 사용자 신고 "밴리스트가 전~혀 안나온다").
  지금은 진단 로그와, 아이콘 템플릿을 못 받았을 때의 폴백으로만 남겼다.
- **red 비율 조건(행 안 폭 ≤0.10 · 최대 ≤0.55)** — 오탐을 1→0 으로 만들지만 red 는 표식 선명도에 따라
  움직이는 값이고, 실측 표본이 전부 **재인코딩된 녹화 프레임**(mov→png→jpeg)이다. 라이브 캡처는
  표식이 더 선명해 red 가 높고 카드마다 더 흔들릴 수 있다.
- **프레임 단위 하한(행 ≥2 · 이웃 행 간격 규칙성)** — 스크롤이 막 시작돼 한 행만 보이는 순간을 통째로
  버리고(v1/f015·f016 손실), 원본 해상도에서는 헤더 잡음 한 조각이 그 프레임의 진짜 행 전부를 죽여
  87→81 이 된다 (위험이 비선형이다).
- **OCR 분류(`classifyAcScreen`)** — 정확도는 같지만(info 42/42 · 오탐 0) 803ms 라 밴 화면에서 OCR 을
  건너뛰는 설계의 목적을 정면으로 버리고, `INFO_WORDS` 에 중국어가 없어 **CN 클라에서는 통째로 닫힌다**.
  대신 이미 OCR 이 돈 프레임에서는 확증으로 쓴다 — info 인데 버튼 서명이 거짓이면 `console.debug` 로 남긴다.

⚠ 회귀 하네스에서 `grid.ts` 는 ②만 재므로 v2/f001 을 **알려진 오탐**으로 두고, ③이 그걸 죽인다는 사실은
`bond.ts` 가 같은 117프레임으로 따로 잰다. 둘을 합쳐야 0 이다 — 한쪽만 보고 판단하지 말 것.

#### 다 읽은 행을 버튼 때문에 버리지 않는다 — `cut` 을 둘로 갈랐다 (2026-09-07 오후)

사용자 신고: "시라쿠사 맹약 밴목록이 인식이 안됐네." 원인은 `cut` 이 성질이 다른 두 가지를 한 덩어리로
묶고 있었던 것이다. 5장 행이 화면 맨 아래에 오면 '준비 완료' 버튼 패널이 **6번째 열의 표식 자리**를
가리는데(cut ③), 그때 보이는 5장은 배지까지 멀쩡히 읽힌다. 그런데 행을 통째로 버렸다.

"다음 프레임에 위로 올라가면 잡힌다" 는 기대는 성립하지 않는다 — 브리지는 화면이 **300ms 잠잠할 때만**
프레임을 내보내므로(`bridge.ts` SETTLE_MS), 사용자가 그 자리에서만 멈췄다면 그 줄은 영영 못 본다.

그래서 `AcRow.readable` 로 갈랐다:

| cut 이유 | readable | tier | 소비자가 쓰는 것 |
|---|---|---|---|
| ① 위 잘림 · ② 배지 못 읽음 · ⑤ 줄바꿈 고아 | false | null | 아무것도 |
| ③ 다음 열이 버튼에 가림 · ④ 다음 줄이 화면 밖 | **true** | 남긴다 | **카드 단위 사실만** (얼굴로 기물 확정) |

- 카드 단위 사실(어느 기물이 밴됐나)은 못 본 카드가 있어도 **참이다** — 그래서 `bans` 에 넣는다.
- 그 행의 티어 목록은 **완전한 관측이 아니므로** `banObs` 에 넣지 않는다. 넣으면 조합 풀이(`acsolve`)가
  없는 카드를 있다고 믿어 엉뚱한 해를 낸다.
- 그런데 그러면 그룹 UI 가 그 맹약 줄을 통째로 잃는다(그룹을 `banObs` 키로 만들었으니까). 그래서
  스토어에 **`banSeen`(맹약 → 그 줄에서 본 카드 수, 최댓값)** 을 따로 두고, 그룹 목록과 "확정/본 카드"
  분모는 이 값을 쓴다. 조합 풀이만 `banObs` 를 쓴다.
- 효과 (실측): 밴 프레임 통과 31/33 → **32/33**, 통과 행 88 → **91** (하네스 실패 프레임 v1/f020 예견 ·
  v2/f016 기적이 정확히 이 경우였다).

#### 한 행 안에서 같은 기물을 두 번 배정하지 않는다

게임의 한 맹약 줄에 같은 오퍼레이터가 두 번 나올 수 없다. 그런데 카드마다 독립적으로 1위를 뽑으면
같은 (맹약,티어) 카드 두 장이 같은 기물을 고를 수 있고, 그러면 **한 기물을 통째로 잃는다** —
사용자 실플레이의 시라쿠사 행이 그 경우였다 (T1 카드 두 장 · (siracusaShip,T1) 후보는 프로방스·텍사스
정확히 2명). 점수 내림차순 그리디로 서로 다른 기물을 배정한다. 후보 수 = 카드 수면 배정이 **강제**되므로
얼굴 점수가 애매해도 정답이고, 그때 margin 은 1 로 둔다.
실측 정확도는 그대로(pick 99.7% · 힌트 경로 294자리 전부 동일)이고, 픽스처에 충돌 행이 없어
`verify-ac/face.ts` 가 **합성 검사**로 불변식을 못박는다 (같은 얼굴 두 장 → 서로 다른 기물).

#### 얼굴 후보는 (맹약, 티어)로 좁혀서 비교한다 (사용자 지시 2026-09-07)

행의 맹약을 아이콘으로 먼저 알아내므로, 카드 얼굴을 **그 맹약에 속한 기물과만** 비교한다
("사르곤에 속한 기물이랑 만 비교하는 식으로"). (맹약,티어) 조합당 후보는 1~4명뿐이다
(1명 57조합 · 2명 52 · 3명 13 · 4명 1) — 티어 전체를 다 재고 나중에 거르던 것과 **결과는 한 자리도
다르지 않고**(같은 부분집합의 argmax — 하네스가 294자리 전부 일치로 확인) 비교가 줄어든다:

- 카드당 비교 **20.5명 → 1.8명 (11.2배)**, 전체 6024 → 538회 (`verify-ac/face.ts` 실측)
- 후보가 1명인 조합(57/123)은 비교 없이 확정되고 마진이 1 이 된다
- 받아야 할 초상 템플릿도 그만큼만 — 예열이 아직 도는 중에도 첫 프레임을 곧바로 시작할 수 있다

#### 밴 리스트는 맹약별로 묶어 보여 준다 (사용자 요청 2026-09-07)

게임의 '사용 제한 오퍼레이터' 화면과 같은 모양이다 — 왼쪽에 맹약, 오른쪽에 그 줄의 카드.
정답 밴 목록 2판으로 게임 규칙을 실측해 맞췄다: 줄 순서 = `bonds` 배열 순(진영 먼저, 14행 중 어긋난 행 0),
기물은 자기 소속 맹약 줄에 **빠짐없이** 나오고(빠진 자리 0건), 줄 안은 티어 내림차순(14행 중 13행).
그래서 칩이 24~26% 늘어난다 (v1 19→24 · v2 21→26) — 접을 양은 아니다.

- ⚠ 그룹은 **화면에서 실제로 본 줄(`banSeen` 키)** 로만 만든다. 확정 기물의 소속 맹약을 보고 "그 줄도
  있겠지" 라고 지어내면 밴 하나가 틀렸을 때 없는 줄까지 만들어 낸다.
- 제목을 눌러 **접었다 펼 수 있다** (기본 펼침, 사용자 요청) — 맹약별로 묶으면 세로로 길어져 아래 전략·맹약을
  보려면 한참 스크롤해야 한다. 상태는 컴포넌트 안에만 둔다 (판마다 달라질 성질도, 링크에 실을 값도 아니다).
- 밴된 기물은 **세 겹으로** 표시한다 (사용자와 여러 번 주고받아 정착, 2026-09-07):
  ① 얼굴 우상단에 **금지 표지**(`.ac-banmark`) — 빨간 원 + 흰 사선 + 흰 테두리를 CSS 로 그린다.
    처음엔 8.5px 글자 `✕` 였는데 얼굴 아트 위에서 뭉개져 안 읽혔다("보기 영 이상함") — 도형은
    작아도 모양만으로 읽히고 밝은/어두운 아트 양쪽에서 윤곽이 산다.
  ② 이름에 **빨간 가로줄** (두께 2.5px — 기본 굵기는 글자에 묻힌다).
  ③ 줄·칸 배경을 옅게 물들인다 (`:has(.ac-banmark)`) — 15px 표식은 목록을 훑을 때 놓친다.
  붙는 곳: 기물 카드·기물 고르기 모달·맹약 상세 모달의 소속 오퍼·맹약 카드 얼굴 줄·배치/정비구역 칸·밴 칩.
  **확정(banSure)에만** 단다 — 후보에 달면 "못 쓰는 기물" 이라고 단정하는 셈이라 편성을 잘못 유도한다.
  PRTS 연결 중에만 보인다.
- 전략 목록(전략 탭)에는 지금 고른 전략에 표를 단다 — `내 전략` / `참가자 N` 칩 + 청록 테두리.
- 맹약에 밴된 기물이 있으면 **맹약 이름 바로 오른쪽**에 `밴 N명` 뱃지를 단다 (맹약 탭 카드 + 상세 모달
  제목 줄). ⚠ 카드의 얼굴 줄은 앞 14개만 보여 주므로 `b.chess` **전체**에서 센다 — 15번째 이후가
  밴이면 얼굴 표식이 아예 안 나온다.
- 런바(제목 줄 아래 **PRTS 박스**)에는 시뮬레이션 종류 · 독립/연합 · 현재 밴 목록 버튼 · 전략 칩 ·
  맨 오른쪽 **연결 끊기**만 둔다. '현재 밴 목록'은 창을 띄우지 않고 **이 박스 안에서 펼친다**
  (사용자 지시: 창을 띄우면 판을 가리고, 이 값은 판이 도는 동안 계속 대조하는 자리다).
  그래서 `.ac-runbar` 는 세로 쌓기이고 한 줄이던 것은 `.ac-runbar-top` 이다.
  ⚠ 읽은 값을 문장으로 늘어놓던 자리(`acMsg`)와 중첩 안내·남은 배치·HP·밴 개수는 **일부러 뺐다**
    (사용자 지시: "이거 전부 필요없고, 이자리에 밴 목록 버튼을" · "인게임에서 보는게 편하지"). 인식은
    계속 돌고 브리지 토스트(`noteBridge`)에만 남는다 — 그쪽은 인식이 살아 있다는 신호라 값이 필요하다.
- 전략 칩은 **모달이 아니라 런바에** 둔다 (사용자 지시 "현재 전략은 굳이 모달창 필요 없이 바깥에다").
  누르면 전략 상세가 열리고, 연합은 자리를 최대 4명까지 준다 — 못 읽은 자리는 '읽는 중'으로 남긴다.
- 관측만 있고 아직 아무도 못 붙인 줄은 남기고 `—` 를 그린다. 사용자가 보여 준 줄이 사라지면 "왜 안 잡히지"가 되고,
  관측이 모순이라 풀이가 해를 못 낸 상황을 숨기게 된다. 그룹 머리의 `확정수/화면에서 본 카드수` 가 그걸 설명한다.
- 조합 풀이(`acsolve`)의 **'확정' 버그도 같이 고쳤다**: 해를 상한(64)에서 끊고 교집합을 내면 교집합이 과하게
  좁아진다. 실측 — v1 (쉐라그,T4) 자리에서 정답 실버애쉬가 통째로 빠지고 노시스가 '확정' 으로 나왔다.
  이제 **상한에 걸리면 확정을 내지 않는다**(`truncated`). 스크롤 초반이 늘 그 상태다.

#### 연합 참가자 줄 — 앵커를 갈아엎었다 (2026-09-07 오후)

사용자 신고 "다른사람 전략도 다 띄워줘야함". 원인은 **1인 녹화만 보고 맞춘 기하**였다. 첫 구현
(`participantSlots`)은 카드 좌상단의 청록 배지를 '참가자' 배지로 읽고 앵커로 삼았는데, 그건 사실
**내 카드에만 붙는 '나' 표식**이었다 — 연합 4인 화면에서 남의 줄은 한 줄도 안 잡혔고, 잡힌 한 줄마저
"내 전략과 같으면 내 카드" 규칙에 걸려 버려졌다. 결과는 상대 전략 0명.

새 앵커는 **줄 오른쪽 체크박스**다. 전략을 고른 줄에만 켜지므로 "읽을 썸네일이 있다" 와 조건이 정확히
같다. 밝은 청록을 1px 팽창시키면 체크 글리프(16×15)와 네모 테두리(24×23)가 한 덩어리가 되어 **거의
꽉 찬 정사각**(채움 0.94~0.97)이 되고, 화면의 다른 청록 글자 조각(0.31~0.82)과 갈린다.
기하는 체크박스 마스크 변 단위로 썸네일 중심 −1.80·변 2.25 — dx −2.2~−1.7 · 변 2.0~2.5 를 훑어
**전 구간 오답 0** 이라 값에 예민하지 않다. 1826px 1인 화면과 1920px 4인 화면이 같은 비율이었다.

**기하는 후보만 내고 판정은 `searchBand` + `BAND_ACCEPT` 가 한다** (밴 인식의 두 겹 게이트와 같은 규약).
진짜 줄 0.78~0.87 vs 헛자리 0.29~0.41 이라, 화면 어딘가의 청록 정사각 하나가 없는 줄을 만들어 내지 못한다.
내 줄은 '나' 표식으로 가른다 — 우측 패널은 **돋보기로 남의 전략을 열면 그 사람 것을 보여주므로** 내 것의
근거가 못 된다 (표식을 못 찾을 때만 패널 값과 대조하는 폴백).

자리 번호는 '전략을 고른 줄' 중의 순서라 **누가 새로 고르면 아래 줄이 한 칸씩 밀린다**. 그래서 저장소는
...
#### 남은 것 (2026-09-07 기준 — 픽스처가 없어 못 검증한 것)

- **독립/연합 판정**: `#1234` 의 `#` 이 900px 에서 `1` 로 읽혀 `parseSeats` 가 0/9 — 상대 전략이 잡히면 연합으로 본다.
- **맹약 링 숫자**: tesseract 가 단독 `0` 을 O/()/C 로 낸다. 매핑으로 10/35. 다음 라운드(1 이상 값) 녹화가 오면 숫자 글리프
  템플릿 매칭으로 바꿀 것.
- **배치·정비구역 기물 확정 동기화**: 보드 위 치비 스프라이트는 못 읽는다. 상점 카드·툴팁 이름을 "본 기물" 로만 모은다 —
  구매·판매·이동을 따라가는 상태기계는 그 조작이 찍힌 녹화가 있어야 만든다.
- EN/JA 클라 문구는 데이터(`modes[].n`·`bands[].by`) 로 갈음했고 실제 화면은 미검증.
...
합쳐서 총 +2가 중첩된다는 말이야? 아니면 진짜로 그냥 중복인 거야?"*

**답: 합산이 아니라 표시 중복이다** (산탈라 하나만 예외). 클뜯의 능력 하나는
**설명문(`garrisonDesc`)** 과 **구현(`blackboard`)** 을 따로 갖는데, 구현은 조각 하나만
담고 설명문은 그 기물의 **최종 효과 전체**를 적는다. 그래서 조각이 둘이면 첫 카드의
설명문이 둘째 카드의 내용을 이미 포함하고, 화면에서는 수치가 두 배로 붙는 것처럼 읽힌다.

특질이 둘 이상인 기물 9종 전수 확인 (2026-08-26):

| 기물 | 실제 총 효과 | 겹친 이유 |
|---|---|---|
| 그라벨 | 배치 시 [카시미어] **+1**, 쓰러질 시 [불굴] +2 | `75`는 불굴만 구현하는데 설명문이 카시미어 줄까지 적음 |
| 데겐블레허 | 획득 시 **+8**, 배치 시 +8(전투당 최대 24) | `155`는 배치만 구현하는데 설명문이 획득 줄까지 적음 |
| 해럴드·틴맨·로즈몬티스 | 공·HP +20% ＋ 3중첩당 +1% | `03`이 ×1.2, `09`·`151`이 +1%/3중첩 — `09` 설명문이 둘 다 적음 |
| 메테오 | 공·HP +25% ＋ 약점 대미지 변환 | `137`이 ×1.25, `01`이 변환 — `137` 설명문이 둘 다 적음 |
| 이그제큐터 | [라테라노] +7 · [예견] +3 | `23`이 라테라노, `55`가 예견 — **두 설명문이 서로 양쪽을 다 적음** |
| 니어 더 래디언트 나이트 | 재배치 -1.5% ＋ 공속 +0.5 | `145`→`144`(재배치), `160`→`159`(공속). 공속은 +0.5지 +1.0이 아니다 |
| **산탈라** | 예견 **+8** | `141` 휴식 진입 +4, `142` 휴식 종료 +4 — **발동 시점이 달라 진짜로 둘 다 터진다** |

확정된 규칙 (`build-autochess.py`의 `gar_trigger`·`gar_line_keep`·`dedupe_gar`):
- **판정 기준은 발동 시점**(`blackboard`의 `key`·`eventType`)이지 글자 유사도가 아니다.
  ⚠ 산탈라(유사도 0.848)가 그라벨(0.650)보다 더 닮았는데 산탈라는 지우면 안 된다 —
  글자로 자르면 **오답이 나온다.**
- 여러 줄 설명에서 `<배치 시>`·`<쓰러질 시>`·`<획득 시>` 줄이 **형제 능력의 발동 시점**이면
  그 줄은 형제 몫이라 여기서 뺀다 (그라벨 `75`, 데겐블레허 `155`).
- 카드 통째 제거는 **발동 시점이 같을 때만** 한다. 비교는 강조(`**`)·괄호 주석·따옴표를
  지운 뒤 **부분문자열 또는 토큰 부분집합** — 니어2처럼 긴 쪽이 문장 중간에 구절을 끼워
  넣으면 부분문자열로는 안 걸리고, 니어2 `145`는 부여할 특질을 `'…'`로 감싸서 따옴표를
  안 지우면 토큰이 어긋난다.
- 판정은 **한국어에서 한 번만** 하고 그 결과(줄 번호·능력 id)를 EN/JA에 그대로 적용한다.
  세 로케일의 줄 수가 같은 것은 전수 확인했다 — 로케일마다 따로 판정하면 갈릴 수 있다.
- 분류 태그(`tg`/`evb`/`bs`)는 **손실 0건**이어야 한다 (남은 능력이 같은 맹약을 언급하므로).
  정리 후 전수 대조로 확인할 것 — 여기가 깨지면 특질 필터와 시뮬레이터가 같이 틀어진다.

화면 쪽 함정도 같이 고쳤다 (`app/globals.css`): 맹약 모달의 오퍼 행은 특질을 `<span>`으로
나열하는데 인라인이라 **두 능력이 한 줄로 붙어** 나왔다. `.ac-oprow-gar > span`을 블록으로
세우고, 원문이 두 줄인 특질 6종을 위해 `.ac-gar-txt`에도 `white-space: pre-line`을 준다
(스킬·모듈 `.ac-skmod-d`는 이미 그랬는데 특질만 빠져 있었다).
...
### 번역 사전 공개본 — 밖에 내주는 유일한 창구 (2026-09-17)
...
증분은 **파일 단위 해시**로 한다. `updated`는 파이프라인이 돈 날짜라 내용이 안 바뀌어도
매일 올라가서 증분 기준으로 못 쓴다 (실측 2026-09-17: 19개 파일이 전부 날짜만 바뀌고
내용은 바이트 단위로 동일했다). 그래서 manifest에 `hash`를 싣고, **`files[].url`에 그
해시를 `?v=`로 박아** 둔다 — 받는 쪽이 주소를 조합하지 않아도 되고, 내용이 바뀌면 주소가
바뀌니 중간 캐시가 옛 바이트를 돌려줄 창이 닫힌다. 도메인을 옮겨도 받는 쪽은 manifest만
다시 받으면 따라온다.

**내보내는 길은 사이트와 완전히 갈라 뒀다** (사용자 지시 2026-09-17 "공개 API 업로드
스크립트는 완전히 따로 빼야지"). `scripts/publish-tl.mjs`가 `assets/tl/` 안에서만 다루고,
`r2-sync.mjs`의 `DIRS`에 `tl`은 **없다**. 받는 쪽이 사이트가 아니라 남의 앱이라 나가는
시점·주기·사고 영향이 다르기 때문이다. 한 번 섞었더니 양쪽으로 샜다 — 공개본을 내리려다
사이트 에셋 1,614개(167MB)가 딸려 갔고, `--recache` 한 번에 11,681개(321MB)가 올라갔다.
`deploy.sh`의 트림 목록에만 `tl`이 남아 있다(Pages 중복 서빙 방지).

⚠ **공개 문서는 R2 헤더까지 봐야 한다.** `.md`가 `application/octet-stream`으로 나가면
브라우저가 페이지로 그리지 않고 내려받아 버리고, 캐시가 30일이면 **고쳐도 한 달간 옛
문서가 나간다**. 2026-09-17에 둘 다 실제로 그랬다 — 규격서 주소가 열리지 않았다.
`publish-tl.mjs`는 올린 뒤 공개 주소로 되읽어 manifest 일치와 README 형식을 확인한다.
...
- 팔레트: `--ink #131719 / --paper #f1f0eb / --lime #dfff00` 계열.
- **오퍼 카드 색(`--accent`)은 직군에서 계산한다** (2026-09-18 전면 교체).
  `home.tsx`의 `JOB_HUE` + `accentOf()` — 직군(`jobCode`)별 색상대에 id 해시로 ±8° 변주를
  주고 **채도 18% · 명도 34%로 고정**한다. 빌드마다 같은 색이 나오므로 데이터에 저장하지
  않는다. `operators.json`의 `accent` 필드와 `regen-operators.py`의 `NEW_ACCENTS`는
  **더 이상 쓰이지 않는다** (신규 오퍼에 색을 손으로 넣을 필요가 없다).
  - 종전엔 손으로 고른 54색을 444명에게 나눠 준 값이었다 — 같은 색이 73명씩 겹치고,
    직군·진영·성급 무엇과도 대응하지 않았으며(직군별 통일 0/8) 27명은 기본 회색이었다.
  - ⚠ **채도를 올리지 말 것** (사용자 지적 2026-09-18 "너무 형형색색한데"). 42%로 잡았다가
    카드만 무지개가 됐다. 18%면 직군 차이가 따뜻함/차가움 정도로만 남아 먹색으로 읽힌다.
    직군 이름은 카드에 글자로 이미 있으니 색이 직군을 혼자 설명할 필요가 없다.
  - 성급이 오를수록 채도·명도가 조금씩 올라간다(`12 + rarity*2`% / `30 + rarity`%) —
    인게임도 레어도가 높을수록 화려하다. 폭을 좁게 잡아 여전히 먹색으로 읽힌다.
- **오퍼 카드 — 얼굴이 주인공, 나머지는 호버/꾹으로 펼친다** (2026-09-18 재설계).
  카드 = 초상 칸 하나(5:6)이고 **2행 그리드**다: 위는 얼굴(`1fr`), 아래는 이름 패널(`auto`).
  위 줄(절대배치)에 성급 별 + 직군, 패널엔 접힌 채 이름 한 줄. **호버/꾹/포커스**하면 카드가
  `scale(1.14)`로 뜨고 패널의 안쪽 행이 `0fr→1fr`로 열려 소속·출신·종족·컨셉 태그가 나온다.
  클릭은 상세 모달.
  - **⚠ 얼굴 밑변은 라임 선(패널 윗변)에 늘 붙어 있어야 한다** (사용자 확정: "섬네일의 밑
    변은 연두색 줄에 맞춰서 늘어났다 줄어들었다 항상 거기 붙어있어야함"). 이건 **구조로**
    보장한다 — 패널이 내용만큼 자라면 얼굴 행(`1fr`)이 그만큼 줄고, `img`는
    `object-position: center bottom` 이라 그려진 얼굴의 밑변이 행 밑변에 붙는다.
    **패널 높이를 px 로 박고 아바타를 거기 맞추지 말 것** — 그렇게 했다가 태그가 두 줄이
    되면 패널 밖으로 잘렸다. 숨 쉴 틈(위 16·옆 13)은 `img` 의 padding 이다.
  - **넘치는 글자는 한 줄인 채 흐른다(마퀴)** — 이름·태그 둘 다. 줄바꿈으로 갔다가
    되돌린 자리다(사용자 정정: "한 줄로 하고 자동으로 왼쪽으로 점점점점 흘러가는 방식").
    - 이동량은 `min(0px, calc(100cqw - 100%))` — **넘칠 때만** 움직인다(`100cqw` = 상자 폭,
      `100%` = 흐르는 요소 제 폭). JS 없이 되는 유일한 방법이라 `container-type: inline-size`
      가 상자에 필요하다.
    - ⚠ 흐르는 요소는 **`inline-block`** 이어야 하고(인라인 박스엔 transform 이 안 먹는다)
      **`max-width` 를 걸면 안 된다**(상자 폭에 갇히면 이동량이 늘 0). 둘 다 당했다.
    - 한 방향 무한 루프 — 왕복·가속 금지(사용자 지시). **앞 0.6초·뒤 2초는 멈춰 서서
      읽을 틈**이고, 이건 **고정 시간**이라 퍼센트 키프레임으로는 못 만든다(지속시간이
      글자마다 달라 같은 퍼센트가 다 다른 초가 된다). 그래서 키프레임은 0→끝 직선만 긋고,
      멈춤은 `home.tsx` 가 심는 `linear(0 0%, 0 p1%, 1 p2%, 1 100%)` 이징의 평평한 구간이
      맡는다 — 퍼센트를 계산해 넣을 수 있는 유일한 자리다.
    - **세로 마퀴도 있다** — 소속·출신·종족(`.portrait-facts`)은 두 줄까지만 보이고 넘치면
      아래로 흐른다(실측 8장: 만트라·라이디언·Mon3tr·블레이즈 등 소속이 긴 오퍼).
      이동량이 픽셀이라 `--flow-y` 로 넘긴다 — 부모 높이가 `max-height` 로 정해져 있어
      `100cqh` 가 내용 높이를 안 알려주기 때문(가로처럼 컨테이너 단위를 못 쓴다).
      ⚠ `.portrait-facts` 는 `<small>` 이라 **`display:block` 을 꼭 줘야 한다** — 인라인이면
      `clientHeight` 가 0 이라 "전부 넘친다"고 잘못 재진다.
    - **가로와 세로는 서로 다른 요소가 맡는다** — 이름·태그는 가로, 소속 블록은 세로.
      한 요소에 두 축이 걸릴 일이 없다(실측 동시 넘침 0장). 소속 값이 길면 옆으로 넘치는
      게 아니라 어절 단위로 줄바꿈되어 아래로 자라기 때문이다.
    - **속도는 40px/초로 고정**(`FLOW_PX_PER_SEC`). 지속시간을 고정하면 긴 글자일수록
      빨라져 못 읽는다(사용자 지적) — 그래서 `home.tsx` 가 카드가 보일 때 넘친 거리를 재서
      비례하는 시간을 `--flow-ms` 로 심는다. 안 넘치면 `0ms` 라 애니메이션이 아예 안 돈다.
      웹폰트가 늦게 오면 글자 폭이 바뀌므로 `document.fonts.ready` 뒤에 한 번 더 잰다.
      (뷰포트를 리사이즈하면 거리가 달라져 속도가 살짝 어긋난다 — 다시 재지는 않는다.)
    - 호버와 무관하게 늘 돈다. 화면 밖 카드는 `content-visibility: auto` 라 렌더를 건너뛴다.
    - ⚠ 칩 스타일은 **`.tags-track > span`** 으로 — `.tags span` 으로 쓰면 마퀴 래퍼까지
      칩으로 칠해져 태그 전체가 상자로 묶이고, 래퍼가 첫 자식이라 라임 배경까지 먹는다.
    - ⚠ 전역 리셋 `:where(…, h3)` 이 h3 에 radius 를 물린다 — `overflow:hidden` 과 겹쳐
      글자 모서리가 깎이므로 `border-radius: 0` 으로 끈다.
  - **미니 카드(미실장 스트립)는 펼치지 않는다** — 소속·태그를 숨겨 놔 펼칠 내용이 없는데
    선만 올라갔다(사용자 지적). 확대만 한다. 이름은 길면 **두 줄로 흘린다**(말줌표 금지).
  - **호버 확대가 잘리지 않게 `.results-scroll` 에 `padding: 14px 14px 0`** 을 둔다.
    overflow 는 **패딩 박스**에서 자르므로 이 패딩이 곧 삐져나갈 자리다. 음수 마진으로
    되돌리거나 그리드에 패딩을 주면 안 된다 — 부모 `.results` 도 `overflow:hidden` 이고,
    그리드 쪽은 가로 스크롤이 생긴다(둘 다 해보고 확인).
  - **터치는 꾹 누르면(350ms) 펼치고, 한 번 누르면 모달**. `pointerdown(touch)` 350ms 뒤
    `.peek`, 떼면 닫히고 그 클릭은 모달로 안 간다. iOS 링크 미리보기는
    `-webkit-touch-callout:none`, 안드로이드 컨텍스트 메뉴는 `onContextMenu` 로 막는다.
    키보드는 `:focus-visible` 로 같이 펼친다.
  - **카드는 전부 테마를 따른다** — 껍데기뿐 아니라 초상 바탕·패널·글자까지 토큰
    (`--white`/`--bg-soft`/`--ink`/`--ink2`/`--ink3`/`--muted`/`--line`)으로 쓴다.
    한때 초상·패널만 늘 어둡게 뒀다가 "라이트모드일때는 라이트모드 색에 맞춰줘야지"로
    지적받았다. 다크에서만 필요한 글자 그림자는 `html.dark` 로 따로 준다.
    직군색 광도 테마별로 섞는 상대가 다르다(라이트는 `--white` 34%, 다크는 흰빛 82%) —
    채도 낮은 accent 를 그대로 쓰면 어느 한쪽에서 안 보인다.
    - 라이트에선 **4·5성 별이 종이 위에서 날아간다** — 같은 색 계열을 유지하되 명도를
      낮추고(`#5468c4`/`#bd8a12`) 옅은 그림자로 띄운다. 6성 그라데이션은 `filter` 로.
  - **라임은 카드에서 첫 태그 한 곳뿐** — 패널 윗선의 라임 가로줄은 2026-09-18에 뺐다
    (사용자 "필요 없는거같지 않아?"). 첫 태그 글자색은
    `color-mix(lime 60%, --ink)` — 라임 원색은 흰 바탕에서 1.9:1 이라 안 읽힌다.
  - **섬네일 가장자리는 배경에 녹인다**(`mask-image`, 좌·우·위 3변). 밑변은 **건드리지
    말 것** — 라임 선과 맞닿는 자리라 흐리면 선이 뜬다. 퍼센트는 패딩 포함 박스 기준이라
    앞 8%는 패딩에서 소모된다(15%로 뒀더니 얼굴엔 6px 남짓만 닿아 "사라졌다"는 소리를
    들었다 → 26%).
  - 초상 배경은 조용하게: 직군색은 얼굴 뒤 은은한 radial 광으로만. 인게임식 사선 띠·하프톤은
    넣었다가 뺐다(사용자 "굳이 있어야 함?"). 되살릴 생각이면 커밋 `62ca61a7`.
  - 그리드는 **5열** → ≤1100: 4 → ≤760: 3 → ≤430: 2. 컨테이너 860px라 데스크탑은 늘 5열.
- **오퍼 상세 모달 헤더(`.modal-hero`)** — 높이는 제목 쪽 내용이 정한다(종전 `min-height:150px`
  폐기). 글자·여백을 줄여 **175px → 96px** 로 절반이 됐고, 성급과 직군을 한 줄로 흘린다
  (`.modal-title-main` 이 flex-wrap, 키커와 이름만 `flex-basis:100%`).
  - ⚠ 섬네일에 **제 배경을 주지 말 것** — 얼굴은 정사각이라 헤더가 조금만 높아도 위가
    남는데, 거기에 헤더와 다른 그라데이션이 깔려 "이유 없는 빈칸"으로 보였다(사용자 지적).
    배경을 빼면 남는 자리는 그냥 헤더 배경이라 이어져 보인다. 가장자리는 카드와 같이
    `mask-image` 로 좌·우·위를 녹인다.
  - 카드 색 `--accent` 는 `accentOf()`(직군 색상대 + id 해시 변주, 채도 14~24%) — 위 항목.
