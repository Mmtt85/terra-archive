...
| 배포 | https://terra-archive-kr.nzkonaru.chatgpt.site — **ChatGPT 사이트 빌더로 수동 재배포** (git push 해도 자동 반영 안 됨. 작업 후 사용자에게 재배포 리마인드) |
...
- **시너지 팟 10종** — 컨셉 필터 **최상단 고정** (page.tsx의 `SYNERGY_POTS` 순서):
...
- **보유 오퍼 설정**: 전원 표시하되 1~5성은 기본 보유 체크, 6성은 기본 미보유.
  정렬 = 6성 우선 → KR 출시 최신순. "적용"만 (자동편성 자동 실행 안 함).
