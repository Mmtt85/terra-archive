...
## 사람 손이 필요한 일 = 스킬 딸깍 (이메일이 어떤 스킬 실행할지 콕 집어준다)

도메인 판단이 필요해 자동화하지 않은 일은 **전용 스킬**로 정리돼 있다. 무인 리포트 이메일의
"🖐 사람 손 필요" 섹션이 항목마다 실행할 스킬 이름을 명시하므로, 세션 열고 그 스킬만 돌리면 된다.

| 이메일이 지목하는 항목 | 실행할 스킬 | 하는 일 |
|---|---|---|
