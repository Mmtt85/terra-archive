...
1-B. **적 도감도 같은 구조의 함정이 있다** (2026-08-09 신설).
   `build-enemies`는 CI에서 **`--meta-only --no-images`**로 돈다. 이유가 둘이다:
   ① 등장 작전 역색인을 만들려면 `levels/` **2,283개(179MB)** 를 받아야 하는데 러너에서
   매번 할 일이 아니다. ② 적 초상 1,514장도 `--meta-only`로는 안 받는다.
   그래서 **새 이벤트가 들어오면 CI는 도감 본문만 갱신하고, 그 이벤트 스테이지의 등장 적과
   신규 적 초상은 반영되지 않는다.** 신규 적이 보이면 로컬에서
   `python3 scripts/build-enemies.py`(인자 없이 전체)를 한 번 돌려 R2에 동기화할 것.
   `--meta-only`는 기존 `enemy-stages*.json`을 **덮지 않고 그대로 둔다** — 반쪽 색인으로
   교체돼 등장 작전이 통째로 비는 사고를 막기 위한 것이니 이 동작을 바꾸지 말 것.
